...
:::note
Account verification for funding sources **isn't related** to [account holder verification status](/accounts/guides/onboarding/account-holders#verification-process-statuses) used during account onboarding, or the general [account status](/accounts/concepts/account#statuses).
:::
...
The `TransactionRollingReserve.Updated` and `TransactionRollingReserve.Released` [webhooks](/dev-tools/using-api/webhooks#events-transactions) notify you when a rolling reserve is updated or released. 
...
Account holders and eligible account members can cancel SEPA Direct Debit funding transactions using the [`cancelTransaction`](https://api-reference.swan.io/mutations/cancel-transaction/) mutation. 
...
- [Add a funding source](/accounts/guides/funding/add-source)
- [Initiate a funding request](/accounts/guides/funding/initiate-request)
- [Cancel a funding source](/accounts/guides/funding/cancel-source)
- [Get information about a funding source and request](/accounts/guides/funding/get-info)
