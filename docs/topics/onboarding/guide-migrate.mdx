...
Previous onboarding mutations and queries are deprecated and will be removed in **December 2026**.
...
The mapping tables will be available until the previous mutations are retired in December 2026.
...
- The `finalizeAccountHolderOnboarding` mutation returns an `AccountHolderOnboarding` union type.

Update your response parsing to handle the new type names and field structure.

### Step 5: Update registry data handling (company onboardings)

If you onboard French companies and relied on automatic field population from official registries, you need to update your workflow.

The current API does **not** auto-populate onboarding fields with registry data.
...
In-progress onboardings created with the previous API are not affected by the migration.
...
After the previous mutations are removed in December 2026, all in-progress onboardings using the previous API must already be finalized.
Plan your migration timeline to ensure no onboardings are left in an incomplete state.
...
| December 2026 | Previous mutations and queries removed. |
...
1. Confirm webhook payloads still match your expectations for onboarding status changes.
