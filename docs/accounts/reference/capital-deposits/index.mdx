...
<p className="ia-lede">Status models, cancelation reason codes, and required documents for <Term id="capital-deposit">capital deposit</Term> cases.</p>
...
| `WaitingForVerification` | Possible status if the account holder verification isn't complete at the moment the shareholder's account is created.<br/><br/>Status bypassed if the [account holder status](/accounts/concepts/account-holders/verification#verification-process) is already `Verified`. |
