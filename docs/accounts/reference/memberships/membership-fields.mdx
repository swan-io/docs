...
Field-level requirements and validation rules for <Term id="account-membership">account membership</Term> data. Use this alongside the [membership permissions reference](/accounts/reference/memberships/membership-permissions); for the model and lifecycle, see [Account memberships](/accounts/concepts/memberships).
...
**Requirements vary depending on the <Term id="ibans">IBAN</Term> country**. Swan automatically sets these requirements based on the member's permissions, `accountCountry`, and `residencyAddress.country`.
...
| Field | 🇫🇷<br/>France | 🇧🇪<br/>Belgium | 🇩🇪<br/>Germany | 🇳🇱<br/>Netherlands | 🇪🇸<br/>Spain | 🇮🇹<br/>Italy |

### Detailed optional and conditional requirements {#detailed}
...
These validation rules are the same as those applied when [creating users](/users/concepts/user).
:::
