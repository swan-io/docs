...
Operational tasks for existing <Term id="account-holder">account holders</Term>: add another account, check verification status, request a first transfer, export data, and simulate status changes in Sandbox.
...
But since you can only simulate updating one account holder status at a time, and from your Dashboard, it's simpler to get the ID from the Dashboard.
...
![Screenshot of Swan Dashboard account holders page with a callout of the account holder ID](../../../images/accounts/onboarding/account-holder-id.png)

#### Step 2: Update the status {#simulate-status-update-make-update}

With the account holder ID, use the Event Simulator to change the [account holder verification status](/accounts/concepts/account-holders/verification-statuses) to any available status.
...
![Simulate changing an account holder's verification status with the Event Simulator](../../../images/accounts/onboarding/sandbox-account-holder-verif-status.png)
