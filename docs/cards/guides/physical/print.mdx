...
---
import DeliveryAddressInfo from '../../partials/_deliver-address-callout.mdx';

# Print physical cards

Learn how to print <Term id="cards-physical">physical cards</Term> with the API.

If you use Swan's Web Banking interface, your users can [order their own cards](https://support.swan.io/hc/en-150/articles/17791219525149-Order-your-physical-card) if they have the correct [account membership permissions](/accounts/reference/memberships/membership-permissions#permissions-cards).
They can also review their card's [spending limit](https://support.swan.io/hc/en-150/articles/18989382227357-Spending-limits).
Swan's Support Center provides more information about [physical cards](https://support.swan.io/hc/en-150/articles/18133963401501-Physical-cards) to help your users.

:::tip Prerequisites
1. Confirm the account member has the [correct account membership permissions](/accounts/reference/memberships/membership-permissions#permissions-cards) to manage cards. To use `addCards` or `addCardsWithGroupDelivery`, they must also be able to manage account memberships.
1. To use the `printPhysicalCard` mutation, [add a virtual card](/cards/guides/virtual/add) **before** printing a physical card. With `addCards` and `addCardsWithGroupDelivery`, the virtual card is created in the same call.
:::
...
1. Allow the cardholder to choose their own PIN by setting the `choosePin` parameter to `true` (line 12).
...
1. Add the `accountMembershipId` (line 5).
1. Choose [card settings](/cards/reference/card-settings#settings) and set the spending limit (lines 6-13).
1. Add `physicalCard`, plus the delivery address (line 14). This parameter allows printing the physical card at the same time as adding the virtual card with the same mutation.
1. Allow the cardholder to choose their own PIN by setting the `choosePin` parameter to `true` (line 21).
1. Add the consent redirect URL.
1. Add the success payload, including any information you'd like to review (line 27).
1. Add the consent URL to the success payload (line 36): `statusInfo` > `CardConsentPendingStatusInfo` > `consent` > `consentUrl`.
1. Add rejections (not shown).
...
```graphql {2,5,6-13,14-15,21,24,27,33,36,40} showLineNumbers
...
Notice the [API card type](/cards/concepts/card-formats#formats-api) (included in the mutation, line 40) is `VirtualAndPhysical`, which is one benefit of the `addCards` mutation.
...
1. Add a `card` block for each card you'd like to print (example: lines 15-27).
    ...
    1. Choose [card settings](/cards/reference/card-settings#settings) and set the spending limit (lines 17-24).
    1. Set the `printPhysicalCard` parameter to `true` (line 25).
...
If approved by your PIM (Product Integration Manager), you can use number plates instead of names for your physical cards with all of the mutations outlined on this page.
...
1. Add the `cardId`.
1. To get delivery details, add `physicalCard` > `statusInfo` > `PhysicalCardToActivateStatusInfo`, then all information you'd like to retrieve. Refer to Swan's [card delivery times](/cards/concepts/physical/delivery-and-shipping#hubs) for more information.
