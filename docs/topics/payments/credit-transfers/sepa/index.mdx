...
:::info
Swan automatically includes [Verification of Payee (VoP)](/topics/payments/credit-transfers/sepa/vop/verification-of-payee) in all SEPA Credit Transfers to comply with the EU Instant Payments Regulation.
:::
...
You can include an **optional** [invoice reference](./guide-initiate-ct#invoice-reference.mdx) when you initiate a SEPA Credit Transfer using the `labelType` and `label` fields.
...
| `OgmVcs`| [Belgian OGM VCS standard](https://www.europeanpaymentscouncil.eu/sites/default/files/inline-files/Febelfin%20-%20AOS-OGMVCS_0.pdf), a 12-digit reference number used for payment reconciliation. | `123456789012`       |
...
Learn more about [limited account restrictions](../../../onboarding/account-holders/index.mdx#limited-account-restrictions).

### Sequence number {#sequence-number}

Swan assigns a `sequenceNumber` to every **Booked** transaction.
The number increments by 1 with each new booked transaction for each account, giving you a reliable way to order transactions and spot any gaps.
...
The `sequenceNumber` field is only present when the transaction status is **Booked**. It returns `null` for all other statuses.

To get `sequenceNumber`, [query](https://explorer.swan.io?query=%3D%3D&tab=api) `statusInfo` with the `BookedTransactionStatusInfo` fragment.
...
- [Verification of Payee](./vop/verification-of-payee.mdx)
- Standing Orders
