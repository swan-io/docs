...
This is an ongoing regulatory requirement, not a one-time onboarding event. Swan performs periodic re-verification to keep account holder information up to date.
...
Account verification is an ongoing regulatory requirement rather than a one-time onboarding event. Your users will be re-verified at another stage. 
:::
...
## Periodic re-verification {#re-verification}

To comply with ACPR standards, Swan must periodically verify the information we have for every account holder. 
If an account holder is required to complete periodic re-verification, it applies across all accounts held by that account holder.

Periodic re-verification is a mandatory process occuring every 1, 3, or 5 years, or whenever significant changes are detected in an account holder's profile. 

:::warning Account suspension
Users must complete re-verification **within 90 days**. If they don't, we will automatically suspend their account and restrict all related IBANs, cards, and payment capabilities until verification is complete.
:::

### Completing re-verification in the portal {#re-verification-portal}

Swan provides a no-code, partner-branded portal to handle the re-verification process. No API integration is required on your side.
...
#### Re-verification steps {#re-verification-portal-steps}

The legal representative (account admin):

1. Receives the [initial notification email](/topics/onboarding/account-holders/#re-verification-timeline) with a unique portal link.
...
1. Submits the re-verification request and receives a confirmation email. 

The KYC analyst reviews the submission.
If more information or supporting documents are needed, they send a follow-up request with a new link. 
The portal reopens for the missing items only.

After Swan validates the review, the account holder's [`VerificationRenewalStatus`](https://api-reference.swan.io/enums/verification-renewal-status/) status changes to `Completed`. The account holders banking verification status isn't impacted by verification renewal. 

### Re-verification timeline and reminders {#re-verification-timeline}

The  **90-day window** for the periodic re-verification process starts after the initial notification is sent to your user.
...
We recommend having Swan email your end users directly, with you copied (CC'd). Create a ticket outlining your communication preferences for re-verification. 
You can also specify a unique email address.
:::

| Milestone | Timeline | What happens |
| :--- | :--- | :--- |
| **Start of Re-KYC** | Day 0 | Swan sends an email to the legal representative with a unique portal link. |
| **First reminder** | Day 30 | Your user receives a reminder if they haven't completed the portal flow. |
| **Second reminder** | Day 60 | Your user receives a second reminder if they haven't completed the portal flow. |
| **Final reminder** | Last week | Your user receives a final reminder one week before the deadline. |
| **Re-verification deadline** | Day 90 | Swan suspends all of the account holder's accounts. |

:::note Steps after suspension
- The user can still complete re-verification after suspension using the latest portal link Swan emailed them.
- After they complete the flow, you or the user must submit a support request so a KYC analyst can review the case and lift the suspension.
- Swan may close the account if it remains suspended for too long without completing re-verification.
:::
