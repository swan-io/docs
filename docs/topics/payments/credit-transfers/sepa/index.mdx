...
:::warning
Starting October 9, 2025, Swan automatically includes [Verification of Payee (VoP)](/preview/vop/verification-of-payee) in all SEPA Credit Transfers to comply with the EU Instant Payments Regulation.
:::
...
The `beneficiaryVerification` query will be deprecated. Plan [your migration](/preview/vop/verification-of-payee) before the compliance deadline to ensure uninterrupted service.
:::
...
Use `labelType` to specify the format of the reference, and provide the specific value of the reference in the `label` field.
...
| `Iso` | [ISO 11649](https://www.iso.org/standard/50649.html), the international Structured Creditor Reference standard, widely used in Europe.  | `RF18539007547034` |

The invoice reference is returned in the `transaction.label` when you retrieve transaction data.
