---
title: Using the API
audience:
  - dev
counterpart: /users/guides/user-operations/deactivate/from-the-dashboard
related:
  - /users/guides/user-operations/deactivate
  - /users/guides/user-operations/deactivate/from-the-dashboard
  - /users/concepts/user/deactivation
  - /users/concepts/user/statuses
---

# Deactivate a user using the API

You can **call the API** to deactivate a user.

import DeactivateRequirements from '../../../partials/_deactivate-reqs.mdx';

:::tip Prerequisites
All of the following conditions must be met:

<DeactivateRequirements />

You also need a [**project access token**](/build/using-api/authentication#tokens-project) to call the mutation.
:::

## Guide {#guide}

1. Call the `deactivateUser` mutation.
1. Add the user ID for the user you need to deactivate.
1. Add success payload (line 3) and **all** rejections (lines 6, 11, 16).

## Mutation {#mutation}

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {2,3,6,11,16} showLineNumbers
mutation DeactivateUser {
  deactivateUser(input: { userId: "$USER_ID" }) {
    ... on DeactivateUserSuccess {
      __typename
    }
    ... on UserCannotBeDeactivatedRejection {
      __typename
      message
      userId
    }
    ... on UserAlreadyDeactivatedRejection {
      __typename
      message
      userId
    }
    ... on UserNotFoundRejection {
      id
      message
    }
  }
}

```

## Success payload {#payload-success}

The following payload shows a successful deactivation.

```json {4} showLineNumbers
{
  "data": {
    "deactivateUser": {
      "__typename": "DeactivateUserSuccess"
    }
  }
}
```

## Rejection payload {#payload-rejection}

The following payload shows an unsuccessful deactivation.
This user couldn't be deactivated because they're a <Term id="legal-representative">legal representative</Term>.

```json {4,5} showLineNumbers
{
  "data": {
    "deactivateUser": {
      "__typename": "UserCannotBeDeactivatedRejection",
      "message": "User with id '$USER_ID' can't be deactivated (reason=The user is a legal representative)",
      "userId": "$USER_ID"
    }
  }
}
```
