...
|:---:|---|
| `PendingVerification` | The funding source was added, and account verification for the funding source is in progress.<br /><br />The funding source status can be `Enabled` even if the account verification status is `PendingVerification`. |
| `PendingReview` | Swan couldn't automatically match the external account's IBAN from the last received transfer, and a manual review is underway.<br /><br />**Next step**: Moves to `Verified` or `Rejected` after the review. |
