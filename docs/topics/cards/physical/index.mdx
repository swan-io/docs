...
You can [print a physical card](./guide-print.mdx#print-with-virtual) for another account member with the `addCard` mutation, allowing the member cardholder to choose their PIN by setting the `choosePin` paramater to `true`.
...
When [printing physical cards](./guide-print.mdx) with the `printPhysicalCard`, `addCards`, or `addCardsWithGroupDelivery` mutations, the [address fields](https://api-reference.swan.io/inputs/complete-address-input/) are designed in a specific manner.
...
| `addressLine1` | Partner (you) | Delivery address line 1<br />*(**don't** include the company name here)* | 123 avenue de Paris |
| `addressLine2` | Partner (you) | Optional additional address information | Suite 26 |

∗ You can provide a different `firstName` and `lastName` for `addCardsWithGroupDelivery`.
