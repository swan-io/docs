...
- **Account holder verified:** If the transfer comes from an accepted source, the account holder verification status changes to `Verified`.

- **Account holder refused:** If the transfer source can't be accepted, the account holder verification status changes to `Refused`.

- **Additional transfer requested:** If the first transfer comes from an unaccepted banking service, Swan may ask the user to send a transfer from a different account in their name. The account holder verification status changes to `WaitingForInformation` until the requested transfer is made.  

- **Additional documents requested:** Swan may ask the user for [supporting documents](../../accounts/documents/index.mdx) after receiving the first transfer. The account holder verification status changes to `WaitingForInformation`.

:::info Additional transfer or documents
After the additonal transfer or documents are received, the account holder verification status changes from `WaitingForInformation` to `Pending` while Swan reviews them. Depending on the outcome, the status changes to either `Verified` or `Refused`. 
:::
