---
title: Using the API
audience:
  - dev
counterpart: /accounts/guides/ibans/get-info/from-the-dashboard
related:
  - /accounts/guides/ibans/get-info
  - /accounts/guides/ibans/get-info/from-the-dashboard
  - /accounts/concepts/ibans
---

# Get information about an account's IBANs using the API

Get information about an account's <Term id="ibans">IBANs</Term> with the API.

## Guide {#guide}

1. Call the `account` query.
1. Enter your account ID.
1. Add the information you need. This sample includes information relevant to IBANs, both main and virtual.
1. *(Optional)* Narrow the list of virtual IBANs with the `filters` argument on `virtualIbanEntries`: filter by `status` (`Enabled`, `Suspended`, or `Canceled`) or by entry `ids`.
1. Add [pagination](/build/using-api/pagination) if you're expecting a long payload (line 6).

## Query {#query}

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {2,6} showLineNumbers
query GetAccountIbanInfo {
  account(accountId: "$YOUR_ACCOUNT_ID") {
    IBAN
    paymentAccountType
    paymentLevel
    virtualIbanEntries(first: 2) {
      edges {
        node {
          BIC
          IBAN
          id
          label
          status
        }
      }
      pageInfo {
        hasNextPage
        hasPreviousPage
      }
      totalCount
    }
  }
}
```

## Payload {#payload}

There's a lot to learn from this payload response.

1. The account has a main `IBAN` (line 4), which means the `paymentAccountType` must be `PaymentService` (line 5). Main IBANs are issued when the account type changes from `EMoney` to `PaymentService`.
1. The `paymentLevel` is `Unlimited`, meaning virtual IBANs can be added to the account.
1. There are nine total virtual IBANs (line 32), and only two are listed in this response because of <Term id="pagination">pagination</Term>.

```json {4-6,32} showLineNumbers
{
  "data": {
    "account": {
      "IBAN": "$ACCOUNT_MAIN_IBAN",
      "paymentAccountType": "PaymentService",
      "paymentLevel": "Unlimited",
      "virtualIbanEntries": {
        "edges": [
          {
            "node": {
              "BIC": "SWNBFR22",
              "IBAN": "$VIRTUAL_IBAN_1",
              "id": "$VIRTUAL_IBAN_1_ID",
              "label": "Virtual",
              "status": "Enabled"
            }
          },
          {
            "node": {
              "BIC": "SWNBFR22",
              "IBAN": "$VIRTUAL_IBAN_2",
              "id": "$VIRTUAL_IBAN_2_ID",
              "label": "Virtual",
              "status": "Canceled"
            }
          }
        ],
        "pageInfo": {
          "hasNextPage": true,
          "hasPreviousPage": false
        },
        "totalCount": 9
      }
    }
  }
}
```
