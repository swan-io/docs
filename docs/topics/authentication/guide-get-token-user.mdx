...
![Image of Dashboard API page focused on credentials](../../images/topics/authentication/token-user-credentials.png)
...
&scope=openid%20offline%20idverified
&state=kdqsjdlkjsqdlkqjsdlkjsqd

# Full URL
https://oauth.swan.io/oauth2/auth?response_type=code&client_id=$YOUR_CLIENT_ID&redirect_uri=$YOUR_REDIRECT_URI&scope=openid%20offline%20idverified&state=kdqsjdlkjsqdlkqjsdlkjsqd
```

| Parameter | Explanation | Required |
| --- | --- | --- |
| `response_type=code`<br/>(line 6) |  Initiates the authorization code flow. | ✓ **Yes** |
| `client_id`<br/>(line 7) |  Public identifier for the Swan app, obtained from your Dashboard in step 1. | ✓ **Yes** |
| `redirect_uri`<br/>(line 8) |  Specifies where the authorization server should send the user after approval, added to your Dashboard in step 1. | ✓ **Yes** |
| `scope=openid%20`<br/>`offline%20idverified`<br/>(line 9) |  Defines the requested scopes for the user authorization.<br/><br/><ul><li>`openid`: User will connect to Swan through the Partner</li><li>`offline`: Access is continuous</li><li>`idverified`: If not already completed, user needs to verify their identity</li></ul> | ✓ **Yes** |
| `state`<br/>(line 10) |  A value to be retransmitted in the query string when redirecting back to you. | ✓ **Yes** |
| `phoneNumber` |  Avoids asking the customer to enter their phone number. | ☒ No |
| `firstName` |  Avoids asking the customer to enter their first name during user registration. | ☒ No |
| `lastName` |  Avoids asking the customer to enter their last name during user registration. | ☒ No |
| `birthDate` |  Avoids asking the customer to enter their birth date during user registration (format: YYYY-MM-DD). | ☒ No |
| `identificationLevel` |  Associated with the scope `idVerified` to specify the identification flow. | ☒ No |
| `onboardingId` |  Avoids asking the customer to enter their own address and email if they did it during onboarding. | ☒ No |
...
Read about why in the overview of the [integrate Strong Customer Authentication](../consent/guide-integrate-sca.mdx#overview) guide.
:::
