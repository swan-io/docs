...
1. Call the `initiateCheckMerchantPayment`, completing all mandatory fields: `amount` `value` and `currency`, `CMC7`, `rmlcKey`, and `merchantProfileId` (lines 2-7).
...
1. Add the `InitiateCheckMerchantPaymentSuccessPayload` success payload (line 10).
1. Add the `checkMerchantPayment` to the success payload (line 12)
...
1. Add the rejection `CheckRejection` (line 24), then add the `fnciInfo` (lines 27-33).
