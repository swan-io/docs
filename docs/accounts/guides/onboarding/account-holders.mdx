...
Follow the guide to [add additional individual accounts](/accounts/guides/onboarding/account-holders#add-additional-account) for an existing account holder.

**Companies** can open [multiple accounts](/topics/accounts/multiple-accounts/) linked to a single account holder and registration number. No new verification is needed when creating additional accounts. 
...
Follow the guide to [add additional company accounts](/topics/accounts/multiple-accounts/guide-multiple-accounts-integration) for an existing account holder.
...
However, while account holder verification is in progress, there are [limitations on the account](/topics/accounts/#account-type-level).
...
1. **Onboarding**: Process is finalized for either an [**individual**](/accounts/guides/onboarding/individual) or a [**company**](/accounts/guides/onboarding/company), which creates a user, an account holder, an account, and an account membership linking the user to the account.
1. [**Identification**](/topics/users/identifications/): User that opened the [individual](/accounts/guides/onboarding/individual#identification-recommendations) or [company](/accounts/guides/onboarding/company#identification-recommendations) account completes identification with an ID document and a picture or video.
1. [**First transfer**](#first-transfer): Users may need to send a first transfer to their new Swan account.
1. [**Document collection**](/topics/accounts/documents/): Swan collects required documents.
...
| `WaitingForInformation` | Swan is waiting for information from the account holder, such as a [first transfer](#first-transfer), [supporting documents](/topics/accounts/documents/), or something else. |
...
[Call the API](/accounts/guides/onboarding/account-holders#get-status) to learn which requirement they're missing.
...
| `SupportingDocuments` <br /> `Required` | Provide the requested [supporting documents](/topics/accounts/documents/). |
...
import FirstTransfer from '../../../topics/partials/_first-transfer.mdx';
...
Additionally, if your users complete certain [identification processes](/topics/users/identifications/#processes-recommendations) in certain countries, a first transfer might be required by law.
Review the [guide to request a first transfer](/accounts/guides/onboarding/account-holders#request-first-transfer) for communication options.

:::caution 🇩🇪 Germany
A first transfer is always required for self-employed and individual accounts for which the **[account country](/topics/accounts/#account-country)** is **Germany**, regardless of registration or residency.
:::
...
While there is technically no deadline for sending the transfer, the user's [account is `Limited`](/topics/accounts/#account-type-level) until the transfer is validated.
...
| Additional documents requested | We received the first transfer and asked the user for [supporting documents](/topics/accounts/documents/). | `WaitingForInformation` |
...
Learn more about [requesting a first transfer](/accounts/guides/onboarding/account-holders#request-first-transfer) and [account limitations and capabilities](/topics/accounts/#account-type-level).
...
## Verification renewal (Re-KYC) {#verification-renewal}

As required by financial regulations, Swan periodically verifies account holder information.
...
The legal representative (account admin):

1. Receives the [initial notification email](/accounts/guides/onboarding/account-holders#verification-renewal-timeline) with a unique portal link.
...
1. Uploads [supporting documents](/topics/accounts/documents/#required-documents) if required.
...
1. First, [retrieve the required `accountHolderId`](/accounts/guides/onboarding/account-holders#simulate-status-update-get-id).
...
- [Account onboarding](/accounts/guides/onboarding) — the onboarding hub.
- [Individual onboarding](/accounts/guides/onboarding/individual) · [Company onboarding](/accounts/guides/onboarding/company)
- [Country requirements](/accounts/reference/country-requirements) · [Proof of address](/accounts/reference/proof-of-address) — documents these processes consume.
