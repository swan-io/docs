...
👥 Starting on **March 20th**, account memberships are only returned in the `accountMembership`, `accountMemberships`, and `account.Memberships` queries if you have the `canManageAccountMembership` [permission](/topics/accounts/memberships/#permissions).
