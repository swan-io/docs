---
title: Declare a payment mandate
audience:
  - dev
related:
  - /payments/concepts/merchants
---

# Declare a payment mandate for SEPA Direct Debit

Before accepting <Term id="payments">payments</Term> with <Term id="sdd">SEPA Direct Debit</Term> B2B or Core, you need to declare a <Term id="payment-mandates">payment mandate</Term> to your Swan account.
Each debtor must have their own payment mandate.

[Review the requirements](/payments/concepts/merchants/mandates#sdd) for SEPA <Term id="direct-debit">Direct Debit</Term> payment mandates if needed.

:::tip Prerequisites
- Debtor's account status: any except `Closing` or `Closed`
- Merchant profile status: `Enabled`
- SEPA Direct Debit B2B payment method status: `Enabled`
- [B2B only] Debtor's account type: `Company`
- Mandate includes a unique SEPA Creditor Identifier (SCI)
- Mandate includes a Unique Mandate Reference (UMR)
- A project access token, or a user access token with the `canManageAccountMembership` membership permission
:::

## Guide {#guide}

1. Call the `addSepaDirectDebitPaymentMandate` mutation. You don't need <Term id="consent">consent</Term> from the debtor.
1. Replace `$YOUR_PAYMENT_METHOD_ID` with the ID for your payment method, from your Dashboard (line 4).
1. Choose `Recurrent` or `OneOff` (line 5).
    - [`OneOff` payment mandates](/payments/concepts/merchants/mandates#sdd-oneoff) can only be used once.
1. Choose the language and signature date for the mandate (lines 6-7).
1. Add information about the debtor, including `name`, `IBAN`, and their `country` of residence (lines 9-11).
1. Choose either Core or B2B in your [mandate name](/payments/concepts/merchants/mandates#sdd-info) (line 13).
1. Add validations and rejections that are helpful for you, as well as optional information such as a `reference` for your internal use.

Swan creates the mandate with the status `Enabled`. It's finalized the first time it's used.

:::info B2B
For B2B, **declare the mandate to the debtor's bank** as well.
:::

## Mutation {#mutation}

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {4-7,9-11,13} showLineNumbers
mutation AddSddMandate {
  addSepaDirectDebitPaymentMandate(
    input: {
      paymentMethodId: "$YOUR_PAYMENT_METHOD_ID"
      sequence: Recurrent
      language: it
      signatureDate: "2023-10-10"
      debtor: {
        name: "Francesca Verrilli"
        IBAN: "IT75M0300203280692352275489"
        address: { country: "ITA" }
      }
      name: "B2B/Core SEPA Direct Debit Mandate"
    }
  ) {
    ... on AddSepaDirectDebitPaymentMandateSuccessPayload {
      __typename
      paymentMandate {
        id
        statusInfo {
          status
        }
      }
    }
    ... on InternalErrorRejection {
      __typename
      message
    }
    ... on PaymentMethodNotCompatibleRejection {
      __typename
      message
    }
    ... on ValidationRejection {
      __typename
      message
      fields {
        code
        message
        path
      }
    }
    ... on PaymentMandateReferenceAlreadyUsedRejection {
      __typename
      message
    }
    ... on SchemeWrongRejection {
      __typename
      message
    }
    ... on DebtorAccountNotAllowedRejection {
      __typename
      message
    }
    ... on DebtorAccountClosedRejection {
      __typename
      message
    }
    ... on NotFoundRejection {
      id
      message
    }
    ... on ForbiddenRejection {
      __typename
      message
    }
  }
}

```

import Environments from '../../../../../_shared/partials/_environments.mdx';

<Environments />

## Payload {#payload}

Note the `id` (line 6) for your SEPA Direct Debit payment mandate.

```json {6} showLineNumbers
{
  "data": {
    "addSepaDirectDebitPaymentMandate": {
      "__typename": "AddSepaDirectDebitPaymentMandateSuccessPayload",
      "paymentMandate": {
        "id": "$YOUR_MANDATE_ID",
        "statusInfo": {
          "status": "Enabled"
        }
      }
    }
  }
}
```
