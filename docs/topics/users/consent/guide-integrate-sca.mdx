...
For security reasons, all links must open in in-app browsers. Therefore, **never use WebViews** to open URLs (`consentUrl` or `oAuthUrl`).
:::

## Overview {#overview}

To integrate our SCA, you'll need:
...
| System  | API                                                                                                                                                                                                                                                |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Android | [Custom Tabs API](https://developer.android.com/jetpack/androidx/releases/browser) ([🔗 Implementation reference](https://github.com/swan-io/react-native-browser/blob/main/android/src/main/java/io/swan/rnbrowser/RNSwanBrowserModuleImpl.java)) |
| iOS     | [SFSafariViewController API](https://developer.apple.com/documentation/safariservices/sfsafariviewcontroller) ([🔗 Implementation reference](https://github.com/swan-io/react-native-browser/blob/main/ios/RNSwanBrowser.mm))                      |

There are also several wrappers available for these APIs:

- [@swan-io/react-native-browser](https://github.com/swan-io/react-native-browser) - our **official package**
...
:::warning Follow the implementation references
When using frameworks other than React Native, ensure that your package is configured in a way that follows our implementation references.
:::
...
Swan's react-native-browser takes care of listening to the deep link event. Make sure not to care about the `listener` declaration in steps 3.1, 3.5, and 4.3 and use the `onClose` option instead:
