...
Transaction confirmation statements are **only** available for incoming and outgoing **SEPA Credit Transfers** (Regular and Instant) with the status **`Booked`**.

Transaction confirmation statements are **not available** for card transactions, SEPA Direct Debit transactions (SDD In or SDD Out), fee transactions, internal transfers, or any other transaction types.
