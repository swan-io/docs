...
1. With the invited account member's [user access token](/dev-tools/using-api/authentication#tokens-user), call the `BindAccountMembership` mutation.
...
If the API returns a [user binding error](/accounts/concepts/memberships/statuses#binding-errors), please [**follow the guide**](/accounts/guides/memberships/fix-binding-error) to fix it.
:::
