...
| `scope=openid%20`<br/>`offline%20idverified`<br/>(line 9) |  Defines the requested scopes for the user authorization.<br/><br/><ul><li>`openid`: User will connect to Swan through the Partner</li><li>`offline`: Access is continuous</li><li>`idverified`: If not already completed, user needs to verify their identity</li></ul> |
...
| Parameter | Description |
| --- | --- |
| `phoneNumber` |  Avoids asking the customer to enter their phone number. |
| `firstName` |  Avoids asking the customer to enter their first name during user registration. |
| `lastName` |  Avoids asking the customer to enter their last name during user registration. |
| `birthDate` |  Avoids asking the customer to enter their birth date during user registration (format: YYYY-MM-DD). |
| `identificationLevel` |  Associated with the scope `idVerified` to specify the identification flow. |
| `onboardingId` |  Avoids asking the customer to enter their own address and email if they did it during onboarding. |
