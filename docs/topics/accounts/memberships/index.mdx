...
Refer to the dedicated [account administrator change](./admin-change.mdx) page for the full process, eligibility rules, required documents, and API reference.
...
This type of invitation doesn't require consent from the account holder and skips the status `ConsentPending`.
...
| `taxIdentificationNumber` | <Opt /> | <Opt /> | <Opt /> | <Opt /> | <Opt /> | <Cnd /> |
...
The `taxIdentificationNumber` field is **required** if both `accountCountry` **and** `residencyAddress.country` are 🇮🇹 Italy, and the account membership has the `canInitiatePayments` permission set to `true`.
...
These validation rules are the same as those applied when [creating users](../../users/index.mdx).
:::
