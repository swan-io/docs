...
TO DO: Include tabs widget that shows all matches, their meaning and their UI render 
...
:::tip Token properties
- **Single-use**: Each token works only once
- **Time-limited**: Check `expiresAt` timestamp
- **Optional**: Token improves compliance but isn't required for transfers. It may be required for certain upcoming payment scenarios.
:::
...
Test with these mock beneficiary names:

| Name | Result |
| --- | --- |
| `NO_MATCH` | Returns `SepaBeneficiaryNoMatch` |
| `BIC_NOT_FOUND` | Returns `SepaBeneficiaryVerificationNotPossible` |
| Any other name | Returns `SepaBeneficiaryMatch` |
