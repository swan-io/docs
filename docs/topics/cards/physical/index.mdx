...
|---|---|
| `ToRenew` | Triggered **10 weeks before the last day of the expiration month**, at midnight UTC. This status schedules the renewal. You can stop the process by [canceling](./guide-cancel.mdx) the card, which permanently ends the renewal, or by [suspending](#renew-suspended) it, which temporarily pauses the process. The card won't be renewed if it remains `Suspended` past 8-weeks. |
| `Renewed` | Triggered automatically **8 weeks before the last day of the expiration month**, at midnight UTC. At this point, the new card is created and queued for printing. No updates to the delivery address can be made after this status is triggered. |

:::info Reference date for renewal checkpoints
Swan counts the 10-week and 8-week checkpoints from the **last day of the card's expiry month**, not from the date printed on the card. 
...
1. T-10W checkpoint (22 August 2026, midnight UTC)
...
2. T-8W checkpoint (5 September 2026, midnight UTC)
