...
Aggregate spending limits are rolling limits that track the total amount spent within a defined period - daily, monthly or weekly.
Each new payment is aggregated against the total spending during the defined time period.
Swan computes aggregate spending limits at the **hour level** for cards with daily, weekly, and monthly periods.
Hour-level aggregation provides more precise control over rolling period calculations, ensuring that spending limit is computed consistently. 
