...
Swan also sends your users a text message with a link they can use to start their identification.
The link can be used one time.
If your users need a new link, they can request a new text message.
...
flowchart LR
    IS((Identification<br/>requested)) -.-> NS[NotStarted]
    NS -.-> S[Started]
    NS -.-> E[Expired]
    NS -.-> C[Canceled]
    S -.-> P[Pending]
...
|---|---|
| `NotStarted` | Identification created but user hasn't started it yet. |
| `Started` | When you begin an identification with our provider but don't finish it. |
| `Pending` | When you've completed an identification with our provider and we're waiting for their response. |
| `Valid` | When you've finished the identification and the provider has confirmed it was successful. |
| `Invalid` | When you've completed the identification and the provider has indicated it was not successful. |
| `Canceled` | When you start an identification with our provider but then explicitly request to cancel it, or our system cancels it for other reasons. |
| `Expired` | When you begin an identification flow but don't complete it within the time range (15 minutes for PVID, 24 hours for QES and Expert). |
| `NotSupported` | When you perform an identification that doesn't include a certain level, that level will be marked as "not supported".<br /><br />For example, in an Expert identification, the PVID status will be `NotSupported`. |
| `NotSupported` | In the API, all levels (`Expert`, `QES`, and `PVID`) are all displayed. For each identification, any level that doesn't meet your requirements is marked `NotSupported`.<br /><br />*For example, in an `Expert` identification, the `PVID` status is `NotSupported`.* |

### Rejection reason codes {#tracking-reason-codes}

Learn why your user's **identification** was rejected.
You can also refer users to the Swan's end-user [Support Center](https://support.swan.io/hc/en-150/articles/16421416513693-Identity-verification-rejection-reasons-and-solutions).
