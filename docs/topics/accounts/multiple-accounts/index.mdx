...
### Legal representative membership

An account membership gives a user access to a specific account. 
When an account holder creates additional accounts, their legal representative of their account is automatically added as a member of each new account. 
This ensures the legal representative always has access to manage all the accounts.

### Other account memberships

Account members who aren't legal representatives need a separate invitation for each account. They don't automatically get access to all accounts, even if they work for the account holder.

Memberships are tied to each account, not to the account holder. This means you can decide who has access to each account and keep them separate.

For example, an employee could have access to the operating account but wouldn't see the reserve account unless invited.
