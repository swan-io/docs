...
The `period` field shows `Custom` (line 15) for on-demand statements. Swan automatically generates statements with `period` values of `Monthly` or `Quarterly`.

The `statusInfo.status` shows `Pending` while Swan generates the statement (line 17).

```json {15,17} showLineNumbers
{
  "data": {
    "generateAccountStatement": {
      "closingDate": "2024-06-12T00:00:00.000Z",
