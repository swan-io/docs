...
<p className="ia-lede">The four-digit PIN of a <Term id="cards-physical">physical card</Term>: how it's chosen or assigned, when it becomes available, and what happens after incorrect attempts.</p>
...
To allow your cardholders to choose their PIN, set the `choosePin` parameter to `true` when [printing physical cards](/cards/guides/physical/print).
...
You can [print a physical card](/cards/guides/physical/print#print-with-virtual) for another account member with the `addCards` mutation, allowing the member cardholder to choose their PIN by setting the `choosePin` parameter to `true`.
