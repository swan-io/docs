...
1. If the account balance is **negative**, Swan asks the account holder to transfer funds to their account so the balance reaches zero. The account can also receive incoming SEPA Credit Transfers to help clear the debt.
...
1. The account **undergoes the closing process**. Refer to the [account statuses](../index.mdx#statuses) section for an overview of what happens while an account is `Closing`. The move from `Closing` to `Closed` depends on the account meeting [all of these conditions](./index.mdx#closing-to-closed-conditions), not on a fixed countdown.
