...
:::info Requirements
Bulk <Term id="credit-transfer">credit transfers</Term> require [custom verification](/payments/concepts/verification-of-payee#option-2) using the [`verifyBeneficiary`](/payments/guides/credit-transfers/sepa/verify-beneficiary) mutation. The default verification flow doesn't support bulk payments.
:::
...
| **AccountMembershipStatusNotEligibleRejection** | User's <Term id="account-membership">account membership</Term> must be `Enabled`. |
