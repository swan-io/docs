...
Consider subscribing to the `Account.Closing` and `Account.Closed` [webhooks](/dev-tools/using-api/webhooks#events-accounts) to be notified when the [account status](../index.mdx#statuses) changes.
:::
