...
import AccountCountryList from '../../topics/partials/_account-countries.mdx';
...
Opening accounts is subject to a [thorough verification](/accounts/guides/onboarding/account-holders#verifying-account-holders-multi-account), regardless of residence or account country.
:::
...
The level changes from `Limited` to `Unlimited` when the [account holder verification status](/accounts/guides/onboarding/account-holders#verification-process-statuses) changes to `Verified`.
The account type changes from `EMoney` to `PaymentService` when the [account's main IBAN](/accounts/concepts/ibans#main) is available to the account holder.
...
The only exception is the automatic [billing of banking fees](/accounts/concepts/billing#fees-swan-standard).
