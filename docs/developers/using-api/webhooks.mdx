...
Consider the [basic API flow](../index.mdx#flow) for reference.
...
Webhooks operate with a near-real-time efficiency, typically responding **within seconds** of the event.
Sometimes, webhooks can take **up to 10 minutes** to respond.
...
If we encounter an error during a webhook request, we'll retry it up to **8 times**.
Additionally, outgoing webhook requests have a **timeout of 15 seconds**, after which the webhook event fails and an automatic retry is triggered.

If all retrys fail, you can retry it manually by selecting **Retry** from your **Dashboard** > **Developers** > **Webhooks**.
...
Taking transactions as an example, the `at-least once` delivery means that for the same `transactionID`, you might receive more than one notification.
