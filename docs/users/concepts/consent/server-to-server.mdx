...
<Term id="sca">Strong Customer Authentication</Term> normally requires human interaction, but Swan makes an exception for your own accounts: you can approve <Term id="sensitive-operation">sensitive operations</Term> with S2S consent.
...
The [list of sensitive operations](/users/reference/sensitive-operations) indicates which operations are eligible for server-to-server consent in its **Server-to-server** column.

Learn more in the [guide to implement server-to-server consent](/users/guides/consent/implement-s2s).

## Key cryptography {#s2s-crypto}

Server-to-server consent relies on a pair of keys, public and private.
Swan only accepts keys that meet the following requirements:
...
import ReplaceKeys from '../../partials/_s2s-replace-key.mdx';

<ReplaceKeys />

## Role of projects and legal representatives {#s2s-projects}

For security and regulatory purposes, server-to-server consent is bound to a project, more specifically to the project's <Term id="legal-representative">legal representative</Term>.
...
participant S as Swan API
    ...
    P ->> PS: Requests signature of consent challenge
    PS ->> P: Returns consent challenge signed with private key
