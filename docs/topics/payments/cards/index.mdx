...
When merchants correctly reference the original debit authorization, both `CardOutCredit` and `CardOutDebitReversal` transactions are linked to the same `paymentId` as the original authorization for improved transaction tracking and reconciliation.
