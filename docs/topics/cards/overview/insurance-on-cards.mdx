...
|---|---|---|---|---|---|
| Loss or theft of identity documents | Reimbursement of costs for renewing identity documents (including duplicate fees, stamp duty, and identity document photos). |  ✅  | ✔️ | ✔️ | Up to €350 per year and per claim |
...
sequenceDiagram
    ...
    alt User orders card via Web Banking
        ...
        Your system->>Swan: Calls `addCard`, `addCards`, or `addCardsWithGroupDelivery` mutation
    end

    Swan-->>Your system: Returns card object with `insurancePackageId`
    Swan->>User: Displays consent screen (localized insurance terms)
    User->>Swan: Accepts insurance terms
    Swan->>Your system: Sends `cardInsuranceSubscription.Created` webhook
    Note over Swan: Waits for:<br>Account membership = Enabled<br>Account holder verification = Verified
    Swan->>Insurance partner: Sends POST request to create insurance subscription
    Insurance partner-->>Swan: Confirms subscription and activates insurance
    Swan->>Your system: Sends `cardInsuranceSubscription.Updated` webhook
```
### Steps
