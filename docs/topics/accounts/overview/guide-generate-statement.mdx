...
Notice the `period` = `Custom` (line 15), indicating that you generated this account statement, not Swan.

The `statusInfo` > `status` is `Pending` while the statement is being generated (line 17).
