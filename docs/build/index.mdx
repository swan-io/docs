...
Swan's GraphQL API, the tools to test your integration, and the conventions for calling the API.

## In this section {#in-this-section}

- **Tools**: [API Explorer](/build/tools/api-explorer) · [Testing the API](/build/tools/testing-api) · [Event simulator](/build/tools/event-simulator) · [Sandbox users](/build/tools/sandbox-users) · [Postman collection](/build/tools/postman-collection) · [Test tools guide](/build/tools/test-tools-guide)
- **Using the API**: [Authentication](/build/using-api/authentication) · [Errors and rejections](/build/using-api/errors-rejections) · [Idempotency](/build/using-api/idempotency) · [Pagination](/build/using-api/pagination) · [Payment control](/build/using-api/payment-control) · [Webhooks](/build/using-api/webhooks)
- **GraphQL**: [About GraphQL](/build/graphql)

## Overview {#overview}

Swan uses a **GraphQL API**, explained in more detail on [About GraphQL](/build/graphql).
...
Note that the flow changes when [webhooks](/build/using-api/webhooks) are involved.
...
[Projects are also environment-dependent](/get-started/set-up-swan/create-project#environments).

In short, **test in Sandbox, build in Live**, and remember that you can't move anything between the two.
...
If you need to increase the rate limits for your use case, please use our Partner Support Request form available on your Dashboard.
...
Replace `$YOUR_ACCESS_TOKEN` with a valid [user or project access token](/build/using-api/authentication) before sending your request.
...
1. Otherwise, please use our Partner Support Request form available on your Dashboard.