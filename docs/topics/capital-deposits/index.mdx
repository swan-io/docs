...
| `Refused`   | Document doesn't meet requirements. Review `reasonCode` for a refusal category and the optional `reason` for a plain-text explanation. Both are visible on your Dashboard and available via the API.<br/><br/>**Next step**: Upload a new document, then the status returns to `Uploaded`. |
