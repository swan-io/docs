...
International transfers _can_ introduce more significant risk than transfers that don't cross borders or change currencies.
...
∗ _Currencies marked with a single asterisk can't be sent from a Swan company account._
...
Due to these dynamic forms, it's critical to **run the queries in order** _before_ running the mutation.
