...
To complete the unblocking process, the cardholder must insert their card at an ATM, and enter their original PIN. They don't need to withdraw money.

| Cardholder permission | PIN Entered | Transaction Outcome | Rejection Reason | Card Status Change |
| --- | --- | --- | --- | --- |
| **Withdrawal permissions** | Correct | Accepted, proceeds normally | None | Card is completely unblocked |
| | Incorrect | Rejected | `InvalidPin` | Card remains suspended |
| **No withdrawal permissions** | Correct | Rejected | `TransactionNotAuthorizedForCardholder` | Card is completely unblocked |
| | Incorrect | Rejected | `InvalidPin` | Two more attempts before the card is suspened again |
