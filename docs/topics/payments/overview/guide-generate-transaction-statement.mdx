...
Transaction confirmation statements are **not available** for card transactions, SEPA Direct Debit transactions (SDD In or SDD Out), fee transactions, SEPA Credit Transfers, or any other transaction types not explicitly listed above.
