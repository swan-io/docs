...
| `Canceled` | The case was canceled before completion by either you or Swan. When a case is canceled, the shareholder status becomes [`CapitalDepositWithdrawn`](#shareholders-statuses). |
...
| `Refused`   | Document doesn't meet requirements. Review the `reasonCode` for a machine-readable classification and the optional `reason` for a human-readable explanation of the refusal. Both are available via the API and visible on your Dashboard.<br/><br/>**Next step**: Upload a new document, then the status returns to `Uploaded`. |
...
| `CapitalDepositWithdrawn` | When a [`CapitalDepositCase` is `Canceled`](#case-statuses), the shareholder status changes to `CapitalDepositWithdrawn`.<br/><br/>If an account was already created for the shareholder, the account is also closed automatically. |
...
When a capital deposit case is canceled, any associated shareholder accounts are closed with the reason code `CapitalDepositWithdrawn`.
