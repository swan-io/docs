...
:::tip Prerequisites
The [collection must have the status](./about.mdx#collection-statuses) `Waiting for document`, and there must be at least one document in the collection with the status `Uploaded`.
:::

## API guide

1. First, [retrieve the collection ID](./guide-retrieve-info.mdx) (`supportingDocumentCollectionId`).
1. In the API Explorer, add the `requestSupportingDocumentCollectionReview` mutation.
