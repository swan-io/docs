...
| `Refused` | Swan can refuse a `CapitalDepositCase` if requirements aren't sufficient.  |
| `Canceled` | You can request to cancel a `CapitalDepositCase` any time before completion by sending an email to support@swan.io. When a case is canceled, the shareholder's account is closed automatically. |
