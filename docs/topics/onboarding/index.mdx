...
During onboarding, users also [complete an account holder verification](./account-holders/index.mdx#verification-process) process and [authenticate](/dev-tools/using-api/authentication) (log in) for the first time.
...
<table>
  ...
  <tr>
    ...
    <td>Swan requests [supporting documents](../accounts/documents/index.mdx)<br /><br />💡 *[Subscribe to the webhook](/dev-tools/using-api/webhooks) `supportingDocumentCollection.updated`. You'll be notified when a request is sent to collect documents.*</td>
