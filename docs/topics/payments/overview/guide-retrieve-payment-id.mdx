---
title: Retrieve a payment ID
---

# Retrieve a payment ID

There are two methods you can use to retrieve a payment ID.

1. Locate payment IDs on your **Dashboard**.
1. **Run an API query** to get the ID.

:::caution 
This documentation typically explains using the API first.
However, it's easier to find a precise payment ID on the Dashboard.
:::
...
![Screenshot of Swan Dashboard transactions page with a callout of a single transaction](../../../images/topics/payments/retrieve-payment-id-1.png)

3. **Scroll** to the bottom of the page.
1. Located the information named **Payment**.
1. Click to copy the payment ID.

![Screenshot of Swan Dashboard transactions page with a callout of the transaction ID](../../../images/topics/payments/retrieve-payment-id-2.png)

## API guide

Queries are highly customizable.
This guide focuses solely on using the `payment` query to retrieve payment IDs. 
Learn more about this query in the guide to [get payment information](./guide-get-payment-info.mdx).

1. Add the `payments` query.
...
1. Under `node`, check the boxes for `id` and `createdAt`.
    ...
    * You might choose to retrieve only the `first` 10 IDs on the list, use [pagination](https://docs.swan.io/api/pagination) to filter further, or add a `totalCount`.
1. **▶ Run** your query.

### Query

🔎 [Open the query in API Explorer](https://explorer.swan.io?query=cXVlcnkgUmV0cmlldmVQYXltZW50SWQgewogIHBheW1lbnRzKGZpcnN0OiAxMCkgewogICAgZWRnZXMgewogICAgICBub2RlIHsKICAgICAgICBpZAogICAgICAgIGNyZWF0ZWRBdAogICAgICB9CiAgICB9CiAgICB0b3RhbENvdW50CiAgfQp9Cg%3D%3D&tab=api)

```graphql {2,5-6,9} showLineNumbers
query RetrievePaymentId {
  payments(first: 5) {
    edges {
      node {
        id
