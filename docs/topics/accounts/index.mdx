...
- A `Limited` account can't receive a total of more than €150 in SEPA Credit Transfers over its lifetime.

The €150 limit applies to the cumulative sum received, not the current account balance. 
Swan fees don't count toward this limit. For example, if a user receives €150 and a €2 fee is applied, the account balance becomes €148. 
Since the account has received €150, it has reached its lifetime limit. Swan will automatically reject any more transfers sent to the account.
