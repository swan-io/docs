...
All [sensitive operations](#sensitive) are eligible for server-to-server consent, except for three operations that allow you to view highly sensitive information: `viewCardNumbers`, `viewPhysicalCardNumbers`, and `viewPhysicalCardPin`.
...
| `Canceled` | Consent request can be `Canceled` by whoever initiated the sensitive operation, whether you or your user. `Canceled` also applies to child consents when a preceding child consent `Failed`. |
