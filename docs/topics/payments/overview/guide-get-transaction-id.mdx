...
1. Under `node`, check the boxes for `id`, `createdAt`, and `label`.
    ...
    * Additionally, you might choose to get only the `first` 10 IDs on the list, use [pagination](/dev-tools/using-api/pagination) to filter further, or add a `totalCount`.
