...
The [`CapitalDepositCase`](https://api-reference.swan.io/#definition-CapitalDepositCase) API object compiles all information for a capital deposit case, including:
...
| `WaitingForShareDepositCertificate` | ***Notary*** *reviews the deposited funds and submitted documents, then creates the certificate and uploads it to Swan* |
| `WaitingForRegisterExtract` | **You** upload your register extract for notary review |
| `WaitingForNotaryTransfer` | ***Notary*** *reviews the register extract* |
...
<table>
      ...
      <tr>
         ...
         <td><code>ProofOfIndividualAddress</code><br/><em>See <a href="#proof-of-address">proof of address</a> section</em></td>
      </tr>
      <tr>
         <td>Identity document proving the shareholder's identity (collected during identity verification)</td>
         <td><code>ProofOfIdentity</code></td>
      </tr>
      <tr>
         <td rowspan="4">Company shareholders</td>
         <td>Document available only after the notary validates the case, uploaded during at a specific time during the capital deposit process</td>
         <td><code>RegisterExtract</code></td>
      </tr>
      <tr>
         <td>Draft of the articles of incorporation</td>
         <td><code>ArticlesOfIncorporation</code></td>
      </tr>
      <tr>
         <td>Company's most recent tax statement <em>(if requested)</em></td>
         <td><code>CorporateIncomeTaxReturn</code></td>
      </tr>
      <tr>
         <td>Identity document proving the shareholder's identity (collected during identity verification)</td>
         <td><code>ProofOfIdentity</code></td>
      ...
      <tr>
         ...
         <td><code>CompanyLeaseAgreement</code></td>
      </tr>
      <tr>
         <td>Lease Mandate from legal representative</td>
         <td><code>PowerOfAttorney</code><br/><em>Template available in French</em></td>
      </tr>
   </table>
...
This also means that each shareholder must go through the [onboarding](../onboarding/about.mdx) and [verification](../onboarding/account-holders/about.mdx#verification-process) processes.
...
| `WaitingForVerification` | Possible status if the account holder verification isn't complete at the moment the shareholder's account is created<br/><br/>*Status bypassed if the [account holder status](../onboarding/account-holders/about.mdx#account-holder-verification-statuses) is already `Verified`* |
