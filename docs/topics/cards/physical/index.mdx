...
If your cardholder reports not receiving their physical card, submit a request to [Swan Support](https://support.swan.io/hc/en-150).
...
For details on delivery types and destinations, read more about the [🇫🇷 France](/topics/cards/physical/#hubs-france) and [🇪🇸 Spain](/topics/cards/physical/#hubs-spain) hubs. 
...
You can stop a physical card from being renewed with the `cancelPhysicalCardRenewal` mutation, so it expires instead of renewing. See [Cancel a physical card renewal](./guide-renew.mdx#cancel-renewal).
...
|---|---|
| `ToRenew` | Triggered automatically **10 weeks before the last day of the card's expiry month**, at midnight Coordinated Universal Time (UTC). Also triggered when a `Suspended` card is resumed between the 10-week and 8-week checkpoints. This status schedules the renewal. You can stop the process by [canceling](./guide-cancel.mdx) the card, which permanently ends the renewal, or by [suspending](#renew-suspended) it, which temporarily pauses the process. The card won't be renewed if it remains `Suspended` past 8 weeks. |
...
For a suspended physical card to be renewed, it must be resumed before the **8-week checkpoint** (midnight UTC, calculated from the last day of the expiry month).
Resuming the card during the renewal window moves it into `ToRenew`, then to `Renewed` at the 8-week checkpoint.

| When the card is resumed | Outcome |
|---|---|
| Before the 8-week checkpoint | Card moves to `Renewed` at the checkpoint and is shipped. |
| After the 8-week checkpoint, or not at all | Card stays `Suspended` and is `Canceled` when it expires. |

:::warning Renewal deadline
There's no way to manually trigger the renewal after the 8-week checkpoint passes.
If the card isn't in `ToRenew` by then, it can't be renewed and is `Canceled` when it expires.
:::

If a card is already `Renewed` and is then `Suspended`, **both the existing and new card** are `Suspended`.
The new card is still shipped, but you must resume it before it can be `Activated`.
...
- If the card is `Suspended`, it stays `Suspended`. The card must be resumed before the 8-week checkpoint to be renewed.
...
:::tip
Subscribe to the `Card.Updated` [webhook](../../../developers/using-api/webhooks/#events-cards) to get notified when the card moves to `ToRenew`, giving you time to verify the delivery address with your cardholder.
:::
