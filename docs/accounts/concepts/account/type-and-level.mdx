...
An account's current payment level is part of its general information, on your **Dashboard** under **Data** > **Accounts**.
...
| `Limited` + `PaymentService` | The account's main IBAN is displayed.<br />Money can be added to the account.<br /><br />Account holder verification isn't complete.<br />Payments can't be made from the account.<br />Incoming SEPA Credit Transfers exceeding €50 are automatically rejected.<br />Maximum cumulative balance is €150.<br />Incoming International Credit Transfers are automatically rejected regardless of amount. |
| `Unlimited` + `PaymentService` | Account holder verification status is `Verified`.<br />Payments can be made from the account.<br />The account's main IBAN is displayed.<br />All account functions are possible. |
