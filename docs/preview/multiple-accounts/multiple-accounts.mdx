...
**Multiple accounts allow a single company to create additional Swan accounts** under the same legal entity. These accounts share the same registration number and [local IBAN](/topics/accounts/ibans/#local) as the first account created at onboarding.
...
Multiple accounts are only available for [**company account](/topics/onboarding/company/) types**, which include:

- Registered companies
- Self-employed workers
- Associations or non-profit organizations

:::caution Individual accounts
Individual accounts are eligible. To open an additional account under the same account holder, the user must undergo a new onboarding process, but the KYC verification does not need to be repeated.
:::
...
Offering multiple accounts provides flexibility and operational benefits for different use cases.
...
- Share the same KYB, avoiding duplicate requests.
- Re-KYB checks are performed on a single account holder for all associated accounts.

## Feature activation

The multiple accounts feature is available to all partners, but it isn't **activated** by default. Before creating additional accounts, you must contact your **Strategic Account Manager (SAM)** to request it.

When making a request, make sure to include:

- The purpose of creating multiple accounts.
...
Follow the [**step-by-step guide**](https://www.notion.so/Guide-Open-multiple-accounts-265a3427555a80539c53d4acb4603592?pvs=21) to create multiple accounts.
:::
...
Users are technically restricted from creating new accounts after reaching the maximum limit.

:::info Account status 
Only accounts with an Opened status counts toward the maximum limit. Suspended, Closing, or Closed accounts don’t count.
:::

### Checking your account creation limit
...
Standard [account closure](/topics/accounts/closure/) rules apply. Each account can be closed individually. You can request closure of the first account (created during onboarding) while keeping any additional accounts active.
...
## Setting access in Web banking

The web banking setting `canOpenAccount` is a project-level configuration that controls whether your end users can open additional accounts directly from Swan Web banking.
...
1. **Differentiate by account limits:** Define the number of additional accounts a user can create for each pricing plan, for example, allowing one additional account for a Basic plan and up to five for a Premium one.
