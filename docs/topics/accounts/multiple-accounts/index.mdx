...
## Setting access in Web banking (coming in 2026) {#web-banking-access}

:::note Feature not yet available
The ability to create multiple accounts through Swan Web banking is not yet available. End users can only view their multiple accounts once created via API. Additionally, the Dashboard settings feature to control `canOpenAccount` is not yet available. Both functionalities will be released in a future update in 2026.
:::
