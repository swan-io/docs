...
The lifecycle of an <Term id="identification">identification</Term>, and the reasons one can be rejected.
...
flowchart LR
    ...
    style C fill:#f6ddcc,stroke:#c12236
```

| Identification status | Explanation |
|---|---|
| `Started` | The identification request was created. The user might have begun the identification process, but they haven't completed it. Their identification is still in progress.<br /><br />**Next steps**:<ul><li>When the user completes their portion of the process, the status moves to `Pending`.</li><li>If the time limit passes before they finish, the status moves to `Expired`.</li><li>If the identification is canceled, the status moves to `Canceled`.</li></ul> |
| `Pending` | The user completed their portion of the identification process, and Swan identification service providers are reviewing it.<br /><br />**Next steps**:<ul><li>If the identification is successful, the status moves to `Valid`.</li><li>If it isn't successful, the status moves to `Invalid`.</li><li>If the identification is canceled, the status moves to `Canceled`.</li></ul> |
...
| `NotSupported` | In the API, all [levels](/users/concepts/identifications/levels-processes) (`Expert`, `QES`, and `PVID`) are displayed. For each identification, any level that doesn't meet your requirements is marked `NotSupported`.<br /><br />*For example, in an `Expert` identification, the `PVID` status is `NotSupported`.* |

The `SwanIdentificationStatus` enum also includes `NotStarted`, which is deprecated. Use `Started` instead.
...
| `ExpiredDocument` | The expiry date of the document has passed. |
...
| `SanctionListHit` | The user appears on a sanctions list. |
...
| `UnacceptableDocument` | The document submitted by the user isn't on the list of acceptable document types. |
