...
| `Canceled` | An `Upcoming` transaction is canceled by someone with the right to do so, such as the [account holder](../../onboarding/account-holders/index.mdx) or an [account member](/accounts/concepts/memberships). Only debits with the status `Upcoming` can be `Canceled`, and `Canceled` debits don't impact the account balance. |
