...
| ↓ **Decrease** the authorized amount | Set `accepted` to `true`.<br />Include the lower the authorized amount in the `partialAuthorizationAmountValue`. The lower authorized amount must be greater than 0 and less than the original authorized amount. |
