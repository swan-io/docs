...
### Structured invoice reference  {#structured-invoice-reference}

Send a SEPA Credit Transfer with an optional structured or unstructured invoice reference using the `labelType` field.

:::info Structured reference
- `labelType`: optional; determines the label format - unstructured, structured without a specific standard, or structured using a specific invoice reference standard, such as `OgmVcs` (Belgian OGM VCS standard) or `Iso` (ISO 11649 standard).
- `label`: contains the reference value according to `labelType`. Can be structured or unstructured.
- `transaction.label`: returns the reference value when retrieving transaction data through a query.
:::
