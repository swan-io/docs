...
Accepting payments may introduce risk.

Swan prioritizes a **risk-based approach**, which applies to you and your merchants.
All requests for merchant profiles and merchant payment methods are reviewed by Swan.
Swan also reviews updates to all profiles.
You might be contacted for more information before Swan accepts or rejects the request.

To manage potential risk, Swan can change the status of a profile or a payment method if suspicious activity is detected, either temporarily or permanently.
...
The diagram and status explanations apply to merchant profiles **and** merchant payment methods.
...
| `Enabled` | Merchant profile or merchant payment method is active.<br /><br />Use `Enabled` merchant profiles to request payment methods and accept payments with activated payment methods. Use `Enabled` payment methods to accept payments. |
...
However, the statuses `Suspended`, `Disabled`, and `Rejected` can be assigned to unlimited versions of each payment method.
