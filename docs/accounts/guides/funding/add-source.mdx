...
:::tip Prerequisites
1. The user adding the funding source is either an account member with the `canManageAccountMembership` and `canViewAccount` [permissions](/accounts/reference/memberships/membership-permissions#permissions), or the account holder.
