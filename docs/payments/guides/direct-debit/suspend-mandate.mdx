---
title: Suspend a payment mandate
audience:
  - dev
related:
  - /payments/concepts/direct-debit
  - /payments/guides/direct-debit/enable-mandate
---

# Suspend a payment mandate

Suspend <Term id="sdd">SEPA Direct Debit</Term> received <Term id="payment-mandates">payment mandates</Term> with the API.
SEPA <Term id="direct-debit">Direct Debit</Term> instructions associated with a [`Suspended` received payment mandate](/payments/concepts/direct-debit/received-instructions#received-instructions-execution) are `Rejected`.

:::tip Prerequisites
1. You're an account member with the `canInitiatePayments` [membership permission](/accounts/reference/memberships/membership-permissions#permissions).
1. You only need a [project access token](/build/using-api/authentication#tokens-project) for this mutation.
:::

## Guide {#guide}

1. Call the `suspendReceivedDirectDebitMandate` mutation.
1. Add the ID for the mandate you're suspending.
1. Add the `SuspendReceivedDirectDebitMandateSuccessPayload`, including the `suspendedAt` if you'd like to review the exact time and date at which the mandate was suspended.
1. Add rejections (not shown).

## Mutation {#mutation}

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {3,10-11,13} showLineNumbers
mutation SuspendMandate {
  suspendReceivedDirectDebitMandate(
    input: { receivedDirectDebitMandateId: "$YOUR_MANDATE_ID" }
  ) {
    ... on SuspendReceivedDirectDebitMandateSuccessPayload {
      __typename
      receivedDirectDebitMandate {
        id
        statusInfo {
          status
          ... on ReceivedDirectDebitMandateStatusInfoSuspended {
            __typename
            suspendedAt
          }
        }
      }
    }
  }
}
```

import Environments from '../../../_shared/partials/_environments.mdx';

<Environments />

## Payload {#payload}

The payload shows the status `Suspended`, as well as the date and time at which it was suspended.

```json {8,10} showLineNumbers
{
  "data": {
    "suspendReceivedDirectDebitMandate": {
      "__typename": "SuspendReceivedDirectDebitMandateSuccessPayload",
      "receivedDirectDebitMandate": {
        "id": "$YOUR_MANDATE_ID",
        "statusInfo": {
          "status": "Suspended",
          "__typename": "ReceivedDirectDebitMandateStatusInfoSuspended",
          "suspendedAt": "2024-01-31T13:49:13.620765Z"
        }
      }
    }
  }
}
```
