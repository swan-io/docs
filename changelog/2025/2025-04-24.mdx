...
💳 You can now [set a preferred shipping provider](/cards/concepts/physical/delivery-and-shipping#shipping-methods) for physical cards, when calling the `addCards` mutation.
To do this, select `shippingProvider` in the `physicalCardCustomOptions` input.

📥 You can now [export account membership data](/accounts/guides/memberships/export) in `.csv` format from your Dashboard or with the API by calling the `exportAccountMembershipData` mutation.
