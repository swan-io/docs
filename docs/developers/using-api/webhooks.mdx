...
Your endpoint must respond with a `200 OK` HTTP status code within **10 seconds**. If it doesn't, the webhook delivery is marked as failed and will be [retried automatically](/developers/using-api/webhooks/#overview-retry).

To stay within this limit, avoid any synchronous activity. Save the event data and respond immediately to maximize your chances of a successful delivery.
...
Therefore, **Swan never uses webhooks to send sensitive information** that requires authentication.

To help verify that webhook requests actually come from Swan, and not someone who just knows your endpoint URL, you can use [**webhook secrets**](/developers/using-api/payment-control/#configure). Each request includes a shared secret in the `x-swan-secret` header. This secret acts like a password, known only to Swan and your system.

We strongly recommend checking the HTTP header on every webhook request. If you've set a webhook secret and it's missing or incorrect, reject the request to prevent unauthorized or fake webhook attempts.

If an event involves sensitive information, the webhook only notifies you that an event occurred.
You need to use your Dashboard or the API to review the full details.
