...
| `Disabled` | Account membership is disabled, is no longer available for use, and can't be restored.<br/><br/>When an account member's membership is disabled, their recurring `SingleUseVirtualCards` are [automatically reassigned to the account's Legal Representative](/topics/cards/virtual/#suv-recurring).<br/><br/>If you're subscribed to the `AccountMembership.Disabled` webhook, you'll receive a notification when a membership is `Disabled`. |
