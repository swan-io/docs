...
The following table lists the most common rejection codes for card transactions.
...
| `MerchantShouldResubmitAuthorization` | The merchant didn't use 3-D Secure (3DS). | The merchant needs to resubmit the authorization using 3DS. A common resolution for the cardholder is to remove the card from the merchant's website or app and re-add it, which triggers a new 3DS flow. |
...
| `TransactionTypeNotAllowed` | This transaction type isn't allowed on the card. | Verify [card settings](../../cards/index.mdx#settings). For example, the cardholder might have tried to withdraw money while withdrawals are disabled. |

:::note
This table lists the most common rejection codes. View the complete [`RejectedReasonCode` enum](https://api-reference.swan.io/enums/rejected-reason-code/) in the API Reference for all possible values.
:::
