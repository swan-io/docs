...
Additionally, any debtor using a non-Swan account to initiate a transfer to a Swan account has access to that Swan account's name and IBAN.
Therefore, external debtors can confirm that the beneficiary details are correct before sending the transfer.
