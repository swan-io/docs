...
- **Tools** — [API Explorer](/dev-tools/tools/api-explorer) · [Testing the API](/dev-tools/tools/testing-api) · [Event simulator](/dev-tools/tools/event-simulator) · [Sandbox users](/dev-tools/tools/sandbox-users) · [Postman collection](/dev-tools/tools/postman-collection) · [Test tools guide](/dev-tools/tools/test-tools-guide)
- **Using the API** — [Authentication](/dev-tools/using-api/authentication) · [Errors & rejections](/dev-tools/using-api/errors-rejections) · [Idempotency](/dev-tools/using-api/idempotency) · [Pagination](/dev-tools/using-api/pagination) · [Payment control](/dev-tools/using-api/payment-control) · [Webhooks](/dev-tools/using-api/webhooks)
- **GraphQL** — [About GraphQL](/dev-tools/graphql)

## Overview {#overview}

Swan uses a **GraphQL API**, explained in more detail on [About GraphQL](/dev-tools/graphql).
...
Note that the flow changes when [webhooks](/dev-tools/using-api/webhooks) are involved.
...
Replace `$YOUR_ACCESS_TOKEN` with a valid [user or project access token](/dev-tools/using-api/authentication) before sending your request.
