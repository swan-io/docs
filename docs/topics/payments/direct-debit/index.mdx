...
1. Swan adds the SEPA Direct Debit received payment mandate:
    ...
    - **B2B**: Adds the mandate with the status `ConsentInitiationPending`. After calling `enableReceivedDirectDebitMandate`, a consent is created and the status moves to `ConsentPending`. Once the debtor consents to the mandate, the status changes to `Enabled`.
