...
Account members can also add cards for themselves or other account members, determined by their [membership permissions](/accounts/reference/membership-permissions#permissions-cards).
...
Learn more about these features in the [card packages section](/topics/cards/card-packages).

### Card insurance

Swan offers [insurance coverage](/topics/cards/card-packages/#insurance) to protect cardholders.
...
| `cardContractExpiryDate` | Date you'd like the card to expire.<br /><br />Leave it empty if you don't want to set an expiration date, in which case, cards are renewed automatically every three years. Refer to [renewal and expiry](./virtual/index.mdx#renewal) for details. | API |
