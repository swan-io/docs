...
To [query this limit](./request.mdx), use the `paymentAmountLimit` field at the `merchantPaymentMethod` level in the API.
...
- [Accept payments with cards (step-by-step)](./accept.mdx)
- [Request card payment method](./request.mdx)
- [Create a card payment link](./create-link.mdx)