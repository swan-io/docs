...
| Parameter | Description |
| --- | --- |
| `phoneNumber`<br />`firstName`<br />`lastName`<br />`birthDate`<br />`birthCity`<br />`birthCountry`<br />`nationality`<br />`language` |  Avoids asking the customer to enter this information during user registration.<br /><br />*`birthDate` format: YYYY-MM-DD (year-month-day)* |
| `identificationLevel` |  Indicate your preferred [identification level](../../../topics/users/identifications/index.mdx#levels-processes).<br /><br />Swan **recommends choosing `auto`**, which allows Swan to direct your users the most appropriate identification flow for their situation. |
