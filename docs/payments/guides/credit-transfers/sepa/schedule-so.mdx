...
Schedule a <Term id="standing-order">Standing Order</Term> to send money on a recurring basis using the SEPA network.
...
1. Add all required information, including `period`, account ID, your redirect URL, your `beneficiary`, and whether or not the <Term id="ibans">IBAN</Term> belongs to you (lines 4-14).
...
1. Add the consent URL to the success payload (line 28): `statusInfo` > `StandingOrderConsentPendingStatusInfo` > `consent` > `consentUrl`.
