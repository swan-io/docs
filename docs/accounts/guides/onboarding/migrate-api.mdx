...
1. Create a company onboarding with `relatedIndividuals` (at least one <Term id="legal-representative">legal representative</Term> and one <Term id="ubo">UBO</Term>).
...
1. Confirm that [webhook payloads](/build/using-api/webhooks) for `Onboarding.Created` and `Onboarding.Updated` still match your expectations.
