...
After it's decoded, you can use the `sub` field to identify the user. In the `id_token`, `sub` represents the `userID`. In Sandbox, this is the [Sandbox `userID`](/developers/tools/sandbox-users/).
:::
