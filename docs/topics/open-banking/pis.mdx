...
| `RJCT` | Final | Single and bulk | Rejected | `Rejected` | All transactions are `Rejected`, or the consent failed (`CustomerRefused`, `CredentialRefused`, `Expired`, `Failed`, `Canceled`). |
...
- **Failed consent**: one of `ConsentCustomerRefused`, `ConsentCredentialRefused`, `ConsentExpired`, `ConsentFailed`, or `ConsentCanceled`.
