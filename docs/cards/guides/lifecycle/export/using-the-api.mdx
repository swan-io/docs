---
title: Using the API
audience:
  - dev
counterpart: /cards/guides/lifecycle/export/from-the-dashboard
related:
  - /cards/guides/lifecycle/export
  - /cards/guides/lifecycle/export/from-the-dashboard
---

# Export card data using the API

Export your card data as a CSV with the `exportCardData` mutation.

## Guide {#guide}

1. Call the `exportCardData` mutation.
1. Add your `email` (line 4). A link to download the `.csv` export is sent to the email address you provide.
1. Narrow the export with `filters` if needed (line 5).
1. Add the success payload, including the `exportId` (line 10), and rejections.

## Mutation {#api-mutation}

<a href="https://explorer.swan.io?query=&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {4,5,10} showLineNumbers
mutation ExportCards {
  exportCardData(
    input: {
      email: "$YOUR_EMAIL_ADDRESS"
      filters: { types: Virtual, statuses: Enabled }
    }
  ) {
    ... on ExportDataSuccessPayload {
      __typename
      exportId
    }
    ... on MaximumSimultaneousExportsRejection {
      __typename
      message
    }
    ... on MaximumDailyExportsReachedRejection {
      __typename
      message
    }
  }
}
```

## Payload {#api-payload}

The payload returns the export ID.

```json {5} showLineNumbers
{
  "data": {
    "exportCardData": {
      "__typename": "ExportDataSuccessPayload",
      "exportId": "$EXPORT_ID"
    }
  }
}
```

After finalizing your export, [a download link is sent to the email address you provided](/cards/guides/lifecycle/export#next).
