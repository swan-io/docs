...
Subscribe to the `DigitalCard.Created` and `DigitalCard.Updated` [webhooks](/developers/using-api/webhooks#events-cards) so you receive notifications when a digital card is created and whenever its status changes.
Swan sends both events, no matter how the card was added, including when a cardholder adds it to their wallet manually. This helps you keep your app in sync with the card's actual provisioning status.

- `DigitalCard.Created` is sent when the digital card is created, before it can be used in a wallet.
- `DigitalCard.Updated` is sent for every status change after that, such as `Enabled`, `Suspended`, or `Canceled`.

The event's `resourceId` is the digital card ID. Use it with the `digitalCard` query to [get the latest digital card information](./guide-get-info.mdx).
...
- [Get digital card information](./guide-get-info.mdx)
