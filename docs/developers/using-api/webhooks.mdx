...
You're aware of events you request or that your users request directly from you.
...
:::warning Webhook retention period
Webhook events are retained for 90 days. After 90 days, events are no longer available for you to retry manually or review in your **Dashboard** > **Developers** > **Webhooks**.
:::

### Subscription limits {#subscription-limits}

To avoid overconsumption caused by too many subscriptions to the same webhook event, there's a limit on how many subscriptions you can enable per event.
...
| `AccountMembership.Updated` | <ul><li>Account membership [permissions](/topics/accounts/memberships/#permissions) were updated.</li><li>Membership details changed: spending limits, language, email address, or tax ID.</li><li>Membership [status](/topics/accounts/memberships/#statuses) transitioned from `ConsentPending` to `InvitationSent`.</li><li>Membership [status](/topics/accounts/memberships/#statuses) updated to `BindingUserError`.</li></ul> |
...
| `DigitalCard.Created` | New digital card was created. |
| `DigitalCard.Updated` | Digital card [status](/topics/cards/digital/#statuses) (for example, `Enabled`, `Suspended`, or `Canceled`) changed during provisioning or wallet usage. |
