...
Swan will notify you with a webhook of type [`MerchantProfile.updated`](https://docs.swan.io/developers/using-api/webhooks/#events-merchants) with a similar JSON response as mentioned above once the status of the approval has been updated (wether the request for activation has been approved or rejected).
