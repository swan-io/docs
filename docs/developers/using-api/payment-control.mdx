...
1. Choose **accept** as your [**default response**](#configure) in case of timeout for a better user experience.
...
You can test it with [Swagger's editor tool](https://editor-next.swagger.io/).
...
:::note Example
For example, consider the **fluctuating nature of currency exchange** in your payment control configuration.

Rather than assuming a static exchange rate of 1 USD being equivalent to 1 EUR, assume 1 USD to be 1.1 EUR.
This might result in an account's spending limit being reached quicker, but it can help prevent over capture and therefore a negative balance.

Alternatively, you can advise account holders to maintain a buffer of 10% or more in their accounts, especially if they frequently make payments in currencies other than euros.
:::
...
If you use the API, you also need to provide the `protocol=HttpJson`.
...
In case of timeout, or if your endpoint wasn't reachable, Swan applies your [default response](#configure), which you set when configuring payment control.
...
| --- | --- |
| Accept the authorized amount | Set `accepted` to `true`. You don't need to include `partialAuthorizationAmountValue` in your response. |
| Decrease the authorized amount | Include the lower the authorized amount in the `partialAuthorizationAmountValue`. The lower authorized amount must be greater than 0 and less than the original authorized amount. |
| Reject a partial authorization | Set `accepted` to `false`. You don't need to include `partialAuthorizationAmountValue` in your response. |