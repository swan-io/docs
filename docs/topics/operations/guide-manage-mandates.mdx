...
You can only update a B2B received mandate through the API; it's not available in the Dashboard. If your development team has integrated the [update mandate](/topics/payments/direct-debit/guide-update-mandate) feature, your user can update the mandate from your product. The update requires a [user access token](/developers/using-api/authentication/#tokens-user) from an account member with the `canInitiatePayments` permission.
...
1. The statuses of the mandate, the account, and the debtor's IBAN are all `Enabled`.
...
If your user consents to a mandate after 6:00 AM CET/CEST on the due date, Swan won't process it until the next scheduled payment. Let your users know about this timing so they understand why their consent might still show as pending.
