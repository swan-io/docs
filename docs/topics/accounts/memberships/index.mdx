...
**Who can resolve binding errors depends on the error type:**

- **Self-resolvable**: Users can fix `emailVerifiedMatchError` and `idVerifiedMatchError` themselves by verifying their email or completing identification.
- **Admin resolution required**: All other binding errors (`firstNameMatchError`, `lastNameMatchError`, `birthDateMatchError`, `mobilePhoneMatchError`) can only be resolved by other account members with the `canManageAccountMembership` permission and status `Enabled`.

Refer to the guide to [fix a user binding error](./guide-fix-binding-error.mdx) for detailed resolution steps.
