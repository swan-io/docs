...
:::tip Stop renewal without canceling
To stop a physical card from renewing without canceling it, call the `cancelPhysicalCardRenewal` mutation while the card is in `ToRenew` status.
The card stays active until it expires, then isn't renewed.
See [Cancel a physical card renewal](./guide-renew.mdx#cancel-renewal).
:::
