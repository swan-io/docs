...
Visit **Payments** > [Internal Direct Debit](https://docs.swan.io/concept/payment/direct-debit#internal-direct-debit) for information about non-merchant use cases.
:::
...
| Feature | Standard | B2B |
| --- | --- | --- |
| Debtor account type | Individual and company | Company |
| Refund requested by debtor | Up to 8 calendar weeks after due date and time, with no questions asked | Not possible |
| Returned by Swan | Up to 5 business days for insufficient funds | Up to 3 business days for insufficient funds |
| Debtor consent required for received direct debit mandate | Not required | Mandatory |
...
| Frequency | One-off and recurring | One-off and recurring |
...
| Payment type | Choose one of the following options:<ul><li>Recurring</li><li>One-off</li></ul> |
...
| --- | --- |
| B2B | <ol><li>Mandate created with the status `ConsentInitiationPending`.</li><li>Before the debtor can consent, you must [declare the payment mandate](https://docs.swan.io/guide/accept-payments-merchants/accept-payments-with-internal-direct-debit#declare-a-payment-mandate-for-idd).</li><li>Debtor must consent, but only the first time the mandate is used.</li></ol> |
...
When the sequence value is `OneOff` (instead of `Recurring`), the received internal direct debit payment mandate is canceled automatically after the transaction is executed, regardless of the transaction status.
...
- [**Step-by-step**: Accept payments with Internal Direct Debit](./guide-idd.mdx)
