...
To receive notifications when a digital card is created or its status changes, subscribe to `DigitalCard.Created` and `DigitalCard.Updated` [webhooks](../../../developers/using-api/webhooks). `DigitalCard.Created` is triggered when the card is first created and before it becomes usable in a wallet. You should also subscribe to `DigitalCard.Updated` to track status changes.
