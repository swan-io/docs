...
|---|---|
| `WaitingForDocument` | Onboarding is created and ready for documents, or documents were submitted but Swan is requesting additional documents.<br/><br/>**Next steps**: Moves to `PendingReview` after onboarding is finalized. |
| `PendingReview` | Onboarding is finalized and a document review is needed.<br/><br/>**Next steps**: Can move to `Approved`, `Rejected`, or `Canceled` depending on partner actions and Swan decisions (or, if documents are insufficient, back to `WaitingForDocument`). |
| `Approved` | Swan approved the <Term id="supporting-documents">supporting document</Term> collection. |
...
|---|---|
| `WaitingForUpload` | URL to upload the document has been generated.<br/><br/>**Next steps**:<br/><ul><li>If the document is uploaded → `Uploaded`</li><li>If the collection switches to a final status before the document is uploaded → `NotUploaded`</li></ul> |
