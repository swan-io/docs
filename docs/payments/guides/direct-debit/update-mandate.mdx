...
Update <Term id="sdd">SEPA Direct Debit</Term> B2B received <Term id="payment-mandates">payment mandates</Term> with the API.
You **can't update** the sequence (`OneOff` or `Recurrent`).
...
Make sure to send the `consentUrl` to your user so they can provide <Term id="consent">consent</Term> for the update.
