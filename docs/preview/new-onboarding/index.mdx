...
The redesigned onboarding API is live. All guides under **Topics > Onboarding** use the new mutations, and the field reference pages are the canonical source for country-specific requirements and available options.

:::caution Deprecation timeline
New no-code (v1) onboardings stop at the end of September 2026. The previous onboarding mutations and queries are removed at the end of December 2026 — migrate before then to avoid disruption. Onboardings already started in v1 can still be [finalized](/topics/onboarding/overview/guide-finalize) until then.
:::

## Overview

The redesigned onboarding API replaces the previous mutations and queries. It uses consistent naming conventions, structured input types, and typed responses.

### What's changing

| Aspect | Deprecated API | New API |
|---|---|---|
...
For country-specific requirements and available options, see:
...
French company onboardings support pre-fill via the `companyInfoRegistryData` query for company, representative, and UBO fields.
