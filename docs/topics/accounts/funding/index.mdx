...
<mark>need more clarity here</mark><br /><br />
...
<mark>You must provide the `signatureDate` when [adding a SEPA Direct Debit funding source with the API](./guide-add-source.mdx)</mark>, and be able to provide proof of signature if requested by the funding source's financial institution.
...
| `Canceled` | Payment mandate is canceled and no longer available for use. If the funding source is `Canceled`, the payment mandate status also changes to `Canceled`. |
...
sequenceDiagram
    ...
    A-->>S: Adds an account funding source
    ...
    S-->>A: Requests consent to adding the account funding source<br />and the payment mandate
    alt
    rect rgb(253, 235, 219)
    A-->>S: Doesn't consent to payment mandate
    Note over S: All statuses remain the same
    end
    else
    ...
    alt declares mandate to debtor institution
    rect rgb(210 246 231)
        A-->>D: Declares payment mandate<br />to debtor institution
    ...
    else doesn't declare mandate to debtor institution
    rect rgb(253, 235, 219)
        A-->>S: Initiates account funding request
