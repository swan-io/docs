...
```
∗ The **requester** can be the <Term id="account-holder">account holder</Term>, the account's <Term id="legal-representative">legal representative</Term>, or an account member with the `canManageAccountMembership` permission.
The requester provides <Term id="consent">consent</Term> (diagram line 4).
