...
To manage potential risk, if suspicious activity is detected, Swan can change the status of a profile or a payment method to `Suspended` or `Rejected`.
...
| `Enabled` | Merchant profile or merchant payment method is active.<br /><br />Use `Enabled` merchant profiles to request payment methods and accept payments with activated payment methods. Use `Enabled` payment methods to accept payments.<br /><br />Updates to profiles and payment methods can only occur when they have the status `Enabled`. |
