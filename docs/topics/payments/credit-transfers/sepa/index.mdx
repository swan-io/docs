...
**IBAN verification** is a service that allows Swan to verify that credit transfers are sent to the right beneficiary *before* initiating the transfer, thus reducing errors and potential fraud.
When performing IBAN verification, Swan confirms that the beneficiary's name matches the name registered for the IBAN provided.
...
Refer to [Swan's public roadmap](https://swanio.notion.site/Swan-Public-Roadmap-385e4b2e91b3409786a6c8e885654a22) to see the other IBANs for which Swan plans to integrate IBAN verification. 
