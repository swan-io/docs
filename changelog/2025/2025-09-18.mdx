...
- A new `WaitingForInformation` status and `verificationRequirements` list will show missing information on merchant profiles and payment methods. Submit the required information using the `requestMerchantProfile` or `requestMerchantProfileUpdate` mutations.
...
- The fields `invoiceExample`, `expectedMonthlyPaymentVolume`, `merchantWebsite`, `socialNetwork`, `supportWebsite`, `termsAndConditions` will be **removed**.
...
To calculate card spending limits, each payment is recorded at the start of the hour. The rolling period is then based on that hour, not the exact transaction time.
