...
:::tip Prerequisites
If you're using the API, you'll need either a [project access token](https://docs.swan.io/api/authentication/project-access-token), or a [user access token](https://docs.swan.io/api/authentication/user-access-token) for the associated user.
Also, the [document must have the status](./about.mdx#document-statuses) `Uploaded`.
:::

## API guide

1. First, [retrieve the document ID](./guide-retrieve-info.mdx).
1. In the API Explorer, add the `deleteSupportingDocument` mutation.
