...
For users with non-sensitive access to accounts, defined by their [account membership permissions](/accounts/concepts/memberships#remove-identification), identification might not be necessary in some B2B situations.
:::
...
1. Follow the [guide to get a user access token](/dev-tools/using-api/authentication#auth-code).
...
To start an identification, follow the [guide to get a user access token](/dev-tools/using-api/authentication#auth-code).
...
For each [account country](/accounts/concepts/account#account-country) and type of account, Swan recommends a process and, if possible, offers other acceptable processes.
...
Sometimes, a [first transfer](/accounts/guides/onboarding/account-holders#first-transfer) is also required.
