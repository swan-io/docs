...
1. (If needed) Completes **email verification**, if this was included in the OAuth URL.
...
Share the support article on [Identification rejection reasons and solutions](https://support.swan.io/hc/en-150/articles/16421416513693-Identification-rejection-reasons-and-solutions) to help your users troubleshoot the identitfication process.
:::
...
flowchart TB
    ...
    EntersMobileNumber -- User on computer --> ReceivesLink([Receives text message with<br/>link to open on mobile device])
    ...
    ReceivesLink --> OpensLink([Opens link to verify number])
    OpensLink --> QueryParams
    ...
    UserStartedComputer -- Yes --> ConfirmsNumber([Confirms number a second time])
    ...
    style ReceivesLink fill:#D4D3D5,stroke:#514F59
    style OpensLink fill:#D4D3D5,stroke:#514F59
