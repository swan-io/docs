...
This lifecycle is described for onboarding-related collections; collections requested for [transactions or verification renewal](/accounts/concepts/documents#required-documents) move through the same statuses.

| Collection status | Explanation |
|---|---|
| `WaitingForDocument` | Onboarding is created and ready for documents, or documents were submitted but Swan is requesting additional documents.<br/><br/>**Next step**: Moves to `PendingReview` after onboarding is finalized. |
| `PendingReview` | Onboarding is finalized and a document review is needed.<br/><br/>**Next step**: Can move to `Approved`, `Rejected`, or `Canceled` depending on partner actions and Swan decisions (or, if documents are insufficient, back to `WaitingForDocument`). |
...
|---|---|
| `WaitingForUpload` | URL to upload the document has been generated.<br/><br/>**Next step**:<br/><ul><li>If the document is uploaded → `Uploaded`</li><li>If the collection switches to a final status before the document is uploaded → `NotUploaded`</li></ul> |
...
| `Refused` | Swan refused the document (refer to [`Refused` document reason codes](#refused)). |
| `NotUploaded` | URL was generated but collection was closed before a document was uploaded. |

:::note Deleting a document
An `Uploaded` document can be deleted by you or by Swan (for example, if the wrong file was shared). Deletion removes the document rather than assigning it a status. See [Delete a document](/accounts/guides/documents/delete).
:::

## `Refused` document reason codes {#refused}

When any type of <Term id="supporting-documents">supporting document</Term> is `Refused`, the reason is displayed on your Dashboard.
Navigate to the supporting document area for the relevant onboarding, transaction, or <Term id="account-holder">account holder</Term> verification.
