...
You can **print an additional line** of text underneath the card holder name.
...
You can request a number plate system instead of card holder names if it's logical for your use case.
For example, when cards stay with a vehicle and are used exclusively to purchase fuel, a number plate system makes sense.
...
**10 weeks before** the original card expires, **verify the delivery address** with your card holder.
...
After the card holder makes an in-person transaction with the new card using the Chip and PIN payment method, the new card's status changes from `Renewed` to `Activated`.
