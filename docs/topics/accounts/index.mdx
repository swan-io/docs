...
Statements include all `Booked` transactions from the previous month, ordered by [`sequenceNumber`](../payments/index.mdx#sequence-number).
...
| `Suspended` | Swan suspends an account while a compliance, KYC, regulatory, or other review is ongoing. An account can have several active suspension reasons at once, and it returns to `Opened` only after all of them are resolved. While suspended, the account can't be used. This results in the following events: <br/><br/> 1. All card payments are `Rejected`. <br/> 2. SEPA Credit Transfers: <br/> &nbsp;&nbsp;&nbsp;&nbsp; ◦ Incoming Credit Transfers are `Accepted`. <br/> &nbsp;&nbsp;&nbsp;&nbsp; ◦ Outgoing Credit Transfers are `Rejected`. <br/> 3. SEPA Direct Debits (SDD): <br/> &nbsp;&nbsp;&nbsp;&nbsp; ◦ Incoming SDDs are `Booked`. <br/> &nbsp;&nbsp;&nbsp;&nbsp; ◦ Outgoing SDDs are `Rejected` or `Returned` on the scheduled date. <br/> 4. Funds available in the account can't be used while Swan reviews the suspension. <br/> 5. Some previously authorized transactions are `Booked`. <br/> 6. For merchant accounts: received French check payments are `Returned`. <br/><br/> You can retrieve the current [suspension reasons](#suspension-reasons) with the API. |
...
Swan provides suspension reasons so you can understand why an account is `Suspended`.
Retrieve them with the API by [querying the account's suspension information](./closure/guide-get-suspension-info.mdx), or view them in your **Dashboard** > **Data** > **Accounts** on the account's detail page.

Reasons are available for most accounts suspended from 21 September 2026.
The API returns an empty `suspensionReasons` list for accounts suspended before this date, and for some suspensions triggered before this date but applied in the following weeks.

| Reason code | Explanation |
| --- | --- |
| `ComplianceReview` | The account is under review by Swan. No action needed from you. |
| `KYCReview` | The account holder hasn't responded to a KYC request. Consider contacting the account holder. |
| `RegulatoryRequirement` | The account is suspended to comply with a regulatory obligation. No action needed from you. |
| `Other` | Reason not covered by the other values. Swan doesn't share additional detail. |

:::note Multiple active reasons
Several reasons can be active at the same time.
The account stays `Suspended` until every active reason is resolved.
:::
