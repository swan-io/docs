...
| `WaitingForTransfer` | Waiting for the shareholder to deposit the full share capital in their Swan account created during onboarding, which **must** be transferred from an account belonging to the shareholder. The transfer must be a [SEPA Credit Transfer](/accounts/guides/capital-deposits/#transfer-requirements); international credit transfers aren't accepted and are rejected automatically. |
