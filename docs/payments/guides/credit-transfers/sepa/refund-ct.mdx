---
title: Refund a credit transfer
audience:
  - dev
related:
  - /payments/concepts/credit-transfers/sepa
  - /payments/guides/credit-transfers/sepa/cancel-ct
---

# Refund a credit transfer

Refund an incoming <Term id="credit-transfer">SEPA Credit Transfer</Term> with the API.

## Guide {#guide}

1. Call the `refund` mutation.
1. Add the `originTransactionId`, `consentRedirectUrl`, and `amount`.
1. Add the `RefundSuccessPayload` payload.
1. Add the consent URL to the success payload (line 15): `consent` > `consentUrl`.
1. Add the `RefundRejection` rejection in case of error.

## Mutation {#mutation}

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {6,15} showLineNumbers
mutation RefundCreditTransfer {
  refund(
    input: {
      consentRedirectUrl: "$YOUR_REDIRECT_URL"
      refundTransactions: {
        originTransactionId: "YOUR_ORIGIN_TRANSACTION_ID"
        amount: { value: "22", currency: "EUR" }
      }
    }
  ) {
    ... on RefundSuccessPayload {
      __typename
      consent {
        id
        consentUrl
      }
    }
    ... on RefundRejection {
      __typename
      code
      message
    }
  }
}

```

import Environments from '../../../../_shared/partials/_environments.mdx';

<Environments />

## Payload {#payload}

The payload provides the `consentUrl` to finalize the refund.

```json {7} showLineNumbers
{
  "data": {
    "refund": {
      "__typename": "RefundSuccessPayload",
      "consent": {
        "id": "YOUR_REFUND_ID",
        "consentUrl": "https://identity.swan.io/consent?consentId=$YOUR_CONSENT_ID&env=Sandbox"
      }
    }
  }
}
```
