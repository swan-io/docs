...
| **Data requirements** | Limited data collection | ~25-30 new fields for improved risk assessment and compliance |
...
- `accountInfo`: Account-level details (`country`, `name`, `intendedUsage` <sup>(new)</sup>).
- `accountAdmin`: Individual's personal information (`email`, `employmentStatus`, `monthlyIncome`, `address`, `sourceOfFunds` <sup>(new)</sup>, etc.).
...
New dedicated queries retrieve company information from official registries, enabling pre-fill capabilities for smoother onboarding. This replaces the previous behavior where empty onboarding fields were automatically populated with registry data.
...
Both queries require a `registrationNumber` and `residencyAddressCountry`. Currently available for companies registered in **France** (`FRA`) only.
