...
This number is tied to **balance movements** on the account: each time a movement is applied to the account booked balance, the counter increments.
Since one booked transaction triggers one balance movement, the `sequenceNumber` effectively increases with each new booked transaction.

This provides a reliable way to **order transactions chronologically**.
