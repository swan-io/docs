...
audience:
  - dev
counterpart: /accounts/guides/account-operations/generate-statement/from-the-dashboard
related:
  - /accounts/guides/account-operations/generate-statement
  - /accounts/guides/account-operations/generate-statement/from-the-dashboard
...
1. Set the `language` for your account statement, and choose the file `format` (`PDF` or `CSV`) (lines 7-8).
...
1. Add `statusInfo` for all statuses (line 8). Find the URL to download the statement in `statusInfo` > `GeneratedStatusInfo` > `url` (line 12).
