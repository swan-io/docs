...
Using mobile phone numbers to identity users greatly simplifies their experience: users only need to know their mobile number to log into Swan, and occasionally their chosen 6-digit passcode.
...
When a user wants to access their data or make transactions through your service with Swan's API, they must be [authenticated](../authentication/index.mdx).
...
1. Confirms their mobile number, after which they continue the identity verification process on their mobile device. How they confirm step depends on whether they opened the onboarding link from a computer or their mobile device:
    - **Computer**: Swans sends your user a text message with a link to open on their mobile device.
    - **Mobile device**: Swans sends your user a text message with a verification code. 
...
If you want information about a `userId`, [impersonate the user](../authentication/guide-impersonate.mdx) with a project access token instead.
