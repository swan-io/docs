...
| --- | --- | --- |
| `Match` | Exact match found between provided details and account holder information | Safe to proceed with transfer |
| `CloseMatch` | Close match with suggested correction from beneficiary's bank | Review suggested name correction before proceeding |
...
In the Sandbox environment, the `verifyBeneficiary` mutation will return simulated verification results based on the beneficiary name pattern:

- Names starting with "Match" return `Match` result.
- Names starting with "Close" return `CloseMatch` result.
- Names starting with "No" return `NoMatch` result.
- Names starting with "Not" return `VerificationNotPossible` result.

This allows you to test all verification scenarios during development.