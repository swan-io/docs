...
- The transfer exceeds the [maximum transaction amount limit](/payments/concepts/credit-transfers/international#transfer-limits). 
...
When Swan receives an incoming International Credit Transfer, Swan books it immediately and creates an `InternationalCreditTransferTransaction` type (step 8 in the standard flow described).
...
You can use the `Transaction.Booked` [webhook](/build/using-api/webhooks) to be notified any time a transaction is created.
