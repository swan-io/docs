...
**Account statements** include **all booked transactions** for a given period, including:

- SEPA Credit Transfers (incoming and outgoing).
- SEPA Direct Debit transactions (SDD In and SDD Out).
- Card transactions.
- Fee transactions.
- Internal transfers.
- All other transaction types.

This is **different from transaction confirmation statements**, which only confirm specific SEPA Credit Transfer transactions.

To generate a confirmation for a single SEPA Credit Transfer transaction, see [transaction confirmation statements](https://docs.swan.io/topics/payments/overview/guide-generate-transaction-statement/).
:::
