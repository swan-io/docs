---
title: Invoices
related:
  - /accounts/concepts/billing
  - /accounts/guides/billing/get-list
  - /accounts/guides/billing/get-details
audience:
  - dev
  - ops
---

# Invoices {#invoices}

Swan **generates a dedicated invoice** automatically for all **company** <Term id="account-holder">account holders</Term>, generated at the same time as the [monthly account statement](/accounts/concepts/account/statements).
The invoice lists all banking fees charged during the **monthly billing period**, which begins on the first of each month.

:::info Individual accounts
Monthly invoices aren't generated for **individual** accounts.

Instead, fees are included on the standard [monthly account statement](/accounts/concepts/account/statements).
Swan includes an **annual fee statement** automatically once a year, at the end of January's account statement.

Individual account holders can also review fee transactions in their transaction history.
If you're not using Swan's Web Banking interface, make sure to include fees in your implementation of the [transaction history](https://swan-io.github.io/swan-partner-frontend/specs/banking/history).
:::

## Refund notes and skipped invoices {#refund-notes}

If a **fee is refunded** and the corresponding invoice was already generated, Swan generates a refund note for the refunded amount.
Access refund notes the same way you access invoices.
If **no banking fees were charged** during the billing period, Swan doesn't generate an invoice.

## Invoice language {#language}

Invoices are generated in the [account language](/accounts/concepts/account/language), and labels for each fee transaction are also in the account language.
You or your user can change the account language; doing so impacts new invoices and fees, but won't change existing fee labels or previously generated invoices.

## Access and notifications {#access}

Subscribe to the `AccountInvoice.Generated` [webhook](/build/using-api/webhooks) to receive a notification each time an invoice is generated.
You can also access company account invoices with the API, from your Dashboard, or on Swan's Web Banking interface.

## VAT number {#invoices-vat}

Optionally, add your account holder's Value Added Tax, or VAT, number.
If you do, Swan adds the VAT number to the invoice.

You can add the VAT number several ways:

- When [creating the onboarding link](/accounts/guides/onboarding/company/create) for the new company account
- When [updating a company onboarding](/accounts/guides/onboarding/company/update)
- When [updating an account holder](https://explorer.swan.io?query=%3D%3D&tab=api) using the `updateAccountHolder` mutation

:::note European companies outside of France
If you provide a valid VAT number for a company registered in a European country other than France, Swan automatically applies the **reverse charge mechanism**.
With the reverse charge mechanism, **VAT isn't added** and **must be paid by the end user** in accordance with article 283-2 of the French General Tax Code.
:::
