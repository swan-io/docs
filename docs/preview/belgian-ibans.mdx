...
Belgian account memberships follow standard Swan patterns with no special requirements:

<FieldRequirementsLegend />

| API field | Requirement | Notes |
|---|:---:|---|
| `accountId` | <Req /> | Link to Belgian account |
| `email` | <Req /> | Member email address |
| `restrictedTo.firstName` | <Req /> | Member identification |
| `restrictedTo.lastName` | <Req /> | Member identification |
| `residencyAddress` | <Opt /> | No special requirements |
| `taxIdentificationNumber` | <Opt /> | Not required for members |