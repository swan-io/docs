...
This guide helps partner support teams answer user questions about SEPA Direct Debit mandates. It covers checking mandate details in the Dashboard, [troubleshooting rejected payments](#troubleshoot-rejected-direct-debits), and knowing when to [raise a Swan Support request](#contact-swan-support).
...
When a company (creditor) needs to take money from your user's account (for subscriptions, rent, utilities, or other recurring payments), they use a direct debit. When your user (debtor) is a business, they authorize this by signing a mandate.
...
:::warning
Any update triggers a new consent cycle. The mandate can't be used until your user consents to the updated details. After the user consents, the mandate status returns to `Enabled`. Warn the user if a payment is due soon; if they don't approve in time, the next direct debit will fail.
:::
...
| --- | --- | --- | --- |
| **Suspend** | Temporarily blocks future direct debits. Any pending instructions are rejected. | Yes, the mandate can be re-enabled later. | Through your product (if your dev team has integrated it) or through a Swan Support request. |
| **Cancel** | Permanently ends the mandate. Future direct debits from this creditor are rejected. | No. | By Swan Support only. |
...
1. Confirm with your user whether they want to pause direct debits temporarily (suspend) or stop them permanently (cancel).
...
:::warning
If the user hasn't contacted the creditor, ask them to try resolving it directly with them first. Swan will ask for this evidence.
:::
