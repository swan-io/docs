...
- **Category-index pattern — pick ONE wiring, never both (13 August 2026):** a category that sets `link:` to its index must NOT also list that index as a child — the index then appears twice in the pagination sequence and its "next page" loops back to itself. Either `link:` the index and leave it off the child list (the dual-path task pattern above), or go link-less with the index as an explicit "Overview" child (the guide task-group pattern; Onboarding, Capital deposits, and Memberships precedent — the category click still lands on the index via `ia-sidebar-index-targets.js`).
