...
| Method | Explanation |
| --- | --- |
| Phone number | <ul><li>Phone's SIM card serves as the authentication factor.</li><li>Account member can be assigned all [membership permissions](#permissions).</li></ul> |
| Email | <ul><li>Verified email serves as the authentication factor, and the membership isn't enabled until user verifies their email.</li><li>Account member can only be assigned the `canViewAccount` and `canManageCards` [memberships permissions](#permissions).</li><li>Swan still collects the user's phone number during the [sign-up process](../../users/index.mdx#signup) so they can perform sensitive operations such as initiating payments, ordering cards, and viewing sensitive card information.</li></ul> |
