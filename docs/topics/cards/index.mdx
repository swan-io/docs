...
Swan aggregate spending limits at the hour level for all cards with monthly, weekly, and daily spending limit periods. This change affects spending limits computation only and provides more precise control over rolling periods.

#### How hour-level aggregation works

For spending limits computation only, each payment will be timestamped at the first second of the first minute of any hour (:00:00). Each hour will reset the rolling period calculation.

Example: Payment at `Today 16:30` on a card with daily spending limit
- Current spending calculated from: `Yesterday 16:30 (excluded)` to `Today 16:30 (included)`
- If payment is accepted, it will be timestamped at: `Today 16:00:00`
- Result: `Tomorrow 16:00` the payment will no longer be included in daily spending calculation

#### Affected spending limit periods

- `Daily` (24 hours rolling) - payments timestamped to the hour.
- `Weekly` (7 days rolling) - payments timestamped to the hour.
- `Monthly` (30 days rolling) - payments timestamped to the hour.

:::info Rolling spending limit
This change does not affect `Always` (lifetime) spending limits or Swan's imposed spending limits.
:::
