...
By default, when a user performs SCA in a mobile app, Swan opens the smartphone's default browser and redirects the user out of the mobile app to complete SCA. Once SCA is complete, the user must exit their smartphone browser and return to the mobile app.

With SCA integration, you can embed an in-app browser directly into your mobile app so SCA opens within the application. Once SCA is complete, the user closes the in-app browser and remains inside your mobile app.
...
## When to use SCA integration {#when-to-use}

SCA integration is relevant when you have a mobile app and your users perform all actions from the mobile app. This improves the user experience by keeping users inside your mobile app rather than redirecting them to an external browser.
...
- `consentId`.
- `env`: `Sandbox` or `Live`.
- `status`: `Accepted`, `CustomerRefused`, `OperationCommitting`, `CredentialRefused` (deprecated), `Created`, `Started`, `Expired`, `Failed`, or `Canceled`.
