...
Initiating credit transfers is a [sensitive operation](../../../users/consent/index.mdx#sensitive) and requires consent from an account member with the `canInitiatePayments` [membership permission](/accounts/reference/memberships/membership-permissions#permissions).
