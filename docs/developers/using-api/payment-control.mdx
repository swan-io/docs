...
The `partialAuthorizationAmountValue` field accepts only decimal values and always returns amounts in EUR, which is Swan's account currency. 
You must provide the amount in EUR, even if the user made the card payment in another currency, such as USD.
:::
