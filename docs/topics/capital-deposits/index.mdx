...
| `WaitingForVerification` | Possible status if the account holder verification isn't complete at the moment the shareholder's account is created<br/><br/>_Status bypassed if the [account holder status](../onboarding/account-holders/index.mdx#verification-process-statuses) is already `Verified`_ |
