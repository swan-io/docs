...
- **Categories without a `link:` are auto-wired** by `ia-sidebar-index-targets.js`: click (and direct URL, via generated redirects) resolves to the category's own `<dir>/index` child if listed, else the nearest ancestor index. All children of an index-less category must share ONE directory, or the generated redirect degrades (fires from the shallower common path, or not at all — the direct URL then 404s). To give such a category a real landing page, list its index as a child labeled "Overview" (get-started/become-a-partner precedent).
...
- **Docs v2/v3 fork alignment (deferred by decision, 12 August 2026):** this branch forked from live in June 2026; live Docs v2 kept evolving. Known drift so far: `accounts/concepts/closure/reason-codes.mdx` predates the June reversal (live schema deprecates `DormantAccount` → `Inactivity` reinstated with new description; `CloseAccountReasonType.CapitalDepositCanceled` deprecated → `CapitalDepositWithdrawn`; ALL accounts, including pre–2 April 2026 closures, retroactively migrated to the 2026 codes — so "keep their original reason codes" claims are stale). Live v2 (`docs.swan.io/topics/accounts/closure/`) already carries the corrected wording to align with. Repo changelog stops at 7 May 2026; live has 18 June, 16 July, 30 July entries. During alignment, sweep ALL pages against live v2 and the post-May changelog entries, not just this one.
