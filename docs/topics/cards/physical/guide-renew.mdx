...
To stop a physical card from renewing without canceling it, call the `cancelPhysicalCardRenewal` mutation while the card is in `ToRenew` status. The card stays active until it expires, then isn't renewed.
...
The card stays active until its expiry date, then expires instead of renewing.
...
- Subscribe to the `Card.Updated` [webhook](/developers/using-api/webhooks/#events-cards) to know when a card enters `ToRenew` status.
:::
...
It stays active until its expiry date, then expires instead of being renewed.
