...
The `createMultiConsent` mutation combines up to 100 individual consents for sensitive operations into a single [multi-consent](/users/concepts/consent#multi-consent), so your users can approve several sensitive operations at once with fewer consent prompts.
