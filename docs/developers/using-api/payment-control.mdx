...
- **€0 Authorizations**: Enable zero-amount authorization forwarding. These are transactions merchants use to validate cards without charging them. Disabled by default.
- **Credit Authorizations**: Enable credit authorization forwarding for refund-related transactions (identified by `operationType: "Credit"`). Disabled by default.
