...
When Swan receives an incoming <Term id="credit-transfer">International Credit Transfer</Term>, Swan books it immediately and creates a `InternationalCreditTransferTransaction` type (step 8 in the [standard flow](/payments/concepts/credit-transfers/international/incoming#incoming-flow) for incoming transfers). 
