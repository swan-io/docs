...
| --- | --- | --- | --- |
| `amount` | `AmountInput` | ✅ | Amount for the payment intent. |
| `label` | `String` | No | Label for the payment intent, displayed on the merchant's bank statement. |
...
After creating the payment intent, the merchant uses their terminal app to process the payment. The payment intent moves through several statuses as it progresses.
...
| `Expired` | Payment intent expired before it could be processed. |

## Step 3: Confirm the payment {#confirm-payment}

After the payment is completed, the merchant payment object is updated and the underlying transactions are created as `Upcoming`. When [settlement](./index.mdx#settlement) arrives, the transactions are updated to `Settled`.
