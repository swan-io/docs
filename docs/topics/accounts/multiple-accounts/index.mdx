...
:::info Account status 
All accounts, regardless of their status, count towards the maximum limit. `Suspended`, `Closing`, or `Closed` accounts are counted. 
:::
...
| `AccountHolderAccountsSuspendedStatusNotEligibleRejection` | One or more accounts are `Suspended` for compliance reasons. Account creation is suspended. |
| `AccountHolderAccountsCreationLimitRejection` | The maximum number of accounts has been reached (all account statuses count toward the limit). Check project settings for the Multiple Accounts feature. |
