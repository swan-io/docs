...
| `cardContractExpiryDate` | Date you'd like the card to expire.<br /><br />Leave it empty if you don't want to set an expiration date, in which case, cards are renewed automatically every three years. Refer to [renewal and expiry](./virtual/index.mdx#renewal) for details.<br /><br />To stop automatic renewal for an already-issued physical card, use the `cancelPhysicalCardRenewal` mutation. See [Cancel a physical card renewal](./physical/guide-renew.mdx#cancel-renewal). | API |
