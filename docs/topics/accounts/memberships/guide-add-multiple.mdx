...
1. Add the account ID to which you're inviting account members (line 4).
1. Add the `consentRedirectUrl` (line 5):
    - `consentRedirectUrl` is **optional** when all invited members have no permissions (only `canViewAccount: false` or no permissions at all).
    - `consentRedirectUrl` is **mandatory** when any invited member is granted permissions beyond viewing. The API will return a validation rejection if missing.
