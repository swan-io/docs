...
Because errors aren't [strongly typed](/build/graphql/types) like the rest of the GraphQL schema, there's **no guaranteed format or structure** to error responses.
...
In the Swan [schema](/build/graphql/schema), a `Rejection` is an object containing a descriptive message.
...
- [Authentication](/build/using-api/authentication) · [Idempotency](/build/using-api/idempotency) · [Webhooks](/build/using-api/webhooks)
