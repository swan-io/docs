...
| Permission type | Add trusted beneficiaries<br/>via save parameter | Initiation using<br/>swanAccount token | Initiation using<br/>TrustedBeneficiary token |
|---|:---:|:---:|:---:|
| **canManageBeneficiaries** only | <Yes />  | <No /> | <No /> |
| **canInitiatePayments** only | <No /> | <Yes />  | <Yes />  |
...
This feature will be available soon in Swan's API.
:::
...
No Trusted Beneficiary is added when calling `verifyBeneficiary`, but your request to save it is contained within the token. The beneficiary is only saved after successful payment authorization, similar to the functionality described in [Add a beneficiary](../sepa/guide-add-beneficiary).
...
Company end-users can opt-out of VoP for bulk transfers through the `updateAccountSettings` mutation. Individual end-users cannot opt-out. This setting applies to both Sandbox and Live environments.
