...
1. Add all other objects you need to review.
    - Example: add `supportingDocumentPurpose` to know which supporting document is required (line 10).
    - 
    - Example: add `supportingDocuments` > `statusInfo` > `status` (lines 13-14) to retrieve the document's status.
    - Example: add `supportingDocuments` > `statusInfo` > `status` (lines 13-14) to get information about why a document was `Refused` (line 15).
