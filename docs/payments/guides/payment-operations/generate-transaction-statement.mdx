...
- You have a [project access token](/build/using-api/authentication#get-token-project).
- The [transaction status](/payments/concepts/transactions/statuses#transactions-statuses) is `Booked`.
- Subscribe to the `TransactionStatement.Generated` [webhook](/build/using-api/webhooks#events-transactions).
:::
...
For a comprehensive statement including all transaction types and fees, see [account statements](https://docs.swan.io/accounts/guides/account-operations/generate-statement).
...
If the [transaction statement status](/payments/concepts/transactions/statuses#transactions-statements-statuses) is `Failed` or `Expired`, **return to step 1** to generate your statement again.
:::
