...
Follow the guide to [add additional company accounts](/accounts/concepts/multiple-accounts#create) for an existing account holder.
...
However, while account holder verification is in progress, there are [limitations on the account](/accounts/concepts/account#account-type-level).
...
| `WaitingForInformation` | Swan is waiting for information from the account holder, such as a [first transfer](#first-transfer), [supporting documents](/accounts/concepts/documents), or something else. |
...
:::caution 🇩🇪 Germany
A first transfer is always required for self-employed and individual accounts for which the **[account country](/accounts/concepts/account#account-country)** is **Germany**, regardless of registration or residency.
:::
...
While there is technically no deadline for sending the transfer, the user's [account is `Limited`](/accounts/concepts/account#account-type-level) until the transfer is validated.
...
Learn more about [requesting a first transfer](/accounts/guides/onboarding/account-holders#request-first-transfer) and [account limitations and capabilities](/accounts/concepts/account#account-type-level).
