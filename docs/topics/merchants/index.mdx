...
Review the [full list of merchant webhooks](/dev-tools/using-api/webhooks#events-merchants), or subscribe to them directly from your **Dashboard** > **Developers** > **Webhooks**.
...
To learn how to request payment methods, refer to the request payment method guides for [card payments](./online/cards/guide-request.mdx), [SEPA Direct Debit](./online/sdd/guide-request.mdx), [Internal Direct Debit](./online/idd/guide-request.mdx), [in-person cards](./in-person/cards/guide-request-method.mdx), and [French checks](./in-person/checks/guide-request.mdx).
...
- **Rolling reserve value, payment amount limit, and payment methods**: 
  subscribe to the [merchant payment methods webhooks](/dev-tools/using-api/webhooks#events-merchants-payment-methods).  

- **Merchant profile changes**:
  subscribe to the [merchant profile webhooks](/dev-tools/using-api/webhooks#events-merchants-profiles).  

- **Transaction-level rolling reserve releases**:
  the `TransactionRollingReserve.Updated` and `TransactionRollingReserve.Released` [webhooks](/dev-tools/using-api/webhooks#events-transactions) notify you when the rolling reserve of an individual transaction is updated or released.  
...
| Merchant's own SCI | **Merchant's name** | Requires [updating the merchant payment method](./online/sdd/guide-update.mdx) through the API. French merchants can request an SCI through the [Partner Support Request form](https://supportform.swan.io/).|
...
- Request payment methods:
    ...
    - [In-person cards](./in-person/cards/guide-request-method.mdx)
    - [French checks](./in-person/checks/guide-request.mdx)
