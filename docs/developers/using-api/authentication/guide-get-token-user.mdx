...
After it's decoded, you can use the `sub` field to identify the user. The `sub` in the `id_token` represents the `userID`. In Sandbox, this is the [Sandbox](/developers/tools/sandbox-users/) `userID`.
:::
