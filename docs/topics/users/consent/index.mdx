...
| `Failed` | The user successfully accepted the consent. However, a technical error occurred during the execution. This means the operation did not complete and the user must retry.<br /><br />For multi-consents, this status indicates a child consent `Failed`; any child consents already `Accepted` remain valid, while unexecuted child consents are `Canceled`. |
