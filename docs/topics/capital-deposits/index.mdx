...
The `CapitalDepositCase` object includes two status fields that work together to provide visibility into your capital deposit progress:

- **`status`**: The main case status that indicates which major phase the case is in.
- **`statusDetails`**: A sub-status that provides additional context about what's happening within certain phases. When the status detail would simply duplicate the main status, this value is `null` to avoid confusion.

```mermaid
%%{ init: { 'flowchart': { 'curve': 'linear' } } }%%
flowchart LR
    DC((Capital deposit<br/>case created)) -.-> WR[WaitingFor<br/>Requirements<br/><em>statusDetails active</em>]
    WR -.-> WS[WaitingForShare<br/>DepositCertificate<br/><em>Notary</em>]
    WS -.-> WRE[WaitingFor<br/>RegisterExtract]
    WRE -.-> WNT[WaitingFor<br/>NotaryTransfer<br/><em>statusDetails active</em>]
    WNT -.-> CA[Canceled]
    WRE -.-> CA
    WS -.-> CA
    WR -.-> CA
    ...
    subgraph F[Final status]
        direction TB
        CA
        style V fill:#fff,stroke:#fff,stroke-width:0px,color:#fff
        CO
    end

    subgraph CO[Completed]
    end
...
#### Status reference {#case-statuses-reference}

| Case status | Status details | Explanation |
| --- | --- | --- |
| `WaitingForRequirements` | `Started` | The case has been initiated and is pending shareholder verification, transfers, document uploads, and completion of company onboarding. |
| `WaitingForRequirements` | `Pending` | Swan is reviewing the submitted information. |
| `WaitingForRequirements` | `WaitingForInformation` | After review, Swan has requested additional details from the customer and is awaiting their response. |
| `WaitingForRequirements` | `null` | All initial requirements are being gathered. This is the default status after the `CapitalDepositCase` is created, provided there were no technical issues.<br/><br/>You complete the company onboarding process and upload required documents to pass to the next status. |
| `WaitingForShareDepositCertificate` | `null` | **_Notary reviews_** _the deposited funds and submitted documents, then creates the certificate and uploads it to Swan_. |
| `WaitingForRegisterExtract` | `null` | You upload your register extract for notary review. |
| `WaitingForNotaryTransfer` | `ReadyToBeVerified` | The company's account is ready and awaiting final verification. |
| `WaitingForNotaryTransfer` | `Verified` | The company's account has been successfully verified. |
| `WaitingForNotaryTransfer` | `null` | **_Notary reviews_** _the register extract_. If the document is rejected by either Swan or the notary, the case will move back to `WaitingForRegisterExtract`. |
| `Canceled` | `null` | You can request to cancel a `CapitalDepositCase` any time before completion by submitting a request to [Swan Support](https://partner-support.swan.io/hc/en-gb/requests). When a case is canceled, the shareholder is also canceled. |
| `Completed` | `null` | Notary reviewed and approved all elements and transferred the funds to the new company; the process is complete. |
