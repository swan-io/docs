...
### Ultimate beneficial owners {#onboarding-form-ubo}

An ultimate beneficial owner (UBO) is an owner of a company holding more than 25% of that company, whether directly or indirectly.
For UBOs owning 25% or more of the company, set the UBO `type` to `HasCapital` in the API.
Then, provide the percentage of capital each UBO owns and whether they own the capital directly or indirectly.
...
Swan considers them the default UBO and therefore the company's legal representative.
For default UBOs owning less than 25% of the company, set the UBO `type` to `LegalRepresentative` in the API.

### Country-specific information {#onboarding-form-country}
