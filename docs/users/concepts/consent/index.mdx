...
You can either use a [user access token](/build/using-api/authentication#tokens-user) in the name of the user wanting to make the payment, or with a [project access token](/build/using-api/authentication#tokens-project) impersonating that user.
...
<details>
  <summary>Video completing Strong Customer Authentication on mobile</summary>
    <div>
        <img src={require("../../../images/users/consent/sca-flow.gif").default} alt="Map of Swan coverage for individual and company accounts" />
    </div>
</details>
...
The following mutations are sensitive operations and typically require consent. Some may not, depending on context (for example, `suspendAccountMembership`).
...
<h3 id="sensitive-accounts">Accounts and onboarding</h3>
...
When the user consents to the multi-consent, the status changes to `OperationCommitting` and the user is informed their multi-consent is complete.
...
Human interaction is always required for SCA, but Swan makes an exception for your own accounts.
...
### Role of projects and legal reps {#s2s-projects}
...
Users are guided through a  similar but slightly different flow for online payments that require [3-D Secure consent](/payments/concepts/cards#3ds).
