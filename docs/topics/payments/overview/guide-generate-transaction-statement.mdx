...
- You have a [project access token](/dev-tools/using-api/authentication#get-token-project).
- The [transaction status](../index.mdx#transactions-statuses) is `Booked`.
- Subscribe to the `TransactionStatement.Generated` [webhook](/dev-tools/using-api/webhooks#events-transactions).
:::
