...
The number increments by 1 with each new transaction that affects the account's booked balance, providing a reliable chronological record you can use to detect gaps or out-of-order processing.

:::note Migration in progress
`sequenceNumber` is populated for all newly Booked transactions.
Backfilling historical transactions is in progress; older transactions may not yet have a value.
:::

Retrieve `sequenceNumber` from `statusInfo` using the `BookedTransactionStatusInfo` fragment:
...
The `sequenceNumber` field is only populated when the transaction status is **Booked**; it returns `null` for all other statuses.
