...
Hour-level aggregation provides more precise control over rolling period calculations, ensuring that spending limit is computated consistently. 
...
When a payment is processed, it's rounded down to the start if the hour (HH:00:00) for spending limit calulations. The rolling window is then calculated form that hour mark, not the exact transaction time. 

**Example**: Payment make `Today` at **16:30** on a card with daily spending limit
- For computation, the payment is recorded as `Today` at **16:00**.
- The 24-hour (daily) rolling window closes `Tomorrow` at **16:00**, at which point the payment is no longer included in daily spending calculation

#### Affected spending limit periods

Payments are timestamped to the hour for the following limit periods:

- `Daily`: 24 hours rolling
