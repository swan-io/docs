---
title: Update member
audience:
  - dev
related:
  - /accounts/concepts/memberships/inviting
  - /accounts/concepts/memberships/permissions
  - /accounts/reference/memberships/membership-permissions
  - /accounts/reference/memberships/membership-fields
  - /accounts/guides/memberships/fix-binding-error
  - /accounts/guides/memberships/suspend
  - /accounts/guides/memberships/resume
---

# Update member

You can update the permissions for an existing <Term id="account-membership">account membership</Term>, as well as the account member's personal information, with the API.
If you use Swan's Web Banking interface, your users can also update account memberships from their Web Banking accounts.

import Prerequisites from './partials/_update-prereqs.mdx';

<Prerequisites />

## Guide {#guide}

1. Call the `updateAccountMembership` mutation.
1. Add the account membership ID and your redirect URL.
1. Add any information or permissions you'd like to update (lines 5-13).
1. Add the success payload with the `consentUrl` (line 19).
1. Add rejections (starting on line 22).
1. Get the `consentUrl` from the payload and send it to the account member updating the membership. Their <Term id="consent">consent</Term> finalizes the update.

## Mutation {#mutation}

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {4,13,19} showLineNumbers
mutation UpdateMembership {
  updateAccountMembership(
    input: {
      accountMembershipId: "$MEMBERSHIP_ID"
      canViewAccount: true
      canManageAccountMembership: true
      canManageBeneficiaries: true
      canManageCards: true
      canInitiatePayments: true
      consentRedirectUrl: "$REDIRECT_URL"
      email: "$MEMBER_EMAIL"
      taxIdentificationNumber: "$MEMBER_TAX_ID"
      language: en
    }
  ) {
    ... on UpdateAccountMembershipSuccessPayload {
      __typename
      consent {
        consentUrl
      }
    }
    ... on AccountMembershipCannotBeUpdatedRejection {
      id
      message
    }
    ... on AccountMembershipNotFoundRejection {
      id
      message
    }
    ... on ForbiddenRejection {
      __typename
      message
    }
    ... on InvalidPhoneNumberRejection {
      __typename
      message
    }
    ... on PermissionCannotBeGrantedRejection {
      __typename
      message
    }
    ... on UserNotAllowedToManageAccountMembershipRejection {
      __typename
      message
    }
    ... on ValidationRejection {
      __typename
      fields {
        code
        message
        path
      }
      message
    }
  }
}
```

## Payload {#payload}

The eligible account member updating the membership must consent to the operation with the `consentUrl` (line 6).

```json title="Payload" {6} showLineNumbers
{
  "data": {
    "updateAccountMembership": {
      "__typename": "UpdateAccountMembershipSuccessPayload",
      "consent": {
         "consentUrl": "https://identity.swan.io/consent?consentId=$YOUR_CONSENT_ID&env=Sandbox"
      }
    }
  }
}
```
