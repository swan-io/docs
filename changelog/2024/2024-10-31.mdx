...
⚠️ Starting on **November 28**, we’ll start enforcing [two required fields for ultimate beneficial owners](/accounts/guides/onboarding/company#country-reqs) (UBO)—`direct` and `indirect`—for the mutations `onboardCompanyAccountHolder` and `createCapitalDepositCase`.