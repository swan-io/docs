...
You can set the shipping method for your users' physical cards. To do so, select the `shippingProvider` option in the `physicalCardCustomOptions` input when you [add a card](https://api-reference.swan.io/mutations/add-cards/), or use the [`printPhysicalCard`](/topics/cards/physical/guide-print#print-one-mutation) mutation.
