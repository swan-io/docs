...
2. **Verification request**: Swan sends a VoP request to the beneficiary's bank (Responding PSP) with the IBAN and account holder name.
...
For bulk credit transfers, each transfer in the batch must include a valid `BeneficiaryVerificationToken` obtained through the `verifyBeneficiary` mutation. If any transfer in a batch is submitted with full beneficiary details instead of a token, the entire batch initiation will be rejected.

Company accounts have the option to opt-out of VoP for bulk transfers through the `updateBulkVerificationOfPayeeSetting` mutation. When opted-out, they can submit bulk transfers without verification tokens. Individual accounts cannot opt-out of VoP for bulk transfers.
    ...
    <div className="card" style={{height: '100%', backgroundColor: '#fffbf0', border: '1px solid #ffeaa7'}}>
      ...
      <div className="card__body">
        ...
        <ul>
          ...
          <li>⚠️ <strong>Important limitation</strong>: If end-users cancel at consent due to a "No Match" result, they'll be redirected to your standard redirect URL without context, potentially causing UX confusion and increased support tickets.</li>
        </ul>
      </div>
    </div>
  </div>
...
1. **Verify beneficiary**: Call the `verifyBeneficiary` mutation and include the `accountId`, IBAN, and account holder name.
...
<Tabs>
  ...
  <TabItem value="close" label="⚠️ Close match">
    **Recommended actions**: 
    - Display suggested name correction
  ...
  <TabItem value="no-match" label="❌ No match">
    **Recommended actions**: 
    ...
    - Option to edit beneficiary details
...
### verifyBeneficiary mutation {#verify-beneficiary}

The `verifyBeneficiary` mutation pre-validates beneficiary details before payment initiation. This mutation requires an `accountId`, IBAN, and account holder name as inputs.

**Returns:**
- Verification result: Match, Close match, No match, or Verification not possible
...
### initiateCreditTransfers mutation {#initiate-credit-transfers}

The `initiateCreditTransfers` mutation has been enhanced to support VoP in two ways:

#### 1. With BeneficiaryVerificationToken
...
#### 2. With SepaBeneficiaryInput (Background VoP)
...
### updateBulkVerificationOfPayeeSetting mutation {#update-bulk-vop}

This mutation allows company accounts to opt-in or opt-out of VoP for bulk credit transfers.

**Input:**
- `accountId`: The company account ID
- `isBulkVerificationOfPayeeEnabled`: Boolean flag to enable/disable VoP for bulk transfers
- `consentRedirectUrl`: URL for consent flow

**Restrictions:**
- Only available for Company account types
- Individual accounts have `isBulkVerificationOfPayeeEnabled` set to `true` and cannot change it
- Requires `canInitiatePayments` permission or account holder status
- No SCA required for opting in

**Account field:**
The `Account` type includes:
- `isBulkVerificationOfPayeeEnabled`: Boolean field indicating VoP status for bulk transfers
- Company accounts default to `false` (opted-out via Terms and Conditions)
- Individual accounts always have this set to `true`