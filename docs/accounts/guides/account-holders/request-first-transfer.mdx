---
title: Request a first transfer
audience:
  - dev
related:
  - /accounts/concepts/account-holders
  - /accounts/concepts/account-holders/first-transfer
  - /accounts/concepts/account/type-and-level
---

import FirstTransferRequest from '../../../_shared/partials/_first-transfer.mdx';

# Request a first transfer {#request-first-transfer}

<FirstTransferRequest />

Swan can either email you or your users to request the first transfer.
Share your preference with your Account Manager.

:::tip Prerequisite
Thoroughly review [first transfer](/accounts/concepts/account-holders/first-transfer) requirements.
:::

:::warning Limited account transfer restrictions
The first transfer must respect the [limited account transfer restrictions](/accounts/concepts/account/type-and-level#limited-account-restrictions): a SEPA Credit Transfer of €50 or less that won't cause the balance to exceed €150. Transfers that break a restriction are automatically rejected.
:::

## Option 1: Swan emails your user {#guide-swan}

1. Swan emails your user, requesting a first transfer.
1. Swan sends users their Swan <Term id="ibans">IBAN</Term> and BIC by email. Swan also informs users about account restrictions: transfers must be €50 or less, and the balance can't exceed €150.
1. Your user sends a first transfer to their Swan account.
1. Swan verifies the transfer source.
1. Swan [updates the account holder verification status](/accounts/concepts/account-holders/first-transfer#first-transfer-waiting) accordingly.

## Option 2: You email your user {#guide-partner}

1. Swan emails you to request a first transfer from your user.
1. You email your user, requesting a first transfer. Make sure to:
    - Include the user's Swan IBAN and BIC in the email.
    - Inform the user that transfers from third-party platforms like Lydia or PayPal aren't accepted.
    - Inform the user that while their account is limited, transfers must be €50 or less, and the balance can't exceed €150.
1. Your user sends a first transfer to their Swan account.
1. Swan verifies the transfer source.
1. Swan [updates the account holder verification status](/accounts/concepts/account-holders/first-transfer#first-transfer-waiting) accordingly.
