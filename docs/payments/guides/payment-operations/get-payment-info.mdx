---
title: Get payment information
audience:
  - dev
related:
  - /payments/guides/payment-operations
  - /payments/concepts/transactions
  - /payments/guides/payment-operations/get-payment-id
---

# Get payment information

You can query information about a single <Term id="payments">payment</Term> and multiple payments **using the API**.

:::tip Prerequisites
You need a **project access token** to run these queries.
:::

## Multiple payments {#guide-multiple}

Get information about multiple payments using the API.
This sample queries the project's `Rejected` payments.

1. Call the `payments` query.
1. Add all the information you'd like to review.
    * The sample query uses several `payments` query options, including filtering for only `Rejected` payments.
1. Add optional messages to the success payload, either for validation or in case of rejection.
    * The sample query adds a more thorough `Rejected` payload.

### Query {#query-multiple}

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {2,9-12} showLineNumbers
query RejectedPayments {
  payments(filters: { status: Rejected }) {
    edges {
      node {
        createdAt
        id
        statusInfo {
          status
          ... on PaymentRejected {
            __typename
            reason
            status
          }
        }
        updatedAt
      }
    }
    totalCount
  }
}
```

import Environments from '../../../_shared/partials/_environments.mdx';

<Environments />

### Payload {#payload-multiple}

Review all `Rejected` payments for your project and the reason for rejection.

```json showLineNumbers
{
  "data": {
    "payments": {
      "edges": [
        {
          "node": {
            "createdAt": "2023-03-30T14:12:18.874Z",
            "id": "$PAYMENT_ID",
            "statusInfo": {
              "status": "Rejected",
              "__typename": "PaymentRejected",
              "reason": "Expired"
            },
            "updatedAt": "2023-03-30T14:12:18.989Z"
          }
        },
        {
          "node": {
            "createdAt": "2023-03-30T14:12:18.874Z",
            "id": "$PAYMENT_ID",
            "statusInfo": {
              "status": "Rejected",
              "__typename": "PaymentRejected",
              "reason": "AccountNotFound"
            },
            "updatedAt": "2023-03-30T14:12:18.989Z"
          }
        },
      ],
      "totalCount": 2
    }
  }
}
```

## Single payment {#guide-single}

Get information about a single payment using the API.

1. Call the `payment` query.
1. Enter the [payment ID](/payments/guides/payment-operations/get-payment-id) for the payment you're querying.
1. Add all the information you'd like to review. The sample query uses several `payment` query options.
1. Add optional messages to the success payload, either for validation or in case of rejection.

### Query {#query-single}

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {2} showLineNumbers
query PaymentInfo {
  payment(id: "$YOUR_PAYMENT_ID") {
    createdAt
    id
    statusInfo {
      status
      ... on PaymentRejected {
        __typename
        reason
        status
      }
    }
    transactions {
      totalCount
      edges {
        node {
          amount {
            currency
            value
          }
        }
      }
    }
    updatedAt
  }
}

```

### Payload {#payload-single}

View all the requested information about the single payment.

```json showLineNumbers
{
  "data": {
    "payment": {
      "createdAt": "2023-05-03T12:34:05.227Z",
      "id": "$PAYMENT_ID",
      "statusInfo": {
        "status": "Initiated"
      },
      "transactions": {
        "totalCount": 1,
        "edges": [
          {
            "node": {
              "amount": {
                "currency": "EUR",
                "value": "250.00"
              }
            }
          }
        ]
      },
      "updatedAt": "2023-05-03T12:34:05.227Z"
    }
  }
}
```
