...
- The [merchant profile status](../index.mdx#profiles-statuses) doesn't change (remains `Enabled`), and the merchant can continue to use their merchant profile with the existing information.
