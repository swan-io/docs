...
If you're using Swan's Web Banking interface, your users can [invite members](https://support.swan.io/hc/en-150/articles/17648698750877-Add-a-member-to-your-account) directly from the app.

| Method | Explanation |
| --- | --- |
| Phone number and email | <ul><li>Phone's SIM card serves as the authentication factor.</li><li>Account member can be assigned all [membership permissions](#permissions).</li><li>You can also invite by email and have Swan collect the phone number when the member [signs up for Swan](../../users/index.mdx#signup).</li></ul> |
| Verified email only | <ul><li>Verified email serves as the authentication factor, and the membership isn't enabled until user verifies their email.</li><li>Account member can only be assigned the `canViewAccount` and `canManageCards` [memberships permissions](#permissions).</li><li>Swan still collects the user's phone number during the [sign-up process](../../users/index.mdx#signup) so they can perform sensitive operations such as initiating payments, ordering cards, and viewing sensitive card information.</li></ul> |
...
| --- | --- |
| `canViewAccount` | View the account and information about the account, including but not limited to: main and virtual [IBANs](../ibans/index.mdx), [trusted beneficiaries](../../payments/credit-transfers/index.mdx#beneficiaries), [payment mandates](../../payments/direct-debit/index.mdx#mandates), [transactions and transaction details](../../payments/index.mdx#transactions), and account members.<br /><br />Cardholders with the `canViewAccount` permission can view their own transaction history, even if they have no other permissions. |
...
Please note that cardholders with the `canViewAccount` permission can view their own transaction history, even if they have no other permissions.
