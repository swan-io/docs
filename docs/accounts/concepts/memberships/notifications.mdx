...
Swan sends email notifications to users when <Term id="account-membership">account memberships</Term> are managed.
...
- **API integration**: When using the API directly, call the `sendAccountMembershipInviteNotification` mutation to send invitation notifications. Learn more in the [guide to send or resend membership invitation notifications](/accounts/guides/memberships/resend-invitation).

:::caution Forked Web Banking migration
If you forked Swan's Web Banking before the migration to partner-branded notifications on Thursday 12 March 2026, you must update your forked Web Banking to continue relying on Swan to send the invitation notification by email.
:::
...
Invitation notifications can be translated into multiple languages automatically.
