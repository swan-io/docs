...
When the Swan API detects <Term id="transactions">transactions</Term> with associated banking fees, the fees are scheduled automatically, except fees related to **payment incidents**.

There are two dedicated [transaction types](/payments/concepts/transactions) for fees:
...
| --- | --- |
| <Yes /> **Sufficient** to cover fee | Swan executes the fee transaction. The status changes to [`Booked`](/payments/concepts/transactions/statuses). |
...
Check your Terms and Conditions for your precise fees.

| Fee | Billed with Value Added Tax (VAT) | Booked |
| --- | :---: | --- |
| Cash withdrawals from cash machines (ATM) in euros | <Yes /> | Immediately |
| Cash withdrawals from cash machines (ATM) *not* in euros | <Yes /> | Immediately |
| Payment for goods and services in stores or remotely *not* denominated and carried out in euros | <Yes /> | Immediately |
...
| Handling fees for management of an unauthorized overdraft | <No /> | 14 days after disclosure of the charges on the account |
| Adding funds to the account by bank card | <Yes /> | Immediately |
| Call to Customer Service | <No /> | n/a |
| <Term id="sdd">SEPA Direct Debit</Term> merchant fees for transactions up to €200 | <Yes /> | Immediately |
...
But the **fees you can charge are strictly regulated**.
...
[Which fees you can and can't charge is regulated](/accounts/concepts/billing/regulated-fees).
Refer to the guide [build a compliant billing offer](/accounts/guides/billing/compliant-billing) to customize and present your offer.

To learn more, discuss billing offers with your Account Manager.
