...
Add SEPA Direct Debit B2B received payment mandates with the API or by sending a link to an account member.
...
Only Company accounts are eligible to set up SEPA Direct Debit B2B mandates.
:::
...
## Add with Dashboard link {#dashboard-link}

Send a link to an account member with the `canInitiatePayments` [membership permission](../../accounts/memberships/index.mdx#permissions) to complete the mandate registration process.
...
1. Locate the **SDD B2B mandate link**.
1. Click to copy the link.
1. Send the link to an account member with the `canInitiatePayments` membership permission.
...
1. They enter their B2B mandate details exactly as shown on the physical mandate.
1. They consent to adding the payment mandate with Strong Customer Authentication.
