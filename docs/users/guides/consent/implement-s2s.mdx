...
[Server-to-server (S2S) consent](/users/concepts/consent/server-to-server) **bypasses direct user intervention**, easing the <Term id="consent">consent</Term> process for bulk actions required by your integrations.
...
1. Retrieve the created consent with the `consents` query. Swan provides the consent `challenge` in the response.

<a href="https://explorer.swan.io?query=cXVlcnkgR2V0Q29uc2VudENoYWxsZW5nZSB7CiAgY29uc2VudHMgewogICAgZWRnZXMgewogICAgICBub2RlIHsKICAgICAgICBpZAogICAgICAgIGNoYWxsZW5nZQogICAgICAgIHB1cnBvc2UKICAgICAgICBzdGF0dXMKICAgICAgfQogICAgfQogIH0KfQo%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql showLineNumbers
query GetConsentChallenge {
  consents {
    edges {
      node {
        id
...
```json {8} title="Payload" showLineNumbers
{
  "data": {
    "consents": {
      "edges": [
...
import ReplacePublicKey from '../../partials/_s2s-replace-key.mdx';
