...
### Providing correct account holder information {#correct-information}

Swan verifies against the exact legal name and details registered in the system. To ensure successful incoming transfers, advise your users to provide senders with their exact account name and IBAN as they appear on their Swan account.
...
- Provide the legal company name, not trade names or merchant names.

**Example**: If a company's legal name is "PIANA SAS" but they operate as "Happy Place," senders must use "PIANA SAS" for VoP requests to match. Similarly, if an account holder's name is "Henri Charles Dupont," senders should use the complete name, not just "Henri Dupont."
