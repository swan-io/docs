...
- The [account membership status](/accounts/concepts/memberships#statuses) **isn't** `ConsentPending` or `Disabled`.
...
- You're using a [user access token](/developers/using-api/authentication/#tokens-user).
:::
