...
All authorization transactions are linked to the same payment, including authorizations, debits, credits, and chargebacks.
...
<mark>Confirm wording with n16/nelson/jan</mark><br /><br />
...
Note that you can set a lower `monthlySpendingLimit` by [updating the card](../../cards/overview/guide-update.mdx), but you can't set any type of additional limit for ATM withdrawals.
:::
...
Limited forever cards allow companies and individual account holders to provide a card with a limited lifetime spending amount.
...
With 3DS, card holders are required to perform [Strong Customer Authentication](../../consent/index.mdx#sca) before finalizing their payment.
...
1. Cardholders always receive a text message, even if they're already on their mobile device and regardless of your [notification preferences](../../consent/index.mdx#notifications).

<mark>confirm with BenG</mark><br /><br />
...
participant CM as Cardholder's mobile device
    ...
    M ->> S: Queries the consent
    S ->> M: Returns confirmed consent
    M ->> CH: Confirms online payment
```
