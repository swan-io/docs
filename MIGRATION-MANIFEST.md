...
**Nested guide sub-hub** (e.g. `guides/onboarding/`, blessed 12 August 2026; template settled 13 August 2026): verb-first lede → `## In this section` nav flowmap (`grid`/`lanes`/`pathway` per §9) → genuinely shared operational content after (cross-cutting notices, link mechanics). Nothing else sits between the lede and the nav. Shape precedents: onboarding, account-holders (lede → nav directly); capital-deposits, memberships (one orienting line after the nav). A concept-pointer paragraph BETWEEN lede and nav is the retrofit target: trimmed on Account operations and Memberships (13 August 2026; the pointer survives in the `related` rail). Five Accounts hubs still carry it — closure, ibans, documents, billing, funding — retrofit during DOC-1879 (correcting the earlier "one outlier" claim: a full sweep found six, not one). The leaf-coverage gate applies to the sub-hub's own leaves, including dual-path trios (each trio's chooser must appear in a lane).
