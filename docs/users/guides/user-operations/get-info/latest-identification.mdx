---
title: Latest identification
audience:
  - dev
counterpart: /users/guides/user-operations/get-info/from-the-dashboard
related:
  - /users/guides/user-operations/get-info
  - /users/guides/user-operations/get-info/from-the-dashboard
  - /users/concepts/identifications
  - /users/concepts/identifications/levels-processes
  - /users/guides/user-operations/get-info/identifications
---

# Get the latest identification

**Call the API** to get [identification levels](/users/concepts/identifications/levels-processes) awarded to your user.

[Identification](/users/concepts/identifications) information is also available on your Dashboard: [look up users from the Dashboard](/users/guides/user-operations/get-info/from-the-dashboard).

:::tip Prerequisites
You need a [**project access token**](/build/using-api/authentication#tokens-project) to call the query.
:::

## Guide {#guide}

1. Call the `user` query.
1. Add the user ID.
1. Add all information you'd like to review.
    - The sample query retrieves all identification levels awarded to the user, as well as information about their last PVID identification.
1. Filter and order the <Term id="identification">identifications</Term>, using [pagination](/build/using-api/pagination) with `first: 1` to get only the most recent PVID result (lines 9-11).

## Query {#query}

<a href="https://explorer.swan.io?query=%3D%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {2,9-11} showLineNumbers
query LatestPvidIdStatus {
  user(id: "$YOUR_USER_ID") {
    identificationLevels {
      PVID
      expert
      QES
    }
    identifications(
      filters: { processes: PVID }
      orderBy: { field: updatedAt, direction: Desc }
      first: 1
    ) {
      edges {
        node {
          id
          process
        }
      }
    }
  }
}
```

## Payload {#payload}

The payload shows that this user is successfully identified using PVID.

```json {5,13-14} showLineNumbers
{
  "data": {
    "user": {
      "identificationLevels": {
        "PVID": true,
        "expert": true,
        "QES": true
      },
      "identifications": {
        "edges": [
          {
            "node": {
              "id": "$YOUR_IDENTIFICATION_ID",
              "process": "PVID"
            }
          }
        ]
      }
    }
  }
}
```
