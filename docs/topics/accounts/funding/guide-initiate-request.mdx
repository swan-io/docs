...
*Please note that the [funding source status](./index.mdx#funding-source-statuses) can be `Enabled` even if the [account verification status](./index.mdx#account-verification-statuses) is `PendingVerification`.*
:::
