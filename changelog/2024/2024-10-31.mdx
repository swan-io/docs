...
⚠️ Starting on **November 28**, we’ll start enforcing [two required fields for ultimate beneficial owners](/topics/onboarding/company/#country-reqs-related-individuals) (UBO)—`direct` and `indirect`—for the mutations `onboardCompanyAccountHolder` and `createCapitalDepositCase`.