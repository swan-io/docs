...
🧪 We added Instant SEPA Credit Transfers and their associated recalls to the [Event Simulator](/dev-tools/tools/event-simulator) on your Dashboard.

💳 You can now approve authorizations with a partial amount.
To do so, indicate the `partialAuthorizationAmountValue` within your [payment control](/dev-tools/using-api/payment-control) response.
