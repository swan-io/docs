...
1. Add one invited account member block for each invitee (lines 7-20).
    - Take note of the syntax: <mark>necessary (ask dev)?</mark>
        - The entire `memberships` object is surounded by `[square brackets]` (lines 6, 48) with no commas.
        - Each invited account member block is surrounded by `{curly brackets}`, again with no commas (lines 7, 20, 21, 33, 34, and 47).
...
1. Send the `consentUrl` to the account holder, legal representative of the account, or account member with `canManageAccountMembership` permissions.
