...
When a Swan KYC analyst determines that a first transfer is needed for verification, they can manually request it from the user. This notification isn't sent for every user—only when Swan identifies it as a necessary verification step.
