...
To understand if a transaction is cancelable, check the `isCancelable` field of the transaction.
Only the full amount can be canceled.

Transactions can only be canceled before they are sent to the SEPA Gateway, respecting the current cutoff time.
The cutoff until which the transaction can be canceled is approximately **10:00 Central European [Summer] Time (CET/CEST)**.

After this cutoff time, the transaction cannot be canceled and will proceed to settlement.
