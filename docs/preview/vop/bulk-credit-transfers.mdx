...
If you haven't implemented `verifyBeneficiary` and companies want to opt in to mandatory VoP, warn them this will add verification steps to their transfer flow.
:::
...
You must implement [the `updateAccountSettings` mutation](#updating-settings). If not implemented, end-users must contact support@swan.io, creating friction and delays.
:::
