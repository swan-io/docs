...
Follow the [migration guide](./overview/guide-migrate.mdx) for step-by-step instructions.
:::
...
In Live, [individual onboarding](./individual/index.mdx) is only available for projects Swan has approved.
...
**Configuration**: This notification is always sent by Swan. For compliance reasons, it can't be deactivated or edited. The recipient is the email address provided by the account's legal representative during the onboarding flow, and the language used is the one selected during the onboarding flow. You can view its configuration in your **Dashboard** under **Settings** > **Notifications**. 
...
* [Migrate to the current onboarding API](./overview/guide-migrate.mdx)
...
* [Individual onboarding field reference](./individual/fields.mdx)
* [Company onboarding field reference](./company/fields.mdx)