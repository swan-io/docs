...
For full product documentation, see the [SEPA Direct Debit overview](/topics/payments/direct-debit/) and [add a payment mandate](/topics/payments/direct-debit/guide-add-mandate/) guides.

For user-facing explanations, share [SEPA Direct Debit mandates](https://support.swan.io/hc/en-150/articles/19637943123101) and [Direct debit overview](https://support.swan.io/hc/en-150/articles/24510545177501).
...
- For B2B mandates, direct your user to register and consent through your product. Core mandates require no action from you or your user.
:::
...
:::info Swan checks on pending consents

Swan monitors daily mandates that are `ConsentPending` and proactively reaches out to you and your users to obtain their confirmation and enable the mandates.
...
| --- | --- | --- |
| About a rejected payment. | Mandate not consented, insufficient funds, account closed, or details don't match. | [Troubleshoot rejected direct debits](#troubleshoot-rejected-direct-debits) |
| How to register a new mandate. | They received B2B mandate details from a creditor. | [Register a new B2B mandate](#register-a-new-b2b-mandate) |
| About an updated mandate that isn't working. | Consent cycle was reset after an update. | [Update a B2B received mandate](#update-a-b2b-received-mandate) |
| How to stop a direct debit. | They want to pause or permanently stop future debits. | [Stop or cancel a direct debit](#stop-or-cancel-a-direct-debit) |
| How to get a refund. | They were debited and want the money back. | [Understand refund rights](#understand-refund-rights) |
| About a core mandate document they sent. | Core mandates are created automatically. | No action needed. |
...
1. Review mandate details: creditor name, SCI / ICS, UMR / RUM, debtor IBAN, sequence type, status, signature date, and last execution date.
...
Your user should register B2B mandates using the registration form link you send them whenever possible. The person completing it must be authorized to manage payments on the account, usually the legal representative.

To send a B2B mandate registration link from your Dashboard or add a mandate with the API, see the guide to [add a received direct debit mandate](/topics/payments/direct-debit/guide-add-mandate/). The guide includes Dashboard steps with screenshots as well as the full API mutation.

### Help your user register the mandate

1. Ask if they can access the registration link.
1. If yes, point them to it and ask them to complete the registration.
1. In the event a mandate registration fails, raise a [Swan Support request](https://partner-support.swan.io/hc/en-gb/requests/new) with the required mandate information.
...
## Replace a B2B received mandate

Don't update an existing B2B received mandate. Instead, cancel it and register a new mandate with the correct information.

- If your product supports B2B mandate management, cancel the existing mandate through your integration. Then ask your user to register a new mandate with the correct details and complete the consent process.
- If your product doesn't support mandate management, raise a [Swan Support request](https://partner-support.swan.io/hc/en-gb/requests/new) to cancel the existing mandate. Include the mandate ID. Once Swan confirms the cancellation, ask your user to register a new mandate with the correct details.

:::caution New consent required

The new mandate can't be used until your user completes the consent process and its status is `Enabled`.

If a direct debit is due soon, warn your user to register and approve the new mandate promptly. Any direct debit received after the existing mandate is cancelled but before the new mandate is enabled might be rejected.
...
:::info No cancel action available

There isn't a public API action to permanently cancel a mandate. The `Canceled` status exists, but you can't set this on your own. A mandate can become `Canceled` through Swan-side processes, such as automatic cancellation of `OneOff` mandates after they're used. Suspending a mandate is effectively permanent unless someone re-enables it.
...
1. If your development team has integrated the [suspend mandate](/topics/payments/direct-debit/guide-suspend-mandate/) feature, direct your user to suspend the mandate from your product. To resume debits later, they can [re-enable the mandate](/topics/payments/direct-debit/guide-enable-mandate/).
1. If your integration doesn't support this yet, raise a [Swan Support request](https://partner-support.swan.io/hc/en-gb/requests/new) with the mandate ID.

:::tip Add this to your product

Your development team can integrate the [suspend](/topics/payments/direct-debit/guide-suspend-mandate/) and [enable](/topics/payments/direct-debit/guide-enable-mandate/) mutations so your users can manage mandates directly from your product.
...
Swan checks all pending direct debit instructions at **6 AM (CET/CEST)** on the day the payment is due, following the [full execution logic](/topics/payments/direct-debit/#received-instructions-execution). At that point, Swan checks that:
...
If your user consents to a mandate after 6 AM (CET/CEST) on the due date, Swan won't process it until the next scheduled payment. Let your users know about this timing so they understand why their mandate might still show as pending.
...
Your user's refund rights depend on their mandate scheme. For full details, including timelines for returns and creditor reversals, see the [SEPA Direct Debit specifications](/topics/payments/direct-debit/#specifications).
...
- **B2B mandates**
  - **No refund rights.** Under the SEPA B2B rulebook, debtors have no right to a refund for an authorized transaction. Authorized B2B direct debits don't have a debtor refund right under the B2B scheme. If your user believes a debit was unauthorized, direct them to contact the creditor.
...
- **Within 8 weeks of the direct debit:** Your user can request a refund without giving a reason.
- **After 8 weeks and up to 13 months:**: a refund is only possible if the transaction was unauthorized or there was no valid mandate. The request requires additional checks and evidence. The receiving/creditor bank may accept or reject the claim.
...
- The transaction ID. 
- Evidence that the mandate was invalid or the debit was unauthorized, if available.
- Correspondence with the creditor or another document supporting the claim, if available.
...
Submit a request via the [Partner support request form](https://partner-support.swan.io/hc/en-gb/requests/new) including all details from the [Contact Swan Support](#contact-swan-support) section.
...
For refund requests, also include the transaction ID. 
