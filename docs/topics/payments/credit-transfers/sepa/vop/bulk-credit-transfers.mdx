...
Subscribe to the `AccountSetting.Updated` [webhook](/dev-tools/using-api/webhooks) to receive notifications when the `allowBulkCreditTransfersWithoutBeneficiaryVerification` setting changes.
