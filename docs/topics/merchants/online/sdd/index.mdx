...
To understand if a payment is cancelable, check the `availableToCancel` balance of the payment.
Only the full amount can be canceled.

Payments can only be canceled before transactions are sent to the SEPA Gateway, respecting the current cutoff times:
...
After these cutoff times, the payment cannot be canceled and will proceed to settlement.
