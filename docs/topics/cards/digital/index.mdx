...
![Add to Apple Wallet badges](../../../images/topics/cards/cards-digital-wallets.png)
...
## Tracking digital card changes {#tracking-status}
To receive notifications when a digital card is created or its status changes, subscribe to `DigitalCard.Created` and `DigitalCard.Updated` [webhooks](../../../developers/using-api/webhooks).

`DigitalCard.Created` is triggered when the card is first created and before it becomes usable in a wallet. You should also subscribe to `DigitalCard.Updated` to track status changes.
The resource ID in the event is a `digitalCardId` that can be used with the [query `digitalCard(digitalCardId: ...)`](https://api-reference.swan.io/queries/digital-card) to retrieve the latest digital card data.

## Query: Get digital card information {#query}

Retrieve digital card information.

<a href="https://explorer.swan.io?
query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>
...
query getDigitalCard {
  digitalCard(digitalCardId: "$YOUR_DIGITAL_CARD_ID") {
    ...
    ... on PendingDigitalCard {
      statusInfo { status }   # ConsentPending | Pending | Declined
    }
  }
}
```
...
- [Add digital cards to Apple Wallet and Google Wallet](./guide-add.mdx)
