...
View a <Term id="cards-virtual">virtual card</Term>'s numbers, expiration date, and CVC, protected by the cardholder's consent.
...
1. Then a `consentUrl` is returned which invites the user to start the Strong Customer Authentication with Swan. You need to open a native pop-up to display the `consentUrl`. This URL redirects the user to a page with the card numbers displayed with the card design.
