---
title: "Using the API"
audience: [dev]
counterpart: /accounts/guides/memberships/export/from-the-dashboard
related:
  - /accounts/guides/memberships/export
  - /accounts/guides/memberships/export/from-the-dashboard
  - /accounts/concepts/memberships
---

<span className="ia-audience-tag ia-audience-tag--dev">Developer</span>

**Also available:** [From the Dashboard](/accounts/guides/memberships/export/from-the-dashboard)

# Export account membership data using the API

Export account membership data with the API to review member roles and permissions for each account programmatically.

1. Call the `exportAccountMembershipData` mutation.
1. Add your `email`. A link to download the `.csv` export is sent to the email address you provide.
1. If required, add filters to your export (lines 5-11).
1. Add the success payload and rejections.

## Mutation {#api-mutation}

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {4,5-11,17} showLineNumbers
mutation ExportAccountMemberships {
  exportAccountMembershipData(
    input: {
      email: "$YOUR_EMAIL_ADDRESS"
      filters: {
        status: Enabled
        canInitiatePayments: true
        canManageAccountMembership: false
        canManageBeneficiaries: false
        canManageCards: true
        canViewAccount: true
      }
    }
  ) {
    ... on ExportDataSuccessPayload {
      __typename
      exportId
    }
    ... on MaximumSimultaneousExportsRejection {
      __typename
      message
    }
    ... on MaximumDailyExportsReachedRejection {
      __typename
      message
    }
  }
}
```
### Payload {#api-payload}

The payload returns the export ID.

```json {5} showLineNumbers
{
  "data": {
    "exportAccountMembershipData": {
      "__typename": "ExportDataSuccessPayload",
      "exportId": "$EXPORT_ID"
    }
  }
}
```

## Related

- [Export memberships overview](/accounts/guides/memberships/export)
- [From the Dashboard](/accounts/guides/memberships/export/from-the-dashboard)
- [Account memberships](/accounts/concepts/memberships)
