...
🛒 The following improvements have been made to [merchant profiles and merchant payment methods](/topics/merchants/), and the `requestMerchantProfile` and `requestMerchantProfileUpdate` mutations:
...
- The `expectedMonthlyPaymentVolume`, `merchantWebsite`, `socialNetwork`, `supportWebsite`, and `termsAndConditions` fields are being deprecated and replaced with the new fields: `expectedMonthlyMerchantProcessingVolume`, `merchantWebsiteUrl`, `socialNetworkUrl`, `supportWebsiteUrl`, and `termsAndConditionsUrl`.
