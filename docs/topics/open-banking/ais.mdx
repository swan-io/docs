...
:::note Consent covers all accounts
The consent flow requires users to give access to all accounts at once. Users cannot choose specific accounts to include.
:::
...
For complex cases involving multi-project users, submit a [Partner support request form](https://partner-support.swan.io/hc/en-gb/requests/new).
...
| `creditor.currency` | `creditor_details.account.currency` | Optional. Only served when the user is on the credit side. |
...
| `debtor.IBAN` | `debtor_details.account.iban` | Optional. Always served when the transaction is a SEPA Credit Transfer or the user is on the debit side. |
| `debtor.currency` | `debtor_details.account.currency` | Optional. Only served when the user is on the debit side. |
