...
1. Confirms their mobile number, after which they **continue the sign-up process** on their mobile device. How they confirm this step depends on whether they opened the sign-up link from a computer or their mobile device:
...
- If you use Swan's **Web Banking interface**, Swan redirects your user to the interface automatically.
- If you're using a **custom API integration** or a customization of Swan's **open source frontend**, your user is redirected to the `redirectUrl` you supplied when creating their sign-up link.
