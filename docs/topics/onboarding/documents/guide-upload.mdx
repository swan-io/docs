...
:::tip Prerequisites
Before you can upload a document, the [collection status](./index.mdx#collection-statuses) must be `Waiting for document`.
:::
...
_Replace all `$VARIABLES` with your values from step 2._

:::tip Upload successful
When you upload a document with the HTTP POST cURL command, Amazon sends a _204: No content error_, **but the document was successfully uploaded**.
:::
