...
Initiate accepting <Term id="payments">payments</Term> with French checks.
...
1. Call `initiateCheckMerchantPayment`, completing all mandatory fields: `amount`, `value`, `currency`, `CMC7`, `rlmcKey`, and `merchantProfileId` (lines 2-7).
1. Optionally, add a `label` and `reference` (not shown). Both appear in the transaction history and to the debtor.
1. Add the `InitiateCheckMerchantPaymentSuccessPayload` (line 10).
1. Add `checkMerchantPayment` to the success payload (line 12).
1. Add the `fnciInfo` to the success payload (lines 15-21). Refer to the explanatory content to [learn about FNCI and their verification results](/payments/concepts/merchants/fnci).
...
At this stage, the <Term id="transactions">transaction</Term> has been initiated and the following events occur:
...
1. **Swan's check provider**:
    1. [Processes the check](/payments/concepts/merchants/french-checks#rolling-reserve), after which the status changes to `Booked`. This can take a few business days.

After a check is `Booked`, the [rolling reserve](/payments/concepts/merchants/french-checks#settlement) window starts.
