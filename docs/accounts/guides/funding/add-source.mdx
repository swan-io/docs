...
<p className="ia-lede">Add a SEPA Direct Debit B2B funding source with the API so funds can be pulled into a Swan account.</p>

:::tip Prerequisites
1. The user adding the funding source is either an account member with the `canManageAccountMembership` and `canViewAccount` [permissions](/accounts/reference/membership-permissions#permissions), or the account holder.
1. The funding source is a non-Swan account that is accessible to the Swan account holder or eligible account member.
1. Call the mutation with a [user access token](/dev-tools/using-api/authentication#get-token-user) associated with the account holder or eligible account member. They need to sign the direct debit mandate by consenting to the [sensitive operation](/topics/users/consent/#sensitive) of adding a funding source.
:::
...
- [Initiate a funding request](/accounts/guides/funding/initiate-request) · [Get funding information](/accounts/guides/funding/get-info) · [Cancel a funding source](/accounts/guides/funding/cancel-source)
