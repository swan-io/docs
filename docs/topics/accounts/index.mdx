...
Each month, Swan **generates account statements** automatically for **all Swan accounts** and use the Central European [Summer] Time (CET/CEST) timezone.
...
They're also available from your Dashboard and, if you're using it, Swan's Web Banking interface.
...
You can generate statements for a period of up to **three months**.
If you generate two statements with identical parameters (language, hour, second, and timezone), Swan only generates one statement.
For the second statement, Swan provides the link to the first statement generated.
If the language parameter changed, however, a new statement is generated in that language.
