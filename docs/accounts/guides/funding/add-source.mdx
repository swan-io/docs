...
1. Call the mutation with a [user access token](/build/using-api/authentication#get-token-user) associated with the account holder or eligible account member. They need to sign the direct debit mandate by consenting to the [sensitive operation](/topics/users/consent/#sensitive) of adding a funding source.
:::
