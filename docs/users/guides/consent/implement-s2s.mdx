...
[Server-to-server (S2S) consent](/users/concepts/consent#s2s) **bypasses direct user intervention**, easing the consent process for bulk actions required by your integrations.

## Step 1: Generate your keys {#generate}

Use your browser's cryptography API to generate your key pair according to the [required algorithm](/users/concepts/consent#s2s-crypto).
...
After you save your public key, Swan sends a text message to your [project's legal representative](/users/concepts/consent#s2s-projects) to consent to adding S2S consent, which includes the first and last name of person installing the key and the key to review.
:::
...
:::tip Impersonation
If the user access token attributed to your [project's legal representative](/users/concepts/consent#s2s-projects) expires and interrupts your process, consider [impersonating the legal representative with a project access token](/build/using-api/authentication#impersonate) instead.
:::
...
1. Request any [sensitive operation](/users/concepts/consent#sensitive) that requires consent.
