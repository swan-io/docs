...
### Specify invoice reference  {#invoice-reference}

You can now add a structured invoice reference to a SEPA Credit Transfer using the new, optional `labelType` field.

:::info Optional invoice reference label
- When using the optional `labelType` field, the reference value is provided in the `label` field. 
- When retrieving transaction data, the structured reference value can be found in the `transaction.label` field, with its type identified by the `labelType` field.
- This feature currently supports the Belgian OGM VCS reference, a standard 12-digit reference number used in Belgium for payment reconciliation. Select the `OgmVcs` in the `labelType` field for Belgian OGM VCS.
:::

1. Call the `initiateCreditTransfers` mutation.
1. Add all required information, marked by an (*) asterisk in API Explorer, including and information such as your redirect URL (line 4), your Swan `accountId` (line 5), and your transfer amount (line 7).
1. For this use case, add information about your `sepaBeneficiary` (line 8), including their name and IBAN (lines 9-10).
1. Add `labelType`, choose an option (lines 13-14), and add the reference value in the `label` input.
1. Add the `InitiateCreditTransfersSuccessPayload` payload (line 18).
...
```graphql {4,5,7,9-10,13-14,18,22} showLineNumbers
