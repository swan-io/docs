...
|:---:|---|
| `PendingVerification` | The funding source was added, and account verification for the funding source is in progress.<br /><br />Note that the funding source status can be `Enabled` even if the account verification status is `PendingVerification`. |
