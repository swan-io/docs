...
:::info 
You can't choose a custom PIN for a cardholder who hasn't [completed identification](../../users/identifications/index.mdx) or whose [account membership status](/accounts/concepts/memberships#binding-errors) is `BindingUserError`.
:::
...
1. Issued in the Netherlands ([account country](/accounts/concepts/account#account-country) is the Netherlands).
...
Between these two checkpoints, you have a 2-week window to [update the delivery address](/topics/cards/physical/guide-renew).
...
:::tip
Subscribe to the `Card.Updated` [webhook](/dev-tools/using-api/webhooks#events-cards) to get notified when the card moves to `ToRenew`, giving you time to verify the delivery address with your cardholder.
:::
