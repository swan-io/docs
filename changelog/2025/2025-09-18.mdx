...
📡 Starting **October 9**, bulk SEPA Credit Transfers from individual account holders will be rejected if [verification tokens](/topics/payments/credit-transfers/sepa/vop/guide-verify-beneficiary/#step-3) aren't included in the payment initiation. You must either implement the `verifyBeneficiary` mutation (recommended) or restrict bulk credit transfer access to company accounts only. This change applies to both Live and Sandbox environments.
