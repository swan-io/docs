...
3. The cardholder **consents** to the operation.
    - After receiving consent, the [physical card status](/cards/concepts/physical/statuses) returns from `Suspended` to its previous status, `ToActivate`. If the card is in the [renewal process](/cards/concepts/physical/renewal#renew-statuses), the status returns to `Renewed`.
