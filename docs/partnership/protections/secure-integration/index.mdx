...
- Retrieve relevant details through the authenticated Swan API or Partner Dashboard when needed.

Don't treat webhook notifications as the sole source of sensitive information. Use the authenticated API or Partner Dashboard to retrieve sensitive details.
...
- Use only exact, pre-approved redirect URIs configured in the Swan Partner Dashboard.
...
Swan handles multi-factor authentication for account operations through its [consent](/topics/users/consent/#sensitive) mechanisms. You need to implement your own authentication controls for sensitive operations handled through your own systems and applications.
...
Multi-factor authentication isn't required for cardholder-only access when the only sensitive data displayed is the card number or physical card PIN. These values are protected by Swan's consent mechanisms. The five-minute idle timeout still applies to card operations.
...
The five-minute requirement applies to sensitive interfaces you operate. Swan's [Time-to-live](/developers/#ttl) documentation describes the session behavior of Swan's own Partner Dashboard and Web Banking user interface.
