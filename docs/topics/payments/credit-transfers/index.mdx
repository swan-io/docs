...
Beneficiaries *not added* as trusted are **unsaved** beneficiaries.
...
An account member with the `canManageBeneficiaries` permission must consent to adding a trusted beneficiary with [Strong Customer Authentication (SCA)](../../users/consent/index.mdx#sca).

### Membership permissions {#beneficiaries-permissions}

How users interact with both trusted and unsaved beneficiaries depends on their [account membership permissions](./memberships/index.mdx#permissions).
...
| `canInitiatePayments`<br />*without* `canManageBeneficiaries` | Initiate transfers to trusted beneficiaries only. |
...
- [Add a SEPA trusted beneficiary](./sepa/guide-beneficiary-add.mdx)
- [Add an international trusted beneficiary](./international/guide-beneficiary-add.mdx)