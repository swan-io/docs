...
At this time, only one notification is available and can only be managed through Swan.
When the full feature is developed, you'll have access to multiple notifications managed from your Dashboard. 
:::

The following table lists information about available notifications.
Take note of the [account membership permission](../accounts/memberships/index.mdx#permissions) required for an account member to receive each notification.
