...
sequenceDiagram
    ...
    S->>S: Account lookup and matching
...
- **Company <Term id="ibans">IBAN</Term>**: `AccountHolder.registration.taxIdentificationNumber`.
...
1. Legal name (`AccountHolderInfo.name`)
...
1. Individual name (`AccountHolderInfo.name`)
...
**Example**: If a company's legal name is "MY COMPANY SAS" but they operate as "Awesome Company", senders can use either name. Swan's matching algorithm checks both the legal name and trade name to improve verification success. Similarly, if an <Term id="account-holder">account holder</Term>'s name is "Henri Charles Dupont," senders should use the complete name, not just "Henri Dupont."

### Updating trade names for VoP matching {#updating-trade-names}

Swan's matching algorithm checks both the legal company name and the trade name. If an account holder's trade name is missing or incorrect, incoming VoP requests might not match successfully when senders use the trade name.

To add or update a trade name, [contact Swan Support](https://support.swan.io/hc/en-150/requests/new):

- **Single account holder**: Submit one ticket with the account holder ID and the trade name to add or update.
- **Multiple account holders**: Submit one consolidated ticket with all account holder IDs and their corresponding trade names.

:::info KYC review required
Trade name updates require a KYC review by our compliance team. Allow time for processing when planning updates.
:::

**Example**: If your client operates as "Tech Solutions Pro" but their legal name is "Tech Solutions Pro SARL", VoP requests using "Tech Solutions Pro" will only match successfully if the trade name is registered. Without a registered trade name, VoP requests will match against "Tech Solutions Pro SARL" only, which could result in a close match or no match depending on what the sender provides.
...
| **Close match** | Similar but not identical (70-99% similarity for names) | <span style={{whiteSpace: 'nowrap'}}>Name: Levenshtein distance</span> |
...
| **Impossible to match** | Account not found, unsupported ID type, or account with `Closed` or `Suspended` status | Not applicable |
...
→ Understand the complete VoP ecosystem with [Verification of Payee overview](/payments/concepts/verification-of-payee)