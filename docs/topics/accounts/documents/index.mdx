...
Documents might be required to support [account holder verification](/topics/onboarding/account-holders/#verification-process), [verification renewal (Re-KYC)](/topics/onboarding/account-holders/#verification-renewal), or to prove the validity of a transaction.
...
:::info `purposeDetails` field
`purposeDetails` is an optional field available on the `SupportingDocumentPurpose` type (returned within `requiredSupportingDocumentPurposes`).
When the requested purpose is `Other`, `purposeDetails` contains free-text details that specify what supporting document is required (for example, “Attorney signed ownership declaration”). 
For standard purposes, `purposeDetails` is typically `null`.
:::
