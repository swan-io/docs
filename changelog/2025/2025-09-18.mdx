...
🛒 Improvements to [merchant profiles and merchant payment methods](/payments/concepts/merchants/), and the `requestMerchantProfile` and `requestMerchantProfileUpdate` mutations:
...
📡 Starting **October 9**, bulk SEPA Credit Transfers from individual account holders will be rejected if [verification tokens](/payments/guides/credit-transfers/sepa/verify-beneficiary/#step-3) aren't included in the payment initiation. You must either implement the `verifyBeneficiary` mutation (recommended) or restrict bulk credit transfer access to company accounts only. This change applies to both Live and Sandbox environments.
...
🧭 Starting **October 21**, card-issuing spending limits will be [aggregated at the hour level](/cards/reference/spending-limits#hour-level-aggregation). 
