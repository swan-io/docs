...
## Test bulk verification {#test-bulk}

Test multiple beneficiaries with different results:

```graphql showLineNumbers
mutation TestBulkVerification {
  verifySepaBeneficiary(
    input: {
      accountId: "$YOUR_SANDBOX_ACCOUNT_ID"
      payees: [
        {
          iban: "FR7614508000300060632710243"
          name: "Perfect Match"
          externalReference: "bulk-match"
        }
        {
          iban: "FR7630006000011234567890189"
          name: "Jon Smith"
          externalReference: "bulk-close-match"
        }
        {
          iban: "DE89370400440532013000"
          name: "Wrong Person"
          externalReference: "bulk-no-match"
        }
        {
          iban: "ES9121000418450200051332"
          name: "Failed Verification"
          externalReference: "bulk-not-possible"
        }
      ]
    }
  ) {
...
## Test VoP settings {#test-settings}

Test company VoP opt-out functionality (requires Company account):

### Test disabling VoP {#test-disable}

```graphql showLineNumbers
mutation TestDisableVoP {
...
### Test bulk transfer without VoP {#test-bulk-no-vop}

After disabling VoP, test bulk transfers without tokens:

```graphql showLineNumbers
mutation TestBulkWithoutVoP {
  initiateCreditTransfers(
    input: {
      accountId: "$YOUR_COMPANY_ACCOUNT_ID"
      consentRedirectUrl: "$YOUR_REDIRECT_URL"
...
### Test bulk without tokens or opt-out {#test-bulk-error}

Test the error when bulk transfers lack tokens and VoP is enabled:

```graphql showLineNumbers
mutation TestBulkError {
  initiateCreditTransfers(
    input: {
      accountId: "$YOUR_SANDBOX_ACCOUNT_ID"
      consentRedirectUrl: "$YOUR_REDIRECT_URL"
...
1. **Test bulk verification** - Verify multiple beneficiaries in one call
...
1. **Test company settings** - Enable/disable VoP for bulk transfers (Company accounts only)
...
- [Manage VoP settings](./guide-manage-settings.mdx) - Company opt-out functionality
