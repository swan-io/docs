...
Swan accounts are defined by their [`PaymentLevel`](https://api-reference.swan.io/enums/payment-level) and [`PaymentAccountType`](https://api-reference.swan.io/enums/payment-account-type/).
There are two possible values for each.

The `PaymentLevel` is determined by the [account holder verification status](../onboarding/account-holders/index.mdx#verification-process#statuses).
...
The `PaymentAccountType` signifies whether the [account's main IBAN](./ibans/index.mdx#main) has been assigned.

- `EMoney` → The account doesn't have it's main IBAN.
- `PaymentService` → The account's main IBAN has been assigned.

Swan accounts have different capabilities determined by the combination of these values.

| `Level` + `Type` | Capabilities |
| --- | --- |
| `Limited` + `EMoney` | ✓ The account can be used for certain actions, such as inviting account members and adding virtual cards.<br /><br />☒ The account doesn't have its main IBAN.<br />☒ Payments **can't** be made from the account.<br />☒ Account holder verification isn't complete. |
| `Limited` + `PaymentService` | ✓ The account has its main IBAN.<br />✓ Money can be added to the account.<br /><br />☒ Account holder verification isn't complete.<br />☒ Payments **still can't** be made from the account. |
| `Unlimited` + `PaymentService` | ✓ Account holder verification status is `Verified`.<br />✓ Payments **can** be made from the account.<br />✓ The account has its main IBAN.<br />✓ All accounts functions are possible. |
