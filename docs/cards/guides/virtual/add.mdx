...
When configuring the `spendingLimit` field, you can choose between [rolling and calendar periods](/cards/reference/spending-limits#limits-periods).
...
If you want to issue single-use virtual cards without having direct human interaction to consent and reveal card numbers, you must be the account holder (not available for end-users) and have [server-to-server consent](/topics/users/consent/guide-implement-s2s) implemented.
