...
🔢 [Pagination](/dev-tools/using-api/pagination) on the `accountHolders`, `accountMemberships`, and `accounts` queries has been fixed. It might break if you implement the filter on the front-end. These queries now only return the requested number of items, rather than all items at once.
