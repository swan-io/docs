...
This formal authorization is a **received <Term id="payment-mandates">payment mandate</Term>**.
...
Creating and authorizing mandates happens off of the SEPA network, and the [mandates must be declared](/payments/concepts/merchants/sepa-direct-debit#mandates-declare) to the SEPA network with two unique identifiers:
...
After direct debits are instructed, other transactions, such as <Term id="r-transaction">R-transactions</Term>, can be linked to the direct debit transaction. An R-transaction never changes the original transaction's status: a `Booked` direct debit stays `Booked`, and the return, refund, or reversal is a new transaction that references it.
These only concern **outgoing** SEPA Direct Debits.
