...
Card products are associated with a specific package level: Standard, Essential and Premium. You can retrieve the package level for a specific card using the card query.

## Prerequisites

## Main IBAN {#guide-main}

1. Call the `RetrieveCardPackage` query.
1. Enter the `cardId`.
1. Add `cardPackage` to receive the card package level.
