...
By default, Maestro secondary card application is embedded in the physical card's chip as a fallback option when the qualified card is bring printed.
...
Transactions processed with Maestro include the masked Maestro card number in transaction details and receipts.
...
1. Printed from the [French printing hub](#hubs-france).
1. [Printed](./guide-print.mdx) with one of the following API mutations: `printPhysicalCard`, `addCards`, or `addCardsWithGroupDelivery`.
