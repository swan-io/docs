...
Send the account's legal representative a Dashboard link so they can complete account closure themselves.
...
Consider subscribing to the `Account.Closing` and `Account.Closed` [webhooks](/dev-tools/using-api/webhooks#events-accounts) to be notified when the [account status](/accounts/concepts/account#statuses) changes.
:::
...
1. Call the `closeAccount` mutation with the legal representative's [user access token](/dev-tools/using-api/authentication#tokens-user).
...
- [Account closure](/accounts/concepts/closure#closure-legal-rep) — the legal representative closure process.
