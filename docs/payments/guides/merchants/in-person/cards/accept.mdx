---
title: Accept in-person card payments
audience:
  - dev
related:
  - /payments/concepts/merchants
---

# Accept in-person card payments

:::info Merchants only
This section only applies to **<Term id="merchants">merchants</Term>** accepting card payments.
Visit the [card payments section](/payments/concepts/cards) for information about payments made with cards from a Swan account.
:::

## Step 1: Set up the development environment {#step-1}

Install the Stripe Terminal SDK and configure your app with the required Apple entitlements and permissions.

→ [Set up the development environment](./setup.mdx)

## Step 2: Request a merchant profile {#step-2}

Before accepting in-person card payments, you need a <Term id="merchant-profile">merchant profile</Term>.

→ [Request a merchant profile](../../profiles/request.mdx)

## Step 3: Request the payment method {#step-3}

Request the in-person card payment method to enable your merchants to accept card payments.

→ [Request the in-person card payment method](./request-method.mdx)

## Step 4: Initialize the device {#step-4}

Set up the connection token provider and connect your device to the Stripe Terminal reader.

→ [Initialize the device for payments](./initialize.mdx)

## Step 5: Create in-person card payments {#step-5}

Call the API to create payment intents and use the Stripe SDK to process payments.

→ [Create in-person card payments](./create-payments.mdx)

## Step 6: Issue a refund {#step-6}

import InPersonCardRefundLimitation from '../partials/_in-person-card-refund-limitation.mdx';

<InPersonCardRefundLimitation />

## Step 7: Test in the Sandbox {#sandbox}

Use Swan's Testing API to simulate payment authorization, rejection, and settlement in a Sandbox environment.

→ [Sandbox](/payments/guides/sandbox/merchant-in-person-cards)