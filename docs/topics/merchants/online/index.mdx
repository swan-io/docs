...
| `Authorized` | *Applies only to card payments.* The payment has been authorized by the customer's bank. The funds are guaranteed but not yet captured. This can be a final status unless the customer disputes the payment or requests a refund. |
| `Canceled` | The payment was canceled before capture. Funds can no longer be captured. This is a final status. |
| `Rejected` | The payment was rejected by Swan or another participant in the payment scheme before capture. This is a final status. |
| `Captured` | The authorized payment has been captured—funds have moved out of the customer's account. This can be a final status unless the customer disputes the payment or requests a refund. |
| `Refunded` | The payment was reversed by the merchant. Can be partial, depending on the payment method. This is a final status. |
