...
Limited accounts can only receive incoming SEPA Credit Transfers of €50 or less, and the total balance can't exceed €150.
Any transfer that exceeds these limits is automatically rejected.
...
These restrictions primarily affect first transfers during the account holder verification process when accounts remain in `Limited` status.
:::
