...
1. Follow the [guide to get a user access token](../../authentication/guide-get-token-user.mdx#auth-code).
...
To start an Expert identity verification, follow the [guide to get a user access token](../../authentication/guide-get-token-user.mdx#auth-code).
...
To start a QES identity verification, follow the [guide to get a user access token](../../authentication/guide-get-token-user.mdx#auth-code).
...
To start a PVID identity verification, follow the [guide to get a user access token](../../authentication/guide-get-token-user.mdx#auth-code).
...
| `ReadyToSign` | The user's identity was verified using the QES process, and the user now needs to sign to finalize the process.<br /><br />**Next steps**:<ul><li>If the user signs successfully, the status changes to `ValidIdentity`</li><li>If the user's signature isn't valid or isn't submitted, the status changes to the corresponding `RejectionReasonCode`</li></ul> |
