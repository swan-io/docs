...
By default, this happens three years after printing the card.
...
If your user prefers to [choose their PIN](#pin-choose), they need to cancel the existing card and order a new one.
It's not possible to choose a new PIN during renewal.

Existing subscriptions might be transferred to the new card automatically if the merchant adheres to this service.
Swan has no control over this process.

### Card delivery {#renew-delivery}

New physical cards are shipped on the eleventh of the month before the card expires.
For example, if the card expires on the **last day of March**, the new card ships on **February 11** of the same year.
...
**10 weeks before** the original card expires, **verify the delivery address** for the new card with your card holder.

| Interface | Address update method |
| --- | --- |
| Swan's Web Banking app | Card holders confirm or update their address in the app at least 8 weeks before renewal. |
| Custom integration | Send the updated address to Swan **at least 8 weeks before** the card renews.<br /><br /><ol><li>First, provide a way for your card holders to verify their address. For example, this could be from your app, with a notification system, or through your own support team.</li><li>Then, send Swan the updated address with the API by calling the `confirmPhysicalCardRenewal` mutation.</li></ol> |
...
:::tip Activating new card
The new card's status changes from `Renewed` to `Activated` after the card holder makes a transaction with the new card.
Then, the old card's status changes to `Canceled` because only one card can be activated at a time.
:::
