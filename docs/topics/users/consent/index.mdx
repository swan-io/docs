...
You can either use a [user access token](/dev-tools/using-api/authentication#tokens-user) in the name of the user wanting to make the payment, or with a [project access token](/dev-tools/using-api/authentication#tokens-project) impersonating that user.
...
Since execution is asynchronous, consider subscribing to [consent webhooks](/dev-tools/using-api/webhooks#events-consent) to stay updated on the progress of each child consent.
...
To update a child consent, you must update the parent [multi-consent](/topics/users/consent/#multi-consent). 
:::
