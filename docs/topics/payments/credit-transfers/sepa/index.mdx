...
## Verification of Payee (VoP) {#verification-of-payee}

Starting October 9, 2025, Swan automatically includes VoP in all SEPA Credit Transfers per the EU Instant Payments Regulation.

VoP verifies beneficiary names against their IBAN before payment execution, reducing:
- Fraudulent transactions
- Misdirected payments  
- Customer disputes

### How VoP works {#vop-works}

1. **Automatic integration**: No changes required to existing [`initiateCreditTransfers`](./guide-initiate-ct.mdx) calls
2. **Real-time verification**: Swan checks beneficiary details with receiving bank
3. **Clear results**: Four possible outcomes displayed in consent screen
4. **Enhanced option**: Pre-verify with [`verifySepaBeneficiary`](./guide-verify-beneficiary.mdx) for better UX

### VoP results {#vop-results}

Swan confirms that the **beneficiary's name** matches the name **registered for the provided IBAN** and returns one of four results:

- ✅ **Match**: Information matches exactly
- ⚠️ **Close Match**: Slight discrepancy with suggested correction
- ❌ **No Match**: Information doesn't match 
- ⚠️ **Verification Not Possible**: Technical issue prevented verification

In cases of mismatch, debtors can update beneficiary information before initiating their transfer.

### Implementation options {#vop-implementation}

| Approach | Development Effort | Benefits |
|----------|-------------------|----------|
| **Automatic** | 0 hours | Compliance, basic fraud protection |
| **Enhanced** | 2-4 hours | Faster UX, proactive error prevention |

**Automatic approach**: VoP works with existing transfer calls starting October 9, 2025.

**Enhanced approach**: Add [`verifySepaBeneficiary`](./guide-verify-beneficiary.mdx) to beneficiary management workflows and include verification tokens in [`initiateCreditTransfers`](./guide-initiate-ct.mdx#enhanced-with-vop).

### Legacy beneficiary verification {#legacy-verification}

:::warning Breaking Change
**Effective October 9, 2025:** The `beneficiaryVerification` query (Dutch IBANs only) is deprecated. Use [`verifySepaBeneficiary`](./guide-verify-beneficiary.mdx) instead for all SEPA IBANs.
:::

The previous IBAN verification feature was limited to Dutch IBANs only. VoP expands verification coverage to all SEPA IBANs and provides enhanced functionality.
...
:::info VoP with Standing Orders
VoP verification is required only when scheduling a Standing Order, not for subsequent executions. Once verified during setup, recurring payments use the same verification.
:::
