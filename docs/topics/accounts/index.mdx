...
Swan accounts are defined by their `level` and `type`.
There are two possible values for each.

The account `level` is determined by the [account holder verification status](../onboarding/account-holders/index.mdx#verification-process#statuses).

- `Limited` → Account holder verification status is `NotStarted`, `Pending`, `WaitingForInformation`, or `Refused`.
- `Unlimited` → Account holder verification status is `Verified`.

The account `type` is determined by the <mark>what?</mark>.

- `E-Money` → The account can't be used to manage or move money.
- `Payment Services` → The account can manage and move money, as permitted by the account `level`. Accounts receive the main IBAN.

Swan accounts have different capabilities determined by the combination of `level` and `type` values.

| `level` + `type` | Capabilities |
| --- | --- |
| `Limited` + `E-Money` | <ul><li>The account can be used to:<ul><li>Invite account members.</li><li>Add <mark>virtual?</mark> cards.</li><li><mark>what else?</mark></li></ul></li><li>The account doesn't have its main IBAN.</li><li>Payments **can't** be made from the account.</li></ul> |
| `Limited` + `Payment Services` | <ul><li>The legal representative finished [identification](../users/identifications/index.mdx).</li><li>Account holder verification isn't complete yet.</li><li>The account has its main IBAN.</li><li>Money can be added to the account.</li><li>Payments **still can't** be made from the account.</li></ul> |
| `Unlimited` + `Payment Services` | <ul><li>Account holder verification status is `Verified`.</li><li>Payments **can** be made from the account.</li><li>All accounts functions are possible.</li></ul> |
...
:::info Finnish (`fi`)
While Finnish is an available account language, it isn't a supported account membership language yet.
When the account language is Finnish, the account membership language defaults to English.
:::
...
The PDF is generated automatically after the account's main IBAN is assigned, meaning the account must have the payment level `Unlimited` and the account type `Payment Services`.
Note that the automatic generation might take a few minutes.
The document is generated in the [account language](#language).

If an account's bank details document wasn't generated automatically, you can [trigger the API](./overview/guide-get-bank-details.mdx).
