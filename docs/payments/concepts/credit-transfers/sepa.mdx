...
Transfer euros between accounts located in the [Single Euro Payments Area (SEPA)](/payments/concepts/transactions#sepa).

:::info
Swan automatically includes [Verification of Payee (VoP)](/payments/concepts/verification-of-payee) in all SEPA Credit Transfers to comply with the EU Instant Payments Regulation.
:::
...
∗ *If you see the acronym **ICT**, it's probably referring to [International Credit Transfers](/payments/concepts/credit-transfers/international), not Swan Internal.*
...
Initiating credit transfers is a [sensitive operation](/users/reference/sensitive-operations) and requires consent from an account member with the `canInitiatePayments` [membership permission](/accounts/reference/memberships/membership-permissions#permissions).
...
1. Credit transfers can't be sent if [**account holder verification**](/accounts/concepts/account-holders/verification#verification-process) is still **in progress**.
...
Learn more about [limited account restrictions](/accounts/concepts/account-holders/first-transfer#limited-account-restrictions).
...
1. Availability 24 hours a day, 7 days a week, 365 days a year, including when [SEPA is unavailable](/payments/concepts/transactions#sepa-availability) for other transaction types.
