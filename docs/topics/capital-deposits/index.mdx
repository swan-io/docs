...
When a capital deposit case has the status `Canceled`, Swan provides a reason code on the [`CapitalDepositCaseCanceledStatusInfo`](https://api-reference.swan.io/objects/capital-deposit-case-canceled-status-info) type explaining why.
