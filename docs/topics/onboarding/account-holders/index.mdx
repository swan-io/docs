...
:::info Verifications
[**Identity verification**](../../users/verification/index.mdx) is performed on the `user` object to verify that the user is a real person compliant with all of Swan's requirements.

**Account holder verification** only begins after identity verification is complete.
...
In this case, Swan only needs to perform identity verification (Know Your Customer [KYC] verification) if a KYC has not been performed within the last three months.
...
1. **Identity verification**: User that opened the [**individual**](../individual/index.mdx#id-verification-recommendations) or [**company**](../company/index.mdx#id-verification-recommendations) account completes identity verification with an ID document and a picture or video.
...
* [Montior onboarding to track the verification process](../overview/guide-get-info.mdx)
