...
Add a single <Term id="account-membership">account membership</Term> with the API or from Swan's Web Banking interface.
...
1. In the `restrictedTo` object, add information about the invited account member (lines 6-9).
    - If you're only granting `canViewAccount` permissions, or you're [not granting any permissions](/accounts/reference/memberships/membership-permissions#permissions-none), the invited account member's `birthDate` isn't required.
...
1. Get the `consentUrl`, included in the payload (line 11). Remember, the payload only provides the `consentUrl` if you added it in step 1.8.
