...
The invitation process allows you to grant account access to new members. When you invite someone to become an account member, they receive an email notification asking them to accept the invitation and bind their Swan user account to the membership.
...
1. A requester with the `canManageAccountMembership` permission creates a new account membership using the API or Swan's Web Banking interface.
...
1. Swan sends an email invitation to the invited member (depending on your [notification configuration](#notifications)).
1. The invited member clicks the link in the email, signs in or signs up to Swan, and accepts the invitation.
1. The membership is bound to the user's Swan account and the status changes to `Enabled`.
...
| `Disabled` | Account membership is disabled, is no longer available for use, and can't be restored.<br/><br/>When an account member's membership is disabled, their recurring SingleUseVirtualCards are [automatically reassigned to the account's Legal Representative](/topics/cards/virtual/#suv-recurring). |
...
**To fix binding errors**, refer to the [guide to fix a user binding error](./guide-fix-binding-error.mdx) for detailed resolution steps based on the specific error type.
...
Swan sends email notifications to users when account memberships are managed. These notifications support your partner project branding, including your logo and accent colors.
...
- **Swan Web Banking**: Invitations are sent automatically when using Swan's no-code Web Banking interface, unless disabled in your Dashboard settings.
- **Custom implementation**: If you've forked Swan's Web Banking frontend or built your own interface, you need to call the `sendAccountMembershipInviteNotification` mutation after creating the membership to send the branded invitation email.
