...
:::warning
**One-off SUV cards automatically expire 30 days after creation**. This applies when the spending limit `period` is set to `Always` in the [`addSingleUseVirtualCard`](https://api-reference.swan.io/mutations/add-single-use-virtual-card/) and [`addSingleUseVirtualCards`](https://api-reference.swan.io/mutations/add-single-use-virtual-cards/) mutations.
...
The `SpendingLimit.period` is **not** `Always`.
...
The spending limit resets on a rolling basis. Refer to [hour-level aggregation](/cards/concepts/spending-limits#hour-level-aggregation) for details on how rolling periods are calculated.
