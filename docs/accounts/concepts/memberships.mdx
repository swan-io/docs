...
| Method | Explanation |
| --- | --- |
| Inviter provides phone number and email | <ul><li>Account member's mobile SIM card serves as the [authentication factor](/developers/using-api/authentication/guide-get-token-user#url-parameters-optional).</li><li>Account member can be assigned all [membership permissions](/accounts/reference/membership-permissions#permissions).</li><li>Swan confirms the member's phone number during the [sign-up process](/topics/users/#signup).</li></ul> |
| Inviter provides email only | <ul><li>Account member's verified email serves as the [authentication factor](/developers/using-api/authentication/guide-get-token-user#url-parameters-optional). The membership isn't enabled until user verifies their email.</li><li>Account member can only be assigned the `canViewAccount` and `canManageCards` [memberships permissions](/accounts/reference/membership-permissions#permissions).</li><li>Swan confirms the member's email during the [sign-up process](/topics/users/#signup).</li><li>Swan collects the user's phone number during the sign-up process so the member can perform sensitive operations such as initiating payments, ordering cards, and viewing sensitive card information.</li></ul> |
...
By default, account memberships inherit the [same language as the account](/topics/accounts/#language).
...
:::info Finnish (`fi`)
Finnish is a supported [**account language**](/topics/accounts/#language) and [**account membership language**](/accounts/concepts/memberships#language) with certain limitations:
...
<ul>
    ...
    <li>Finnish isn't available for the [**bank details document**](/topics/accounts/#bank-details). When the account language is Finnish, the bank details document is generated in English.</li>
</ul>
:::
...
When Swan [accounts are closed](/topics/accounts/), the account memberships are impacted as well.

As soon as an [account status](/topics/accounts/#statuses) changes to `Closing`, account members can no longer manage account memberships and beneficiaries or initiate payments (except to empty the account).
...
- [Add multiple memberships](/accounts/guides/memberships/add-multiple)
- [Resend membership invitation notification](/accounts/guides/memberships/resend-invitation)
- [Fix a user binding error](/accounts/guides/memberships/fix-binding-error)
- [Update a membership](/accounts/guides/memberships/update)
- [Suspend or resume a membership](/accounts/guides/memberships/suspend-resume)
- [Disable a membership](/accounts/guides/memberships/disable)
- [Export membership data](/accounts/guides/memberships/export)
