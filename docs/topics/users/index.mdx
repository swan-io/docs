...
flowchart TB
    ...
    DesktopEntry([User sign-up started on desktop with<br/>an OAuth 2.0 link]) -.-> EntersMobileNumber

    EntersMobileNumber -- User on mobile --> ConfirmsMobileNumberMobile([Confirms mobile number])
    EntersMobileNumber -- User on desktop --> ReceivesLink([Receives text message with<br/>link to open on mobile device])
    ...
    QueryParams -- Yes --> UserStartedDesktop{User started sign-up on desktop?}

    EntersPI --> UserStartedDesktop
    UserStartedDesktop -- Yes --> ConfirmsNumber([Confirms number a second time])
    ConfirmsNumber --> ChoosesPasscode([Chooses 6-digit passcode])
    UserStartedDesktop -- No --> ChoosesPasscode
    ...
    style UserStartedDesktop fill:#E6E6FA,stroke:#6A5ACD
