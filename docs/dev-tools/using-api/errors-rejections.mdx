...
Because errors aren't [strongly typed](/dev-tools/graphql#types) like the rest of the GraphQL schema, there's **no guaranteed format or structure** to error responses.
...
In the Swan [schema](/dev-tools/graphql#schema), a `Rejection` is an object containing a descriptive message.
...
- [Authentication](/dev-tools/using-api/authentication) · [Idempotency](/dev-tools/using-api/idempotency) · [Webhooks](/dev-tools/using-api/webhooks)
