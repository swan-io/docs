...
:::warning One active key at a time
Swan doesn't support multiple active keys.
You can't have two public keys installed at the same time: revoke your existing key before installing a new one.

Because keys can't co-exist, expect a brief disruption between revoking your existing key and consenting with the new one.
Plan key rotation during a low-activity period to minimize the impact on your users and operations.
:::

1. Because Swan doesn't support multiple active keys, **pause all operations and consents** that use S2S consent before revoking your existing public key.
1. Go to **Dashboard** > **Developers** > **Server Consent**.
1. Click **Revoke**.
