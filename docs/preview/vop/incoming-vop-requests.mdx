...
## Important matching considerations {#matching-considerations}

### Trade names and merchant names {#trade-merchant-names}

Swan does not verify against trade names or merchant names. VoP matching uses only the legal company name registered in Swan's system.

**For Swan's partners**: If your end users save beneficiaries using trade names or merchant names instead of legal names, VoP requests will return **No match** results. Communicate to your users that they should save beneficiaries using the legal company name that appears on official documents.

**Example**: If a company's legal name is "PIANA SAS" but they operate as "Happy Place," VoP requests using "Happy Place" will return **No match**.

### Middle names {#middle-names}

Swan verifies against the complete name stored in the system, including middle names.

**Example**: If an account holder's name is "Henri Charles Dupont" but the VoP request only includes "Henri Dupont," the result will be **No match**.
