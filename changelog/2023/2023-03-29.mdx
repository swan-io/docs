...
⚡ We implemented a [fallback option](/payments/concepts/credit-transfers/sepa) for Instant SEPA Credit Transfers.
...
🧪 We added Instant SEPA Credit Transfers and their associated recalls to the [Event Simulator](/build/tools/event-simulator) on your Dashboard.

💳 You can now approve authorizations with a partial amount.
To do so, indicate the `partialAuthorizationAmountValue` within your [payment control](/build/using-api/payment-control) response.
