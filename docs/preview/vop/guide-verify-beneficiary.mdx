...
| Permission type | Add trusted beneficiaries via `save` parameter | Initiation using token from `swanAccount` or SEPA verification | Initiation using token from `trustedBeneficiaryId` verification |
|---|:---:|:---:|:---:|
