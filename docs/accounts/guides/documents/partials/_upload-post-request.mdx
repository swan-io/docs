...
Replace all `$VARIABLES` with the values from the payload.

:::tip Upload successful
When you upload a document with the HTTP POST cURL command, Amazon responds with `204: No content`. This is a success response: **the document was uploaded successfully**.
:::
