...
| `CardSuspended` | The card is suspended. | [Unsuspend the card](../../cards/physical/guide-block-unblock.mdx). |
...
| `InvalidPinAttemptsExceeded` | The cardholder entered the incorrect PIN too many times. | [Unblock the physical card](../../cards/physical/guide-block-unblock.mdx#unblock-pin). |
...
| `ThreeDsError` | There was an error with 3-D Secure (3DS). | Retry the transaction and complete the [3-D Secure authentication](#3ds). |
| `TransactionTypeNotAllowed` | This transaction type isn't allowed on the card. | Verify your [card settings](../../cards/index.mdx#settings). For example, you might have tried to withdraw money while withdrawals are disabled. |
...
:::info Account balances
[Digital wallet payments](/topics/cards/digital/) (like Apple Pay or Google Pay) bypass these limits because they use [Strong Customer Authentication (SCA)](/topics/users/consent/#sca) through a password or biometrics. 
...
<Tabs>
<TabItem value="taxi" label={<span title="Taxi">🚗</span>}>
...
1. About 1-3 days after the trip: The taxi company debits the account €37,50, creating a second transaction with the status `Booked`. The remainder of the total authorized amount after the `Booked` transaction is €12,50.
...
<img src={require("../../../images/topics/payments/cards/auth-example-taxi.png").default} alt="Authorization and partial debit example using a taxi app" />
</a>
...
:::note 3DS exception for recurring payments
...
This exception applies to **fixed-amount** recurring payments, where the amount stays the same for every subsequent payment.

For **variable-amount** recurring payments, where the amount can differ between payments, 3DS authentication **is required again** whenever the amount differs from the initially authenticated amount.
:::

:::tip Avoid repeated 3DS with single-use virtual cards
To avoid repeated 3DS challenges for variable-amount recurring payments, consider using [single-use virtual cards (SUVC)](../../cards/virtual/index.mdx).
Each SUVC requires consent only at creation, making subsequent recurring payments more seamless.
:::
