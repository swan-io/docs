...
Capital deposit cases canceled before 2 April 2026 originally had a `null` cancelation reason at the capital deposit level and mapped to the account-level [deprecated closure reason `CapitalDepositReason`](../accounts/closure/index.mdx#deprecated-reasons).
As of June 2026, Swan migrated these cases to the new cancelation reason codes.
:::
...
These reason codes are derived from the same list as [account closure reason codes](../accounts/closure/index.mdx#closure-reason-codes).

## Required documents {#documents}

Swan needs several documents to process a capital deposit.
Upload them with the API as soon as they're available: document upload runs in parallel with shareholder onboarding and KYC.
Documents must meet Swan's requirements; otherwise, Swan assigns the status `Refused` and you need to upload the document again.

### List of required documents {#documents-list}

All documents are mandatory.
The case, each company shareholder, and each individual shareholder have their own `documents` array with pre-assigned document IDs and types.
...
<table>
  ...
  <tr>
    ...
    <td>
      Proof of registration of the company, issued within the last three months
    </td>
    <td>
  ...
  <tr>
    <td>Draft articles of incorporation of the company being incorporated</td>
    <td>
  ...
  <tr>
    <td>Capital deposit power of attorney, signed between your user and Swan</td>
    <td>
...
:::caution
Shareholder accounts only accept [SEPA Credit Transfers](../payments/credit-transfers/sepa/index.mdx) for capital deposits.
...
Follow the guide to [update a capital deposit company](./guide-update-company.mdx).
