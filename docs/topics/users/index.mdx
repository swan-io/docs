...
When a user wants to access their data or make transactions through your service with Swan's API, they must be [authenticated](/dev-tools/using-api/authentication).
...
:::info Sandbox users
When testing your integration, you'll need [Sandbox users](/dev-tools/tools/sandbox-users).
...
This typically occurs when they're invited to become an [account member](/accounts/concepts/memberships) or during the [account onboarding process](../onboarding/index.mdx).

Your user:

1. Opens their sign-up link.
1. **Enters their phone number**.
    - If you include their number when [creating their sign-up link](/dev-tools/using-api/authentication#url-parameters-optional), they won't need to enter it again.
...
1. **Enters their personal information**: first name, last name, and birthdate. This information must match the information that appears on their identity document.
    - The user skips this step if you included this information when [creating their sign-up link](/dev-tools/using-api/authentication#url-parameters-optional).
...
7. Enters their **birth information**: birth city, postal code, country, and nationality.
    - If you include this information when [creating their sign-up link](/dev-tools/using-api/authentication#url-parameters-optional), they won't need to enter it again.
...
| Full API<br />*or*<br />Customized open source frontend | Your user is redirected to the `redirectUrl` you supplied when [creating their sign-up link](/dev-tools/using-api/authentication#url-parameters-optional). Make sure to declare your `redirectUrl` on your **Dashboard** > **Developers** > **Redirect URIs**. Otherwise, the redirection fails. |
...
:::note Webhook
To get a notification when a user is deactivated, subscribe to the `User.Deactivated` [webhook](/dev-tools/using-api/webhooks).
:::
...
[User access tokens](/dev-tools/using-api/authentication#tokens-user) associated with `Blocked` or `Deactivated` users are revoked as soon as the status changes from `Active`.
You can no longer use the API on their behalf.
You also can't use a [project access token to impersonate](/dev-tools/using-api/authentication#tokens-project) `Blocked` or `Deactivated` users. 
:::
