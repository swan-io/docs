...
Transaction confirmation statements are **not available** for card transactions, SEPA Direct Debit transactions (SDD In or SDD Out), fee transactions, internal transfers, or any other transaction types not explicitly listed above.
