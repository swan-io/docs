...
|---|---|
| `NotStarted` | Verification process hasn't started yet. The account holder's legal representative needs to complete [identification](/users/concepts/identifications) before starting account holder verification.<br/><br/>**Next steps**: When verification starts, the status moves to `Pending`. |
| `Pending` | Swan is reviewing the account holder's information before activating or refusing the account.<br/><br/>**Next steps**:<ul><li>If Swan needs something from the account holder, the status moves to `WaitingForInformation`.</li><li>If the review succeeds, the status moves to `Verified`.</li><li>If Swan won't onboard the account holder, the status moves to `Refused`.</li></ul> |
| `WaitingForInformation` | Swan is waiting for information from the account holder, such as a [first transfer](/accounts/concepts/account-holders/first-transfer), [supporting documents](/accounts/concepts/documents), or something else.<br/><br/>**Next steps**:<ul><li>When the account holder provides the missing information, the status returns to `Pending`.</li><li>The status can also move directly to `Verified` or `Refused`.</li></ul> |
