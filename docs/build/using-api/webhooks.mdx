...
Consider the [basic API flow](/build#overview) for reference.
...
A timeout is considered a delivery failure and is [automatically retried](#overview-retry).
...
## Create a webhook from the Dashboard {#create-dashboard}

You can create webhook subscriptions from your Dashboard:

1. Go to **Developers** > **Webhooks**, then click **+ New**.
1. Name your webhook (for example, Supporting Document Collection).
1. Enter the endpoint URL to call when the event occurs.
1. Choose the event types to subscribe to.
1. Enter a **secret** key, or generate one, to secure your webhook (optional).
1. Click **Save**.
...
:::info
Trigger descriptions will be updated with more detail over time to explain which actions or state changes trigger each webhook event.
:::

### [Account holders](/accounts/concepts/account-holders) {#events-account-holders}
...
### [Account onboarding](/accounts/guides/onboarding) {#events-onboarding}
...
### [Account memberships](/accounts/concepts/memberships) {#events-account-memberships}
...
| `AccountMembership.Updated` | <ul><li>Account membership [permissions](/accounts/reference/memberships/membership-permissions#permissions) were updated.</li><li>Membership details changed: spending limits, language, email address, or tax ID.</li><li>Membership [status](/accounts/concepts/memberships/statuses) transitioned from `ConsentPending` to `InvitationSent`.</li><li>Membership [status](/accounts/concepts/memberships/statuses) updated to `BindingUserError`.</li></ul> |

### Account settings {#events-account-settings}

| `eventType` | Trigger |
| --- | --- |
| `AccountSetting.Updated` | The [`allowBulkCreditTransfersWithoutBeneficiaryVerification` setting](/payments/concepts/verification-of-payee/bulk-transfers/#parameters) changed. |

### [Accounts](/accounts/concepts/account) {#events-accounts}
...
### [Billing](/accounts/concepts/billing) {#events-billing}
...
### [Capital deposits](/accounts/guides/capital-deposits) {#events-capital-deposits}
...
| `CapitalDepositCaseEvent.Canceled` | Capital deposit case was canceled. |
...
| `Card.Updated` | Physical card was printed, spending limit was updated, among other updates (doesn't include PIN notifications). |

### [Consent](/users/concepts/consent) {#events-consent}
...
### [Funding](/accounts/concepts/funding) {#events-funding}
...
### [Merchants](/payments/concepts/merchants/) {#events-merchants}
...
### [Payments](/payments/) {#events-payments}
...
### [Supporting documents](/accounts/concepts/documents) {#events-documents}
...
### [Transactions](/payments/concepts/transactions#transactions) {#events-transactions}
...
| `Transaction.Enriched` | [Enriched transaction information](/payments/concepts/cards/enriched-info#enriched) was received outside of a status update. | Cards |
...
| `TransactionStatement.Generated` | [Transaction statement](/payments/concepts/transactions#transactions-statements) was generated successfully. | All payment types |
...
### [Users](/users/concepts/user) {#events-users}
...
- [Payment control](/build/using-api/payment-control) · [Errors and rejections](/build/using-api/errors-rejections) · [Authentication](/build/using-api/authentication)
