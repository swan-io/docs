...
If a Swan user consents to a transfer after receiving a Close Match, No Match or a Verification Not Possible warning, they accept the risks. This means:

- They are responsible if the money goes to the wrong account.
- They won't be entitled to a refund for the incorrect transfer.

End-users can refer to their Swan account's Terms and Conditions for more information about VoP liability.
...
:::info Requirements
Bulk credit transfers require [Option 2: Custom verification](#option-2) using the [`verifyBeneficiary`](/topics/payments/credit-transfers/sepa/vop/guide-verify-beneficiary) mutation. The default verification flow doesn't support bulk payments.
:::
