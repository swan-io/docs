...
Review the [account administrator change concept](/accounts/concepts/memberships/admin-change) for the full model, including how requests work.
...
If the new administrator doesn't receive the invitation, you can resend it using the `sendAccountMembershipInviteNotification` mutation. The membership status must be `InvitationSent`, and you can send up to 5 invitations per day. Alternatively, contact Swan support via the support ticket.
:::
