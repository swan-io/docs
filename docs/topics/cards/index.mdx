...
:::info Printing hub
The [printing hub](./physical/index.mdx#hubs), where **physical cards are printed then shipped** to your card holders, is defined in the card product ID.
...
Cards with a lifetime limit, often called limited forever cards, allow companies and individual account holders to provide a card with a limited lifetime spending amount (`spendingLimit=Always`).
