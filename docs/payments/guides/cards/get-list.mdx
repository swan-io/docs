---
title: Get a list of card transactions
audience:
  - dev
related:
  - /payments/concepts/cards
  - /payments/guides/cards/get-enriched-info
---

# Get a list of card transactions

You can **call the API** to get a list of all card <Term id="transactions">transactions</Term> executed from an account.

Transaction information is also available on your **Dashboard** > **Data** > **Transactions**, as well as Swan's Web Banking interface if you're using it.

:::tip Prerequisites
You need a **project access token** to call the query.
:::

## Guide {#guide}

1. Call the `card` query.
1. Add the card ID.
1. Add `transactions`.
1. Add all information you'd like to review about each card transaction.
1. Add [pagination](/build/using-api/pagination#arguments) if you're expecting a long list.

## Query {#query}

<a href="https://explorer.swan.io?query=&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {2,6} showLineNumbers
query ListCardTransactions {
  card(cardId: "$YOUR_CARD_ID") {
    transactions {
      edges {
        node {
          ... on CardTransaction {
            id
            label
            statusInfo {
              status
            }
            type
            amount {
              currency
              value
            }
          }
          paymentId
        }
      }
    }
  }
}
```

import Environments from '../../../_shared/partials/_environments.mdx';

<Environments />

## Payload {#payload}

Review these two card transactions for an eCommerce purchase: the authorization and the final <Term id="payments">payment</Term>.
Each transaction has its own transaction ID (lines 8 and 23), but both share the same payment ID (lines 18 and 33).

```json {8,18,23,33} showLineNumbers
{
  "data": {
    "card": {
      "transactions": {
        "edges": [
          {
            "node": {
              "id": "decao_af21958a1f6b29f191c386b14217d8c5",
              "label": "Book purchase",
              "statusInfo": {
                "status": "Booked"
              },
              "type": "CardOutDebit",
              "amount": {
                "currency": "EUR",
                "value": "20.00"
              },
              "paymentId": "cro_f12d71a2a601affcb69f61bc8669e919"
            }
          },
          {
            "node": {
              "id": "aucao_d0c6cc158978d783d53c65bd659454ca",
              "label": "Book purchase",
              "statusInfo": {
                "status": "Released"
              },
              "type": "CardOutAuthorization",
              "amount": {
                "currency": "EUR",
                "value": "15.00"
              },
              "paymentId": "cro_f12d71a2a601affcb69f61bc8669e919"
            }
          },
        ]
      }
    }
  }
}
```
