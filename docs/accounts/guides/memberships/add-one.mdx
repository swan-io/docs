...
1. In the `restrictedTo` object, add information about the invited account member (lines 6-9).
    - If you're only granting `canViewAccount` permissions, or you're [not granting any permissions](/accounts/reference/membership-permissions#permissions-none), the invited account member's `birthDate` isn't required.
...
- [Account memberships](/accounts/concepts/memberships#invite) — the invitation model and membership statuses.
- [Membership permissions](/accounts/reference/membership-permissions) · [Membership fields](/accounts/reference/membership-fields) — what to grant and which fields each country requires.
- [Add multiple members](/accounts/guides/memberships/add-multiple) · [Update member](/accounts/guides/memberships/update)
