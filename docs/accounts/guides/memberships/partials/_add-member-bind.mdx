...
1. With the invited account member's [user access token](/developers/using-api/authentication/#tokens-user), call the `BindAccountMembership` mutation.
...
If the API returns a [user binding error](/accounts/concepts/memberships#binding-errors), please [**follow the guide**](/accounts/guides/memberships/fix-binding-error) to fix it.
:::
