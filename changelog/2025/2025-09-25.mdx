...
- We'll [automatically verify the beneficiary's details](/preview/vop/verification-of-payee#option-1) and display the results on the consent screen for single SEPA Credit Transfers without a `beneficiaryVerificationToken` in the `initiateCreditTransfers` call.
...
📡 On **October 9th**, the `verifyBeneficiary` mutation will be available to [verify beneficiary details](/preview/vop/guide-verify-beneficiary/) before initiating credit transfers. Use the `save` parameter to add a trusted beneficiary when the returned token successfully initiates the credit transfer.
