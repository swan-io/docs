...
| `Canceled` | The case was canceled before completion by either you or Swan. You can request to cancel a CapitalDepositCase any time before completion by submitting a request to [Swan Support](https://partner-support.swan.io/hc/en-gb/requests). When a case is canceled, the shareholder status becomes [`CapitalDepositCanceled `](#shareholders-statuses). |
