...
|---|---|
| `ConsentPending` | Request to add an account membership was sent with the `addAccountMembership` mutation and is waiting for the account holder's consent<br/><br/>**Next steps**:<ul><li>If the account holder consents, the status moves to `InvitationSent`</li><li>If the account holder doesn't consent, the status moves to `Disabled`</li></ul> |
...
| `Enabled` | All user information matches, the account member is ID verified, and the account member can use their account membership and corresponding permissions |
...
| `Disabled` | Account membership is disabled and no longer available for use |

### Binding user errors {#status-binding-errors}
...
- `idVerifiedMatchError` → If `true`, mismatch with ID verification

Account members whose membership status is `BindingUserError` can still access basic account and card information, but they can't perform any [sensitive operations](../../consent/index.mdx#sensitive), such as making a transfer or viewing their card numbers.
...
## Sequence diagram: Adding account memberships {#diagrams-add}

*Requester: account holder/legal rep/account member with manage membership permissions
...
participant S as Swan
    ...
    U ->> S: Requests consent
    S ->> P: Returns `accountMembershipId`
    P ->> I: Sends invitee login URL
