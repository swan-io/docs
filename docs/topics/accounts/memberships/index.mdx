...
1. When using their physical card, **payment terminals** and points of service (POS) screens.

By default, account memberships inherit the [same language as the account](../index.mdx#language).
It's possible, however, that not all account members prefer to use the account language chosen by the account holder.
