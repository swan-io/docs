...
For bulk credit transfers, VoP requirements depend on the account type and the account-level verification setting.

### VoP requirements for bulk transfers {#bulk-requirements}

| Account type | Default token requirement | When tokens required |
| --- | --- | --- |
| **Company** | Optional | When account opts in through settings |
| **Individual** | Required | Always |
...
**Migration options:**
1. **Recommended**: Implement the `verifyBeneficiary` mutation to support VoP
2. **Alternative**: Restrict bulk credit transfer access to company accounts only
:::

**For company accounts with VoP required (opted in)**: Each transfer in the batch **must** include a valid `beneficiaryVerificationToken` obtained through the `verifyBeneficiary` mutation. If any transfer in a batch is submitted with full beneficiary details instead of a token, the entire batch initiation will result in a `ForbiddenRejection`.

**For individual accounts**: All bulk transfers **must** include verification tokens. Individual accounts cannot opt out of VoP requirements.

:::tip Integration requirement
If you support bulk credit transfers, you must implement the `verifyBeneficiary` mutation for individual accounts and company accounts that have opted in to VoP. For company accounts that opt out, verification tokens are optional but recommended.
:::

### Account settings impact {#account-settings-impact}

Company account holders can modify their bulk transfer VoP requirements using the [account-level verification settings](/preview/vop/verification-of-payee#account-settings). This directly affects whether verification tokens are required:

- **VoP optional (opted out)**: Bulk transfers can proceed with or without verification tokens
- **VoP required (opted in)**: All bulk transfers must include verification tokens, or the batch will be rejected

### Updating verification requirements {#updating-verification-requirements}

Company account holders can modify their bulk transfer VoP requirements using the `updateAccountSettings` mutation. This allows them to choose between required verification (opted in) or optional verification (opted out) for bulk transfers.
...
**Parameters:**
- **`allowBulkCreditTransfersWithoutBeneficiaryVerification`**: 
  - `false` = Verification tokens required for bulk transfers (opt in to mandatory VoP)
  - `true` = Verification tokens optional for bulk transfers (opt out of mandatory VoP)
- **`consentRedirectUrl`**: Required for the consent flow when opting out (setting to `true`)

For complete API documentation including all error handling, see [Updating account settings](/preview/vop/verification-of-payee#updating-settings).
...
### Testing bulk credit transfers {#testing-bulk-credit-transfers}
...
Company accounts can modify their VoP requirements through the [account settings](/preview/vop/verification-of-payee#updating-settings), while individual accounts always require VoP for bulk transfers.