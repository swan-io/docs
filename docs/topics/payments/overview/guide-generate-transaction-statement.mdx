...
:::info Supported transaction types
Transaction confirmation statements are **only** available for:

- Incoming and outgoing **SEPA Credit Transfers** (Regular).
- Incoming and outgoing **Instant SEPA Credit Transfers**.

All eligible transactions must have the status **`Booked`**.

**Not supported**

Transaction confirmation statements are **not available** for:

- Card transactions.
- SEPA Direct Debit transactions (SDD In or SDD Out).
- Fee transactions.
- Internal transfers.
