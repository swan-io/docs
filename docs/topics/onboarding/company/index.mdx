...
The `legalFormCode` field is required for all company onboardings. Use the `legalForms` [query](https://explorer.swan.io?query=cXVlcnkgTGVnYWxGb3JtcyB7CiAgbGVnYWxGb3Jtcyhjb3VudHJ5OiBGUkEpIHsKICAgIGNvZGUKICAgIGxvY2FsTmFtZQogICAgbG9jYWxBYmJyZXZpYXRpb24KICB9Cn0%3D&tab=api) to retrieve all valid legal forms for a given country, translated based on the `Accept-Language` header. Each legal form includes a `code` you can use as the `legalFormCode` value.
