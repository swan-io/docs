---
title: Cancel a credit transfer
audience:
  - dev
related:
  - /payments/concepts/credit-transfers/sepa
  - /payments/guides/credit-transfers/sepa/refund-ct
---

# Cancel a credit transfer

Cancel <Term id="credit-transfer">SEPA Credit Transfers</Term>, Instant SEPA Credit Transfers, and Swan Internal Credit Transfers with the API.

:::caution Upcoming transfers only
Only transfers with the status `Upcoming` can be canceled.
If your <Term id="transactions">transaction</Term> has any other status, including `Pending`, you can request a [recall](/payments/concepts/credit-transfers/sepa/r-transactions#recalled).
:::

## Guide {#guide}

1. Call the `cancelTransaction` mutation.
1. Add your [transaction ID](/payments/guides/payment-operations/get-transaction-id).
1. Add the payload and rejection.

## Mutation {#mutation}

<a href="https://explorer.swan.io?query=&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {2} showLineNumbers
mutation CancelTransfer {
  cancelTransaction(input: { transactionId: "$YOUR_TRANSACTION_ID" }) {
    ... on CancelTransactionSuccessPayload {
      __typename
      message
    }
    ... on ForbiddenRejection {
      __typename
      message
    }
  }
}
```

import Environments from '../../../../_shared/partials/_environments.mdx';

<Environments />

## Payload {#payload}

The payload shows the credit transfer canceled (line 5).

```json {5} showLineNumbers
{
  "data": {
    "cancelTransaction": {
      "__typename": "CancelTransactionSuccessPayload",
      "message": "TransactionCanceled"
    }
  }
}
```
