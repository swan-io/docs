...
| **Accounts** | Account subscriptions | Enabled `Verified` accounts not in `Closing` or `Closed` on the 1st of the month. Charged from when the payment level status is set to `Unlimited`. The first and closing months are billed in full. |
| | Account member subscriptions | All additional `Enabled` members for an account (excluding the legal representative). |
| | Account opening fee | Account holders who started verification during the billing period (`Pending` banking verification status). Fee is charged once for the opening of multiple accounts. |
...
| | Apple Pay card subscriptions | Number of cards successfully added as Apple Pay digital cards during the billing period. |
...
| | Card products | Number of card products `Enabled` for a project. The monthly fee still applies if a card product is `Suspended`, as long as it has at least one `Enabled` card. |
