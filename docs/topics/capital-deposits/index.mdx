...
| `Canceled` | The case was canceled before completion by either you or Swan. When a case is canceled, the shareholder status becomes [`CapitalDepositCanceled `](#shareholders-statuses). |
...
| `WaitingForTransfer` | Waiting for the shareholder to deposit the full share capital in their Swan account created during onboarding, which **must** be transferred from an account belonging to the shareholder. The transfer must be a [SEPA Credit Transfer](#transfer-requirements); international credit transfers are not accepted and will be rejected automatically. |
...
Shareholder accounts can only accept [SEPA Credit Transfers](../payments/credit-transfers/sepa/index.mdx) for capital deposits.

:::caution International transfers rejected
International credit transfers received on shareholder accounts are **rejected automatically** before settlement, regardless of the amount.
Ensure all capital deposit transfers are sent as SEPA Credit Transfers from the shareholder's account in their name.
:::
