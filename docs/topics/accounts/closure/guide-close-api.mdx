...
Refer to the [account statuses](../index.mdx#statuses) section for an overview of what happens while an account is `Closing`. The account moves to `Closed` once it meets all [conditions to move from `Closing` to `Closed`](./index.mdx#closing-to-closed-conditions).
:::
