...
Swan generates two types of access tokens: **user** and **project**.
Access tokens are provided through an HTTP authorization header, such as `Authorization: Bearer {access token}`.

| ↓ Token type \| Can call → | Queries | Non-sensitive<br/>mutations | Sensitive<br/>mutations |
| --- | --- | --- | --- |
...
User access tokens allow you to act on behalf of an individual user in your project, often with the goal of executing sensitive operations.
...
**Swan discourages frequent use of refresh tokens** for refreshing your user access token due to high resource consumption.
It's also often unnecessary, considering that a project access token can be used for most of Swan's API operations.

Consider **reserving the use** of user access tokens for **sensitive operations**.
...
When using a user access token, it's recommended to add your pre-approved URIs, or Uniform Resource Identifiers, to your Dashboard.
...
You can also use a project access token to act as a user within your project, referred to as [impersonation](./guide-impersonate.mdx).
...
Learn about impersonation in the [dedicated guide](./guide-impersonate.mdx).
