...
Send <Term id="credit-transfer">SEPA Credit Transfers</Term>, Instant SEPA Credit Transfers, and Swan Internal Credit Transfers with the API.
...
Swan automatically includes [Verification of Payee (VoP)](/payments/concepts/verification-of-payee) in all single SEPA Credit Transfers. Integrate the `verifyBeneficiary` mutation to control how the result is shown.
:::
...
1. Add [an `idempotencyKey`](/build/using-api/idempotency) to prevent duplicate payments (line 4).
1. For this use case, add information about your `sepaBeneficiary`, including their name and <Term id="ibans">IBAN</Term> (lines 10-11).
...
Send a SEPA Credit Transfer with an optional structured or unstructured [invoice reference](/payments/concepts/credit-transfers/label-and-reference#invoice-references) by using the `labelType` and `label` fields.
...
1. Add the consent URL to the success payload (line 25): `statusInfo` > `PaymentConsentPending` > `consent` > `consentUrl`.
1. Include error handling for every rejection type in the `InitiateCreditTransfersPayload` union (lines 31-56).
...
```graphql {4,5,7,8-10,13-14,18,22,25,31-55} showLineNumbers
...
Bulk credit transfers require verification tokens from the `verifyBeneficiary` mutation. Refer to [bulk credit transfers with VoP](/payments/concepts/verification-of-payee/bulk-transfers).
:::
...
### Query the payment before consent is granted {#query-payment}

While a payment is in `ConsentPending` status, you can query the payment's `statusInfo` to access the `consent` object, which contains the `userId` and full `user` details for the user who must <Term id="consent">consent</Term> to the transfer.
...
The `initiateCreditTransfers` mutation requires a [user access token](/build/using-api/authentication#tokens-user), which identifies the initiating user.
When you call the mutation successfully, store the user ID or <Term id="account-membership">account membership</Term> ID alongside the `paymentId` returned in the response.
...
:::danger Rejected
If your <Term id="transactions">transaction</Term> is created with the status `Rejected`, learn why in the `rejectedReasonCode` field.
