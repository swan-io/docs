...
- The `onboardingUrl` field is only returned in the create or update mutation response when the **Public URLs** toggle is enabled in the Dashboard. The previous API returned the URL regardless of the toggle, so enable it under **Settings** > **Onboarding** > onboarding URLs before going live if you rely on the URL.
...
1. If you use Swan's no-code onboarding interface, confirm the mutation response returns `onboardingUrl`, which Swan returns only when **Public URLs** is enabled in your Dashboard.