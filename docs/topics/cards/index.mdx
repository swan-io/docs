...
Swan supports two types of spending limit periods: **rolling periods** and **calendar periods**.
Choose the type that best matches your needs and use case.

**Rolling periods** don't depend on a determined date, day of the week, or time.
Instead, rolling spending limits consider the amount spent over a period of time (last 24 hours, last 7 days, last 30 days).
Rolling periods are the default if no mode is specified.

**Calendar periods** reset at a specific time on a specific day.
For example, monthly calendar limits reset on the 1st of each month at a chosen hour, and weekly limits reset on a chosen day of the week at a chosen hour.

:::info Non-existent dates
If a calendar period's reset date doesn't exist in a given month, the limit resets on the next available day.
For example, if you set a monthly limit to reset on the 31st, the limit resets on March 1st in February (since February doesn't have a 31st).
:::

### Limits imposed by Swan {#limits-swan}

All spending limits imposed by Swan are for a period of **1 month rolling**.
...
- Daily calendar: Resets every day at a specified hour (0-23).
- Weekly calendar: Resets on a specified day of the week at a specified hour.
- Monthly calendar: Resets on a specified day of the month (1-31) at a specified hour.

:::warning Compatibility
When setting a spending limit, you must use either a rolling mode **or** a calendar mode, not both.
The period (`Daily`, `Weekly`, `Monthly`) must match the mode type chosen.
:::
...
Swan computes these limits at the **hour level** for all cards with `Daily`, `Weekly`, and `Monthly` limits (both rolling and calendar modes), ensuring more precise and consistent period calculations.
...
Payments are timestamped to the hour for the following limit periods (both rolling and calendar):
