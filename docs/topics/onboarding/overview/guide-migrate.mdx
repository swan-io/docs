...
Previous onboarding mutations and queries are deprecated and will be removed on **September 30, 2026**.
...
The mapping tables will be available until the previous mutations are retired on September 30, 2026.
...
- The `onboardingUrl` field is only returned in the create or update mutation response when the **Public links** toggle is enabled in the Dashboard. The previous API returned the URL regardless of the toggle, so enable it under **Settings** > **Onboarding** > **Onboarding links** before going live if you rely on sharing public links.
...
After the previous mutations are removed on September 30, 2026, you'll no longer be able to update or finalize onboardings through them. Make sure your integration is fully migrated by then.
...
| September 30, 2026 | Previous mutations and queries removed. |
...
1. If you use Swan's no-code onboarding interface, confirm the mutation response returns `onboardingUrl`, which Swan returns only when **Public links** is enabled in your Dashboard.