...
You can prevent a physical card from being renewed with the `cancelPhysicalCardRenewal` mutation, so it expires instead of renewing. See [Cancel a physical card renewal](./guide-renew.mdx#cancel-renewal).
