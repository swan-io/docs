...
The Swan account holder or eligible account member consents to the account funding source payment mandate through [Strong Customer Authentication](../../users/consent/index.mdx#sca).

However, Swan doesn't provide a method to collect a signature.
As a Swan Partner, it's your responsibility to manage payment mandate signatures.
You must provide the `signatureDate` when [adding a SEPA Direct Debit funding source with the API](./guide-add-source.mdx), and be able to provide proof of signature if requested by the funding source's financial institution.
...
| :---: |---|
| `ConsentPending` | Payment mandate was added while [adding a direct debit funding source](./guide-add-source.mdx) with the `addDirectDebitFundingSource` mutation.<br/><br/>**Next steps**: <ul><li>If the debtor consents to the mandate, the status moves to `Enabled`.</li><li>If the debtor doesn't consent, the status moves to `Rejected`.</li><li>If the debtor doesn't respond to the consent request, the status moves to `Canceled`.</li></ul> |
| `Enabled` | Payment mandate is active and valid, and the corresponding funding source (if also `Enabled`) can be used to fund the account.  |
| `Rejected`<br /><mark>still confirming rejected status info</mark> | Payment mandate was rejected, possibly because the external account's IBAN and Swan payment mandate list different account holders. |
