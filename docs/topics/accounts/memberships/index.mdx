...
| `Disabled` | Account membership is disabled, is no longer available for use, and can't be restored.<br/><br/>When an account member's membership is disabled, their [recurring SingleUseVirtualCards](/topics/cards/virtual/#suv-recurring) are [automatically reassigned to the account's Legal Representative](/topics/accounts/memberships/guide-disable#impact-cards). |
