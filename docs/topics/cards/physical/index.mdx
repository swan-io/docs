...
Understand how Swan determines the name to print on a physical card.
Learn how a PIN is chosen, assigned, and viewed.
Understand what happens if a card holder repeatedly enters their PIN incorrectly.
...
| `Suspended`<br /><br />Also referred to as _Blocked_ | Physical card is suspended and not available for use. Card holders can still view card information and use digital cards (not virtual cards) when a physical card is `Suspended`.<br/><br/>_Cards can be suspended for various reasons, including a request from you or the card holder, or a Swan action in the case of suspicious activity._<br/><br/>**Next steps**:<ul><li>Restore the card's previous status with the API.</li><li>Cancel the card with the API.</li></ul>                                                                                                                                    |
