...
|---|---|
| End customer | Uses your project logo and accent color. Includes the account's IBAN and BIC to help users complete the transfer. |
...
| Configuration | Template |
|---|---|
| End customer | Uses your project logo and accent color. Includes the account number so users can reference it when contacting your support team. |
...
| Configuration | Template |
|---|---|
| End customer | Uses your project logo and accent color. Includes the account's IBAN and BIC so users can start using their account. |
| Partner | Uses Swan branding. Includes data to help with your KYC process: account Holder ID, Account Holder name, account IBAN, account BIC, account admin email, and account admin language. |
...
Failure to complete the re-verification process within a **90-day window** will result in the **automatic suspension** of the account. All associated IBANs, cards, and payment capabilities will be restricted until the verification is complete.
:::
...
Make sure your [branding settings](/topics/projects/guide-activate/#step-8-configure-your-branding-in-live) are up to date (logo and accent color) in **Dashboard** > **Settings** > **Branding**.

#### Re-verification steps {#re-verification-portal-steps}

The re-verification process follows a structured path from initial notification to validation. 
...
1. Uploads supporting documents if requested.
1. Submits the re-verification request.
1. Receives a confirmation of submission email, then waits for the outcome of the KYC analysts review. 
    - Successful re-verification status: `Verified`.
1. Follow-up requests. 
    - If more information or supporting documents are needed, Swan sends a follow-up request. The portal reopens only for the missing items.
...
We recommend setting them to Swan to end customer (Partner in CC). You can choose a unique email address for re-verification emails. 
:::

### Re-verification timeline and reminders {#re-verification-timeline}

The periodic re-verification process has a strict **90-day window**. This starts when the initial notification is sent to your user.
...
| **Re-verification deadline** | **Day 90+** | Swan suspends all of the account holder's accounts. Your users can't send or receive payments, including SCT, SDD, or card payments. |
