...
1. Add the `consentRedirectUrl` (line 11):
    - `consentRedirectUrl` is **optional** when inviting members with no permissions (only `canViewAccount: false` or no permissions at all).
    - `consentRedirectUrl` is **mandatory** when granting any permissions beyond viewing—the API will return a validation rejection if missing.
...
```graphql {4,6-9,11,13-18,21,27,30} showLineNumbers
mutation AddMember {
