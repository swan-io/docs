...
It's worth clarifying that webhooks don't actively _respond_ but autonomously send information to your system, potentially introducing delays of up to 10 minutes after the actual action occurs.
...
### Information included in notifications {#event-handling-info}
...
| `SupportingDocumentCollection.Created` | A new supporting document collection was created either                      |
...
| `User.Deactivated` | User was deactivated<br /><br />_The phone number associated with the deactivated user's Swan account can now be used to sign up a different user._ |
