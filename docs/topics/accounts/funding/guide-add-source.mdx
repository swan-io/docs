...
1. The funding source is a non-Swan account accessible to the Swan account holder or eligible account member.
1. Call the mutation with a [user access token](../../../developers/using-api/authentication/guide-get-token-user.mdx) associated with the account member. The account member needs to sign the direct debit mandate by consenting to the [sensitive operation](../../users/consent/index.mdx#sensitive) of adding a funding source. <mark>fix #3</mark>
:::
...
However, the account holder still needs to declare their payment mandate to the external institution.
:::
