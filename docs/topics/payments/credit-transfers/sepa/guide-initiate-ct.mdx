...
1. You're authenticating with a [user access token](/dev-tools/using-api/authentication#tokens-user).
:::
...
1. Add [an `idempotencyKey`](/dev-tools/using-api/idempotency) to prevent duplicate payments (line 4).
...
The `initiateCreditTransfers` mutation requires a [user access token](/dev-tools/using-api/authentication#tokens-user), which identifies the initiating user.
