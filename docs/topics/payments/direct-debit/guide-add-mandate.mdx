...
Send a link to an account member with the `canInitiatePayments` [membership permission](/accounts/reference/membership-permissions#permissions) so they can complete the mandate registration.
