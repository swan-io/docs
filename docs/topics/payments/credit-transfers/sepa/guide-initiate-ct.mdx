...
Send a SEPA Credit Transfer with an optional structured invoice reference using the `labelType` field.

:::info Optional structured reference
- `labelType`: optional field. Example: `OgmVcs`, which currently supports the Belgian OGM VCS - a 12-digit reference number used in Belgium for payment reconciliation.
- `label`: when `labelType` is used, this field contains the structured reference value.
- `transaction.label`: contains the structured reference value when retrieving transaction data through a query.
:::
