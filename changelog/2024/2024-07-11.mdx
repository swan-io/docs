...
Therefore, we deployed an [automatic renewal process](/cards/concepts/physical/renewal) so virtual and physical cards renew automatically before they expire.
Your cardholders can [update their delivery address](/cards/concepts/physical/renewal#renew-address) for the new physical card if needed.

Users can now [choose their PIN](/cards/concepts/physical/pin#pin-choose) when printing a physical card.
...
🔎 **In 1 week**, we're generating [Bank Details documents](/accounts/concepts/account/documents#bank-details) for all accounts created before 16 November 2023, which weren't available automatically when we launched this feature.
Subscribe to the `Account.Updated` webhook to synchronize your system if necessary.

🌐 Choose and update the [account membership language](/accounts/concepts/memberships/language), used for the invitation email and when ordering and using cards.
