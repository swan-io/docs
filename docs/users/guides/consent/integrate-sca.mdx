...
If you embed Swan into your own mobile app, integrate <Term id="sca">Strong Customer Authentication</Term> (SCA) using an **in-app browser** so your users complete SCA without leaving your app.
Review [SCA in your mobile app](/users/concepts/consent/sca#mobile-app) for how this changes the default browser behavior and when the integration is relevant.

:::danger In-app browser required
For security reasons, all links **must open** in in-app browsers.<br />
**Never use WebViews** to open URLs (`consentUrl` or `oAuthUrl`).
:::

## What you'll need {#what-you-need}

SCA integration works for both OAuth (login/authentication) and Consent (sensitive operations) flows.
...
Swan provides an official React Native package: [@swan-io/react-native-browser](https://github.com/swan-io/react-native-browser)

:::warning Follow the references
When using frameworks other than React Native, make sure your package is configured to follow Swan's implementation references.
:::
...
If you use React Native, follow the [React Native linking guide](https://reactnative.dev/docs/linking).
...
- `consentId`.
- `env`: `Sandbox` or `Live`.
- `status`: the [consent status](/users/concepts/consent/statuses) at the time of redirection.
