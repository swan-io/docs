...
You can retrieve the beneficiary verification result used in a transaction using the `transaction`, `transactions`, `standingOrder`, and `TrustedBeneficiary` queries.
...
Server-to-Server consent works with both VoP integration options: Option 1 and Option 2.
:::
