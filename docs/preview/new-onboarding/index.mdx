...
The new API organizes fields into distinct categories:

| Category | Description |
|---|---|
| **Account** | Account-level settings (`accountCountry`, `accountName`). |
| **Account administrator** | Information about the person opening the account (`email`, `language`, `typeOfRepresentation`, address details). |
| **Organization** | Company details (`name`, `registrationNumber`, `businessActivity`, `monthlyPaymentVolume`, address, tax identifiers). |
| **Representatives** | Legal representatives of the organization (personal details, roles, addresses). |
| **Ultimate Beneficial Owners** | UBOs with ownership details (`direct`, `indirect`, `totalCapitalPercentage`, `type`). |

Field requirements vary by country. French company onboardings support pre-fill via the `companyInfoRegistryData` query for organization, representative, and UBO fields.
