...
When Swan receives an incoming International Credit Transfer, Swan books it immediately and creates a `InternationalCreditTransferTransaction` type (step 8 in the [standard flow](/payments/concepts/credit-transfers/international#incoming-flow) for incoming transfers). 
