...
| `PendingReview` | Onboarding is finalized and a document review is needed<br/><br/>**Next step**: Can move to `Approved`, `Rejected`, or `Canceled` depending on partner actions and Swan decisions (or, if documents are insufficient → `WaitingForDocument`) |
...
An `Uploaded` document can be deleted (for example, if the wrong file was shared). Deletion removes the document rather than assigning it a status. See [Delete a document](/accounts/guides/documents/delete).
:::
