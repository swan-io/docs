...
When the mandate is ready, [declare it to Swan](./declare.mdx) through the API.
...
| --- | --- |
| B2B | <ol><li>Mandate created with the status `ConsentInitiationPending`.</li><li>Before the debtor can consent, you must [declare the payment mandate](./declare.mdx).</li><li>Debtor must consent, but only the first time the mandate is used.</li></ol> |
...
- [Accept payments with Internal Direct Debit (step-by-step)](./accept.mdx)
- [Request Internal Direct Debit payment method](./request.mdx)
...
- [Sandbox](/payments/reference/sandbox/sandbox-merchant-idd)