...
Use the [`createCapitalDepositCase`](https://api-reference.swan.io/#mutation-createCapitalDepositCase) mutation to create your capital deposit case.
...
1. **`companyShareholders`**: Add required information about company shareholders (starts at line 5).
    1. Add the `onboardingInfo` object to prefill onboarding information for `individualUltimateBeneficialOwners` (starts at line 8).
    1. You can also prefill information about the company (starts at line 33).
1. **`individualShareholders`**: Add required information about individual shareholders (starts at line 50).
1. **`totalCapitalDepositAmount`**: Add the combined capital deposit amount from all company and individual shareholders (line 69).
1. **`onboardingCapitalDepositCompany`**: Add the company onboarding object to start creating the future company account (starts at line 70).
    - Note that individual `individualUltimateBeneficialOwners` and company information should match the information entered in step 3.
1. Add the `CreateCapitalDepositCaseSuccessPayload` (starts at line 107). Consider adding several pieces of important information, such as:
    1. `companyOnboarding` > `id`.
    1. `documents` > `type`, `status`, and `id`.
    1. `shareholders` information, including `onboarding` > `id`.
1. Add rejections in case the mutation doesn't succeed (starts at line 154).
1. **▶ Run** your mutation.
...
1. Information about required documents, including the `type`, `status`, and `id` (starts at line 14).
1. Information about all shareholders (starts at line 41), including:
