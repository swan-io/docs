...
:::warning Bulk Transfer Requirement
**For bulk transfers** (multiple credit transfers in one API call): Each transfer MUST include a `beneficiaryVerificationToken` unless your company account has explicitly opted out of VoP. Bulk transfers without tokens will be rejected.
:::
