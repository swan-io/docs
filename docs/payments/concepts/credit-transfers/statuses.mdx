...
<p className="ia-lede">The status flows for <Term id="credit-transfer">credit transfer</Term> <Term id="transactions">transactions</Term>, <Term id="trusted-beneficiary">trusted beneficiaries</Term>, and <Term id="standing-order">Standing Orders</Term>.</p>
...
|---|---|
| `ConsentPending` | An eligible account member added a trusted beneficiary, either directly (with the dedicated mutation or through Web Banking) or when initiating a transfer.<br /><br />To finish adding the trusted beneficiary, an account member with the `canManageBeneficiaries` permission must <Term id="consent">consent</Term> with [Strong Customer Authentication (SCA)](/users/concepts/consent/sca).<br /><br />**Next steps**: The status moves to `Enabled` when consent is received, or to `Canceled` if it isn't. |
| `Enabled` | Consent was received to add the trusted beneficiary. Eligible account members can now initiate transfers to this trusted beneficiary.<br /><br />**Next steps**: The beneficiary moves to `Canceled` if an eligible account member removes it. |
...
| Status | Explanation |
| --- | --- |
| `ConsentPending` | Standing Order was scheduled but consent hasn't been received.<br /><br />**Next steps**: The status moves to `Enabled` when consent is received, or to `Canceled` if consent is refused. |
| `Enabled` | Consent for the Standing Order was received, the order is executed as scheduled indefinitely according to its configuration.<br /><br />**Next steps**: The Standing Order stays `Enabled` until it's canceled. |
...
| Status | Explanation |
| --- | --- |
| `Pending` | Status assigned when the transfer is initiated; the transfer retains the status `Pending` while the transactions associated with the transfer follow the standard transaction status flow<br /><br />**Next steps**: The transfer moves to `Booked` when the funds arrive, or to `Rejected`. |
