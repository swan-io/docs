...
Swan servers will wait for 15 seconds for a response (5 seconds in Sandbox mode), otherwise it will consider the webhook attempt as failed and will proceed with a retry if the maximum number of attempts has not been reached.
