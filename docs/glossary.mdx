...
### [Idempotency](developers/using-api/idempotency.mdx) {#idempotency}

Unique string used to ensure API operations execute only once, preventing duplicate transactions during network issues or request retries. Keys expire after 24 hours and are scoped to authentication context. [Learn more in the dedicated article](/developers/using-api/idempotency.mdx).
...
## [Supporting documents](topics/accounts/documents/index.mdx) {#supporting-documents}
