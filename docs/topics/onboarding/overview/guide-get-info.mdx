...
:::tip Deprecated queries
The previous `onboardings` query is deprecated but still functional. Use `accountHolderOnboardings` for all new integrations.
The previous `onboarding` query (single) is replaced by `accountHolderOnboarding`. The previous `onboardingInfo` query (public) is replaced by `publicAccountHolderOnboarding`.
:::
...
1. Add all the query options you'd like to monitor.
    * For this exercise, add: `createdAt`, `email`, `onboardingUrl`, `statusInfo` > `status`, `id`, and `totalCount`.
