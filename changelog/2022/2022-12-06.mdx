...
💸 [Initiate SEPA Instant Credit Transfers](/topics/payments/credit-transfers/sepa#instant) (`SepaInstantCreditTransferOut`) transactions using the existing `initiateCreditTransfers` API mutation with the `isInstant` input flag.
...
🤖 Use [server-to-server consent](/topics/users/consent#s2s) to avoid any human interaction with the new `grantConsentWithServerSignature` mutation!
...
⚡ During onboarding, once we have verified the legal representative of the account holder is not a politically exposed person or on a sanctions list, we will [distribute the IBAN](/topics/accounts/ibans#main) via API and Web Banking.
