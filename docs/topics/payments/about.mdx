...
There are three payment statuses: `Initiated`, `ConsentPending`, and `Rejected`.

```mermaid
%%{ init: { 'flowchart': { 'curve': 'linear' } } }%%
flowchart LR
    P((Payment<br/>initiated)) --> I[Initiated]
    P --> RJ[Rejected]
    I --> RJ
    CP[ConsentPending] -->I
    P -.-> CP
    ...
    style P fill:#F4F4F5,stroke:#514F59
    style F fill:#fffdf4,stroke:#cca922
```

| Status | Explanation |
|---|---|
| `ConsentPending` | Consent is required for this payment<br/><br/>**Next step**: Transaction flow blocked while waiting for consent; when consent is received, transaction flow resumes and payment status changes to `Initiated` |
| `Initiated` | Payment successfully requested; enters the [transaction](#transaction-statuses) flow<br/><br/>*Possible final payment status; can also change to `Rejected`* |
| `Rejected` | Payment rejected, either directly after payment initiation or from the `Initiated` status<br/><br/>*Final payment status* |

### Push and pull payments

The key difference between push and pull payments lies in **who initiates the payment**.

**Push payments**, such as credit transfers and card payments, refer to transactions where the **debtor initiates the payment**.
...
**Pull payments**, such as direct debits, refer to transactions where the **creditor initiates collecting funds** from the debtor's account.
...
* **Transaction #1** ends with the transaction status `Rejected`.
