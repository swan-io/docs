...
- **Placeholder pages** use the shared `docs/get-started/_partials/_wip-placeholder.mdx` (+ a `{/* TODO:SME */}` marker), never bespoke wording. **Seeded placeholders** (pages carrying draft steps awaiting verification, e.g. the four Dashboard-operations placeholders) instead use the parameterized `docs/_shared/partials/_seeded-placeholder.mdx` (props: `source`, `checks`; the TODO:SME marker is baked into the partial) — added 21 August 2026, replacing four bespoke admonitions; re-tiered to `_shared` the same day when Users gained its first seeded placeholder.
...
- **admin-change.mdx re-split (same day, after review)**: the accretion above made the concept a three-purpose page. The status machine + table moved to `concepts/memberships/statuses.mdx#admin-change` (one statuses leaf per domain, funding precedent); the guide converted to the full chooser shape at `guides/memberships/change-admin/`: dual-audience chooser index (URL unchanged) + `from-support-center.mdx` (ops walkthrough) + `using-the-api.mdx` (dev), counterpart-linked. **Pattern extension: "From Support Center" is the ops-path leaf label when the operator path runs through Swan Support and no Dashboard path exists** (verified against Slack/Linear 21 Aug 2026: initiation is a Forest smart action from a Zendesk ticket; partner Dashboard self-service is not planned — do NOT add a from-the-dashboard leaf or placeholder). Both `{#statuses}` and `{#api}` remain on the concept as one-line pointer headings. Lesson recorded: **after moving sections into a page, re-judge the destination page as a whole against the page-shape rules.**
- **Audience-coherence sweep (same day, all 228 Users+Accounts pages)**: three fixes. (1) `reference/accounts/multiple-accounts-requirements` had a full using-the-api section (`#check-limit`) embedded in a reference page — moved verbatim to `guides/account-operations/create-additional-account#check-limit`; the reference keeps the heading as a pointer. (2) Five flat dev guides (users get-info-one/get-info-multiple/get-list, identifications get-identifications/get-latest-id) acknowledged the ops read path (Dashboard > Data > Users) inline with no leaf — restructured into ONE category `users/guides/user-operations/get-info/`: neutral chooser index + `from-the-dashboard.mdx` (ops, seeded placeholder) + five API leaves (`one-user`, `multiple-users`, `user-list`, `identifications`, `latest-identification`; the two identification lookups moved in from `guides/identifications/`, emptying that category). **Pattern extension: a task family sharing one Dashboard surface gets one from-the-dashboard leaf + N single-endpoint API leaves under a neutral chooser** (5:1 counterpart to the Dashboard leaf is deliberate; the ops leaf itself carries no counterpart). Legacy v2 redirects retargeted to the new URLs (the pre-restructure IA URLs never shipped, so no chained redirects created). (3) Same pattern in `manage-onboardings` (Data > Onboardings), then the multi-action guide itself was split per the task-family pattern: category `accounts/guides/onboarding/manage-onboardings/` = neutral chooser index (URL unchanged) + `from-the-dashboard.mdx` (ops, seeded placeholder) + three SRP API leaves (`get-info`, `list-required-documents`, `finalize`). API-only actions get their own leaves with no Dashboard counterpart — the family does NOT have to mirror Dashboard-vs-API symmetrically. The export section was a pure pointer and dissolved into the chooser's Before-you-start; the six legacy redirect fragments (`#get-info`, `#get-list`, `#finalize`, `#export-onboarding-data`) retargeted to the leaf URLs. This also RESOLVES deviation #3 below: each DeprecationTimeline admonition now renders once on its own leaf. **Seeded-placeholder tally: SEVEN** (was four; +users get-info, +onboarding look-up, +funding get-info on 21 August 2026). Cleared as correct after investigation: Dashboard > Developers sections in dev guides (configure-notifications, implement-s2s) are dev-surface work, not ops paths; membership guides stay flat dev (Dashboard lists/exports only, per the guides hub); close-unlimited/close-limited stay flat ops (Support-only task, no dev alternative).
...
3. ~~`manage-onboardings.mdx` renders `<DeprecationTimeline />` twice~~ — RESOLVED 21 August 2026 by the manage-onboardings split: the queries admonition lives on `get-info.mdx`, the `finalizeOnboarding` admonition on `finalize.mdx`; one render each.
...
### Users+Accounts residual sweep (25 August 2026)

Full page-by-page audit of both domains against this manifest (5 parallel reviewers + mechanical script + production build). Build green, zero warnings. **37 files fixed in place**, notably: ops leaves stripped of arbitrary `counterpart` per the task-family pattern (funding get-info, manage-onboardings); users get-info family brought onto the shared-prereq-partial contract (`get-info/_prereqs.mdx`, parameterized — note the md-mirror does NOT evaluate `??` defaults in `{props.x}` expressions, pass every prop explicitly); users guides hub now lists the get-info chooser (was listing leaves only, ops leaf unreachable); deactivate `from-the-dashboard` converted to the shared seeded placeholder (**tally now EIGHT**); account-holders hub gained the missing `verification-renewal` entry, documents hub the missing `get-ids` entry; sixth undisclosed superset badge (sandbox/documents) got the disclosure line; expert.mdx Oceania intro no longer contradicts the French Polynesia passport row; membership-fields ghost header row and italy duplicate UBO row removed; migrate-api mapping links retargeted to the real mapping homes; add-source payload no longer shows a field the mutation doesn't request; three concept pages gained orienting sentences; five Term rebalances; suspend/resume prereqs moved above `## Guide`.

**MCP re-check (25 August 2026, same day):** items 1, 5, and 6 below were re-verified against the live GQL schema and live v2 docs and are RESOLVED (fixes applied); strikethrough kept for the record. Live v2 also dates the old-onboarding retirement precisely: v1 onboarding creation stops 30 September 2026, deprecated v1 queries/mutations stay available until 31 December 2026 — the retirement inventory above should sequence deletions accordingly.

**Open items from the sweep (need SME/decision; pushed to DOC-1879/DOC-1882 comments 25 August 2026):**
1. ~~`memberships/export` chooser: prereq bullets have unclear provenance AND "Exports are scoped to one account at a time" contradicts both documented flows~~ — RESOLVED by schema + live: `exportAccountMembershipData` exports ALL of a project's memberships (`accountId` is an optional filter), and live v2 has no prerequisites/scope/"What's included" blocks (all three were added in migration without a source). Fixed: lede + Before-you-start rewritten to the schema-true scope. REMAINING for SME: the "What's included" CSV-column list is unverifiable without a real export — folded into pending-verification #10.
2. `change-admin/from-support-center`: the IBAN-country → identification-level table and the full-permissions invitation fact live ONLY on the ops walkthrough; the admin-change concept should own them (§10 move).
3. Onboarding create trios (company + individual): chooser has blocks between lede and picker, no `## Before you start`; API leaves lack `## Guide`/`## Mutation` h2s; `_prereqs-create.mdx` is misnamed (contains a methods overview duplicating the picker copy). Fold into DOC-1879.
4. `company/requirements.mdx` restates reference-owned facts (related-parties rules, legalForms tip, `monthlyPaymentVolume` enum) — DRY surgery.
5. ~~Country-requirements SME: Belgium KVK label / mandatory contradiction; France "Statuts de l'association"; Italy UBO row order~~ — RESOLVED against live v2: Italy dedup + position verified correct (live has one UBO row, after Statuto sociale); Belgium associations row was cooperative-content under a wrong "KVK" label — replaced with live's row ("Uittreksel UBO-register", UBO register extract, NBB-authorization details), and the cooperatives label fixed to live's "UBO-verklaring"; the "Mandatory under Belgian law" + no-asterisk pairing is live-verbatim (it states the company's legal obligation, not Swan's requirement — not a bug). France "Statuts de l'association" in the companies table is ALSO live-verbatim — an upstream content quirk to flag to the SME, not a repo bug.
6. ~~`identity-methods/recommendations.mdx` Belgium RRN footnote future tense~~ — RESOLVED: live v2's QES footnote was already rewritten in present tense ("Belgian passports don't display the... RRN..., which Swan is required to collect... Identity cards and residence permits remain acceptable"); our qes.mdx already matched, recommendations.mdx realigned to it (future-tense promise dropped).
7. `upload-onboarding/from-the-dashboard` states "onboarding status must be `Valid`" mid-steps, absent from the shared prereq partial and the API leaf — Dashboard-only condition or missing shared prereq?
8. Eleven read-only/export choosers have no `## Before you start` (account-holders/export + get-verification-status, account-operations/export + generate-statement + bank-details, billing/get-details + get-list, closure/get-closure-info, documents/get-info + upload-transaction + delete) — bless prereq-less choosers or extend SME item 2; the 21 Aug "every chooser" claim was over-broad.
9. Title ≠ H1 strays: sandbox/documents, sandbox/funding, concepts/funding/statuses — same harmonization decision as the onboarding requirements pages.
10. Statuses-diagram inconsistencies: accounts `account/statuses` + `onboarding/statuses` use a flowmap lifecycle while the other six machines use the §4 Mermaid shape; users consent + identifications statuses diagrams leave the `Canceled` node unstyled (§4 says red).
11. Borderline stub leaves: `account/transactions` (19-line cross-domain pointer), `ibans/main` (blocked on pending-verification #3).
12. Smaller quality notes kept for the content passes: memberships/notifications lede over-promises scope; funding statuses mandate subgraph label "Can use source" overstates; sign-up step 3 promises a computer/mobile difference its identical sub-branches don't deliver; login page ordering (reset before main-flow embed); users concepts-hub fragment descs; add-one/add-multiple member-vs-membership naming; implement-s2s `consents` query shown without fence/badge; configure-notifications duplicates the concept's Web Banking notification fact and states the endpoint-fallback twice; qes/recommendations/capital-deposits-reference inline-asterisk footnotes vs the §4 column rule; create-multiconsent h3 Mutation shape; multi-variant/multi-step API-guide shape drift extends recorded retrofit #1 (upload-transaction vs upload-onboarding step nesting, memberships/export h3 Payload).

### Payments type-misplacement pass (23 September 2026, user-approved "fix everything")

Detector sweep over all 139 Payments pages (explorer badges / GraphQL or JSON fences / Tabs / numbered steps on concept pages; Mermaid or definition partials on guides; stubs under 30 lines; hubs over 150 lines) then hand-verified. Reference pages clean, no stubs, "placeholder" hits were prose about test data. Fixed:

1. **VoP cluster (concept/guide inversion, four pages).** `concepts/verification-of-payee/index.mdx` 384→320 lines: path picker, six-step "Verification flow" list, and the eight Custom-UX tabs moved to the guide; "Integration flows" became a model-level `## Integration options {#integration-flows}` (comparison table + supported-payment-types table + Option 1/2 h3s WITH their flowcharts, anchors `#option-1/#option-2/#payment-support` kept); duplicate bulk-requirements table replaced by a pointer (h3 `#bulk-requirements-summary` kept as pointer); pathway flowmap added (two leaves). `guides/credit-transfers/sepa/incoming-vop-requests.mdx` (220 lines, zero procedures) MOVED to `concepts/verification-of-payee/incoming-requests.mdx` — both live redirects (`/preview/vop/...`, `/topics/.../vop/incoming-vop-requests`) retargeted directly, two changelog links (25 Sept 2025, 30 July 2026) retargeted, "Next steps" arrow tail dropped (rail carries it). `concepts/verification-of-payee/bulk-transfers.mdx` 181→82 lines: mutation + parameters + both payloads + rejection table + webhook + Sandbox moved to NEW guide `guides/credit-transfers/sepa/update-vop-settings.mdx` (single-path API shape, Environments partial); `## Updating account settings {#updating-settings}` kept as pointer; `webhooks.mdx` `AccountSetting.Updated` row retargeted to the guide's `#parameters`; the guide's "Breaking change for individual accounts" fact folded into the concept's `#account-settings`. `verify-beneficiary.mdx` gained `## How the custom flow works {#verification-flow}` and `### Custom UX examples {#custom-ux-examples}` (images `../../../../images/.../vop/PC_*.png`), its bulk section is now a two-line pointer. Sidebar: VoP concept cat +incoming-requests; SEPA guides −incoming-vop-requests +update-vop-settings; CT guides hub lane item swapped likewise. **Rule confirmed:** the `ia-path-picker` module belongs to guide choosers only — it was the sole instance on a concept page.
2. **Direct debit fat hub (177 lines, one leaf) split** into `received-mandates`, `received-instructions`, `r-transactions` leaves + statuses; hub keeps `#mandates`, `#received-instructions`, `#r-transactions` as one-paragraph previews and gained a pathway. Inbound retargets: add-mandate (`#received-instructions-new`), suspend-mandate (`#received-instructions-execution`), changelog 19 July 2024 (`#refunded`) and 26 April 2024 (`#reversed`). The mandate table's in-page `#specifications` links now point at `reference/schemes#received` directly. **Accepted loss:** live deep links to `direct-debit#rejected/#refunded/#reversed/#canceled` land on the hub's R-transactions preview (redirect is page-level; adding four pointer h3s judged clutter).
3. **Merchants hub (176 lines) trimmed**: `## Merchant profiles` and `## Payment methods` bodies moved to `merchants/profiles.mdx` (`#fields` added around the requirements table) and `merchants/payment-methods.mdx`; hub keeps both h2 anchors as previews. Inbound retargets: in-person + online guides hubs, profiles/update, accounts multiple-accounts, glossary (two entries: `#merchant-profile`, `#merchant-payment-method`). Sidebar rows added ahead of Statuses; pathway +2.
4. **International credit transfers**: the `Transaction.Booked` webhook payload JSON under `#incoming-notifications` replaced by prose pointing at `webhooks#events-transactions` and `#event-handling-info`. The `#dynamic-forms-order` four-step list was judged model (why the order matters), left in place.

5. **Credit transfers hub (24 September 2026, user-approved):** `## Beneficiaries` and `## Label and reference` bodies moved to `credit-transfers/beneficiaries.mdx` (trusted-beneficiary definition partial, permissions table `#beneficiaries-permissions`, statuses pointer `#beneficiaries-statuses`) and `credit-transfers/label-and-reference.mdx` (`#label-custom`, `#reference`, `#reference-external`, screenshot). Hub 107→58 lines, keeps both h2 anchors as previews, pathway +2. Ten inbound retargets: glossary (`#trusted-beneficiary`), verify-beneficiary token-permissions, get-transaction-list, membership-permissions ×2 (also `direct-debit/#mandates` → `received-mandates`), fraud-obligations, changelog 25 Sept 2025 / 27 Sept 2024 ×2 / 6 Sept 2024. **Lesson:** grep inbound anchors with an optional trailing slash (`/?#`) — changelog entries use `page/#anchor` and the first sweep missed seven of them.

6. **SEPA and International nested (24 September 2026, user-approved):** `credit-transfers/sepa.mdx` → `sepa/index.mdx` + `sepa/{instant,standing-orders,r-transactions}.mdx`; `credit-transfers/international.mdx` → `international/index.mdx` + `international/{outgoing,incoming}.mdx`. URLs unchanged (flat page → folder index), so no redirects; sidebar now nests two `cat(..., {link})` inside the Credit transfers concept cat (guides side already did). SEPA index keeps overview/types/consent/risk/limitations and pointer h2s `#instant`, `#standing-orders`, `#r-transactions`; **invoice references merged into `label-and-reference.mdx#invoice-references`** (same `label` field was documented twice) with a pointer h3 left on the SEPA index; `#beneficiary-verification` (pre-VoP Dutch IBAN check, Feb–Apr 2024) kept in place with four inbound links — **SME flag for DOC-1881: likely superseded by mandatory VoP since 9 Oct 2025; confirm before retiring.** International index keeps overview/risk/fees + pointer h2s `#outgoing`, `#dynamic-forms` (h3), `#incoming`; dynamic forms travel with Outgoing (outgoing-only API). Leaf anchors keep the legacy IDs (`#instant`, `#instant-fallback`, `#returned`, `#recalled`, `#transfer-limits`, `#incoming-flow`, `#incoming-intermediary-bic`, …) so retargets were mechanical: 10 inbound (glossary ×2, statements, cancel-ct, initiate-ct, get-info, sandbox/international, reference/international-transfers, changelog 21 Nov 2023 + 26 Apr 2024). Partial/image paths one level deeper (`../../../../`). Route count 479→484.

7. **Payments concepts sidebar order normalized (24 September 2026, user flagged "looks off"):** rule applied to every Payments concept category — flat cross-cutting leaves first, nested sub-categories next, **Statuses always last**; hub pathway flowmaps mirror the sidebar order. Changes: Credit transfers (Beneficiaries, Label and reference, SEPA ▸, International ▸, Statuses), Card payments (Statuses moved from first to last, hub pathway too), Merchants (Statuses moved from third to last, hub pathway too), VoP label casing → "Verification of Payee (VoP)" to match the page title. **Not touched (Accounts domain):** onboarding, memberships, documents, and funding categories also have Statuses mid-list — apply the same rule in the next Accounts pass.

8. **Second unsplit sweep + Guides/Reference ordering (24 September 2026, user-approved "Do it"):** `verification-of-payee/integration-options.mdx` NEW (comparison table, supported payment types, Option 1/2 flowcharts; hub 320→239, keeps `#integration-flows/#option-1/#option-2` as pointers; 4 inbound retargeted incl. changelog 25 Sept 2025). `cards/payment-examples.mdx` NEW (the eight Tabs; auth page 313→128, `#examples` pointer, lede trimmed). `merchants/mandates.mdx` NEW — SDD + IDD mandate model consolidated with **method-scoped anchors** (`#sdd`, `#sdd-declare`, `#sdd-info`, `#sdd-oneoff`, `#idd`, `#idd-info`, `#idd-content*`, `#idd-signature*`, `#idd-received`, `#idd-oneoff`) because both source pages used `#mandates-info`/`#mandates-oneoff`; method pages keep `## Payment mandates {#mandates}` as pointers (SDD 227→183, IDD 198→110); 8 inbound retargeted (sdd/idd declare guides ×6, direct-debit received-mandates, glossary). `merchants/fnci.mdx` NEW (french-checks 229→179, `#fnci` pointer, checks/initiate retargeted). **Plan deviation, reported to user:** the proposed `merchants/r-transactions.mdx` consolidation was NOT done — five method-specific R-transaction sections (~220 lines, colliding `#rejected/#returned` anchors) would have recreated the fat page elsewhere; FNCI extraction replaced it for French checks. Ordering: Reference sidebar SEPA-first (matches hub lanes); SEPA guides + CT hub lane in lifecycle order (add beneficiary → verify → initiate → cancel → refund → SO ×2 → VoP settings); Payment operations payments-before-transactions; Merchants guides Profiles ▸ before Missing information (matches hub); in-person cards `accept` roadmap first (matches every sibling); Sandbox SEPA → International → SDD in sidebar + hub. **Gaps noted, not fixed:** Guides `Payment cards` cat and Merchants `Profiles` sub-cat have no hub/link.

Not changed, verified as fine: authorization-and-clearing (313 lines) — Tabs are merchant scenarios, numbered lists are event sequences; merchant method pages share one consistent shape; direct debit / merchant hub numbered lists describe Swan's behavior, not reader steps. Route count 471→477.

### Cards `card-fraud-types` dissolved (11 September 2026)

The Cards reference leaf `cards/reference/card-fraud-types` was a WIP placeholder resting on a wrong premise: its TODO:SME note (and DOC-1880-CARDS-PLAN decision 5) claimed live had no standalone fraud-types page. Live has "Common types of fraud" at `/partnership/protections/fraud/types`, whose Card section (remote purchase, card-not-present, lost/stolen) already migrated to `get-started/become-a-partner/protections/fraud-obligations.mdx#card`; `redirects.js` already points the live URL there. A Cards leaf would restate a fact with an existing canonical home (Base rule 2), and no other source exists for a standalone table: the live schema has no fraud-type enum (fraud appears only as reason codes `FraudSuspected` on rejected transactions and `Fraudulent` on check returns), Linear has no card-fraud taxonomy in flight, and the Slack/Linear trail (DOC-923, DOC-1558) all points to the same Support Center article the fraud-obligations section mirrors. Applied per the stub-leaf rule (§10) with the `guides/onboarding/requirements.mdx` precedent: page deleted, sidebar entry removed, Cards reference hub lost its Fraud lane and the "fraud types" lede mention, and `fraud-obligations` joined the hub's `related` rail. **No redirect** (URL never shipped to live). **No cross-domain flowmap item**: no hub flowmap on the branch points outside its own domain, and `account/transactions` is already recorded as a borderline stub for being a cross-domain pointer — the `related` rail is the sanctioned cross-domain link. The orphan-sweep note in §8 item 10 stays as history. **Payments twin still open:** `payments/reference/transaction-fraud-types.mdx` is a bare shell stub (DOC-1814 blockquote, no placeholder partial) with the same likely answer (fraud-obligations `#types` owns the taxonomy; `RejectedReasonCode`/`TransactionReasonCode` own the codes) — settle it in the Payments pass, not by analogy. Placeholder tallies unchanged (this was a WIP placeholder, not a seeded one). Housekeeping spotted, not fixed: `fraud-obligations.mdx` intro says "Review the page covering common types of fraud" for what is now a same-page anchor.

### Payments `transactions` fat hub split (22 September 2026)

**Revised the same day (user):** Payments is NOT nested under Transactions — a sidebar category implies containment and payments are made of transactions, not the reverse. Final shape: sibling categories under Concepts, **Payments first** (`payments/concepts/payments/index.mdx` hub + `payments/concepts/payments/statuses.mdx` carrying the payment machine; one-leaf hub, so no pathway flowmap per the 30 July rule), then **Transactions** (`index` + `sequence-number` + `statements` + `statuses`, the latter now holding only the transaction and statement machines with `#payments-statuses` kept as a one-line pointer). The `transactions/payments.mdx` leaf created earlier that day was moved, not kept (never built to a shipped URL). The Transactions hub keeps `#payments` as a one-sentence relationship line pointing at the Payments hub. Overview + concepts-hub flowmap `pay` nodes retarget to `/payments/concepts/payments`. Note `merchants/payments.mdx` is a different object (merchant payment) and is untouched. Original record follows for the reasoning that still applies.

**Second revision (23 September 2026, user):** `transactions/sequence-number.mdx` DISSOLVED. Its `## Query` section (explorer badge + `GetTransactionSequenceNumber` fence) was guide material carried over from the legacy hub — §1 folder typing says concepts hold no procedures, and `guides/payment-operations/get-transaction-info#query-single` ALREADY requests `balanceAfter.sequenceNumber` (both its sample queries and payloads include it). Without the query the leaf was six sentences + a note, i.e. a stub (§10 stub-leaf rule). Model facts folded back into the hub as `### Sequence number {#sequence-number}` (increment, null unless `Booked`, statements ordering, schema location, backfill note) with a tail link to the guide; the guide gained one step bullet naming `balanceAfter` on `BookedTransactionStatusInfo` with a link back to the concept anchor. Sidebar row and pathway item removed; the one external inbound (`accounts/concepts/account/statements`) retargeted to the hub anchor. No redirect (URL lived one day on the branch). **Detector for the rest of DOC-1881:** any concept page carrying an `explorer-badge` is misplaced guide content — grep `docs/payments/concepts` for it. Transactions category is now `index` + `statements` + `statuses`.

`payments/concepts/transactions/index.mdx` (182 lines, one `statuses` leaf) carried the full Payment model plus four leaf-weight sub-concepts under a "Transactions" title — the fat-hub shape (§12 detector) that Credit transfers and Cards already avoid. Split per §10 (user-approved): three new concept leaves `transactions/payments.mdx` (types, payment ID, push/pull, risk — sub-anchors `#payments-types/-id/-push-pull/-risk` travel with the content), `transactions/sequence-number.mdx` (GraphQL query + explorer badge + backfill note), `transactions/statements.mdx` (eligibility, webhook, seven-day expiry; sole consumer of `_transaction-statements.mdx`). The hub keeps every existing h2/h3 anchor (`#payments`, `#sequence-number`, `#transactions-statements` become one-line previews per the funding-statuses precedent), keeps Transaction ID, supporting documents, and the PSD2 sharing section, and gained the concept-hub-floor `pathway` flowmap (4 leaves). **Statement statuses stay on `statuses.mdx`** (funding/capital-deposits precedent: one statuses page collects the domain's machines; the leaf points to it). Sidebar: three `doc()` rows added above Statuses. Inbound retargets: 7 links (payments overview + concepts hub flowmap nodes → `/payments`; get-transaction-info ×2, accounts statements, webhooks → `/statements`; accounts statements → `/sequence-number`). No redirects (URLs never shipped to live). Also fixed on the way: `accounts/concepts/account/transactions.mdx` said "six possible statuses" while the statuses page documents eight (`TransactionStatus` has `Settled` + `Deferred`). **Open for DOC-1881:** whether `#transactions-sharing-info` (PSD2 / 180-day SCA) belongs on this hub or on `users/concepts/consent` — left in place, flag for SME comment.
...
11. **Country-requirements tables verified against live v2 (21 August 2026)** — the Mandatory column now matches the live asterisks, with these fixes applied: France associations `Procès verbal du président` Yes→No; Spain associations `Documento de identidad` and `Justificante de domicilio` Yes→No plus missing mandatory row `Certificación de estatutos sociales` added; Italy missing mandatory rows added (companies `Documento d'identità del titolare effettivo`, associations `Visura camerale / Iscrizione al RUNTS`); Netherlands associations missing mandatory `Lidmaatschapsovereenkomst` (+non-mandatory `Organisatiestructuur`) added; **Germany was missing its entire Associations and Homeowners (WEG) sections — both imported verbatim from live**. Belgium matches fully. REMAINING for SME: the non-mandatory row sets still differ from live in France associations (live adds Justificatif de situation financière, Structure de gouvernance, Liste des membres, Déclaration des bénéficiaires effectifs, Décret d'utilité publique, Déclaration sur l'honneur; ours has Avis d'imposition and Premier transfert that live lacks), Spain (live adds identity-document rows in companies/self-employed and estructura/PBC-FT/cuestionario rows in associations), and Italy associations (live adds procura/organigramma/modulo rows). Reconcile row-by-row against live before publishing; live is authoritative.
12. **Public onboarding mutations have no guide leaf** (found in the 21 August 2026 full scan): `migrate-api.mdx` names `createPublicIndividualAccountHolderOnboarding`, `updatePublicIndividualAccountHolderOnboarding`, `createPublicCompanyAccountHolderOnboarding`, `updatePublicCompanyAccountHolderOnboarding`, and `publicAccountHolderOnboarding` as current supported surface, but the create trios document only the authenticated variants and the update guides only the authenticated mutations. Scope decision needed: an SRP dev leaf for public-link flows (natural home: a `create` sibling) vs an explicit scoping note pointing at api-reference. Related: the FRONT-1910 public-links create/update toggle item above.
...
- ~~`_shared/partials/_transaction-statuses.mdx` retier~~ RESOLVED 22 September 2026: the partial no longer exists on the branch and `transactions/statuses.mdx` inlines the table; nothing to retier. The `_transaction-statements.mdx` partial has one consumer (`transactions/statements.mdx`, moved from the hub the same day) plus the glossary — dual-consumption contract intact.
...
- **Old onboarding API retirement (end of September 2026):** everything below retires with the previous onboarding mutations and queries. Per §10 freshness rules, confirm the removal actually shipped before deleting anything — deploys slip. The full list (inventoried 13 August 2026):
  ...
  - The `guides/onboarding/partials/_deprecated-mutation-timeline.mdx` partial and the deprecation admonitions on its consumers: the onboarding hub, `manage-onboardings/get-info` and `manage-onboardings/finalize` (formerly one `manage-onboardings` page), `company/update`, `individual/update`, and both create `using-the-api` guides.
...
- **Docs v2/v3 fork alignment (deferred by decision, 12 August 2026):** this branch forked from live in June 2026; live Docs v2 kept evolving. Repo changelog stops at 7 May 2026; live has 18 June, 16 July, 30 July entries. During alignment, sweep ALL pages against live v2 and the post-May changelog entries. **Post-fork changelog sweep for Users + Accounts done (28 August 2026):** 18 June — virtual IBAN status filtering documented on `ibans/get-info/using-the-api.mdx` (`filters` arg, `VirtualIbanEntriesFilterInput`, schema-verified); 2026 reason codes already aligned; FRONT-1910 tracked on DOC-1879; 255-char company-name limit stated nowhere in v3, no drift. 30 July — unified document rejection labels verified: `documents/statuses.mdx#refused` matches the live `SupportingDocumentReasonCode` enum member-for-member. Remaining entries are Cards/Payments: 16 July `cancelPhysicalCardRenewal` + `addDigitalCards` undocumented (→ DOC-1880); 30 July VoP `Closing`-status matching re-check on `incoming-vop-requests.mdx` + `addReceivedSepaDirectDebitB2bMandate` SEPA-format validation from 31 August, no note on `direct-debit/add-mandate.mdx` (→ DOC-1881). Drift found and already fixed in v3 (12 August 2026): `accounts/concepts/closure/reason-codes.mdx` June reversal (`DormantAccount` deprecated → `Inactivity` reinstated; `CapitalDepositCanceled` → `CapitalDepositWithdrawn` added; retroactive migration of pre–2 April closures). Fixed same day but absent from live v2 (inherited gaps, pre-fork): funding `FundingSourceStatus.Rejected` and `AccountVerificationStatus.PendingReview`, supporting-document reason-code table conflation — v2 needs these ported the other way.
