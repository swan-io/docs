---
title: Using the API
audience:
  - dev
counterpart: /accounts/guides/closure/close-link/from-the-dashboard
related:
  - /accounts/guides/closure/close-link
  - /accounts/guides/closure/close-link/from-the-dashboard
  - /accounts/concepts/closure
---

# Send a link to close an account using the API {#api}

Generate a closure consent link with the API and send it to the account's <Term id="legal-representative">legal representative</Term>.

:::tip Prerequisites
You need the legal representative's [user access token](/build/using-api/authentication#tokens-user).
:::

import SuspendedCaution from '../partials/_suspended-caution.mdx';

<SuspendedCaution />

## Guide {#guide}

1. Call the `closeAccount` mutation with the legal representative's [user access token](/build/using-api/authentication#tokens-user).
1. The API returns a `consentUrl`.
1. Send the `consentUrl` to the account's legal representative so they can complete [Strong Customer Authentication](/users/concepts/consent/sca).
1. As soon as the legal representative consents, the account status changes to `Closing`.

:::note Account statuses
Refer to the [account statuses](/accounts/concepts/account/statuses) section for an overview of what happens while an account is `Closing`.
Consider subscribing to the `Account.Closing` and `Account.Closed` [webhooks](/build/using-api/webhooks#events-accounts) to be notified when the account status changes.
:::

## Mutation {#mutation}

The example below shows the success payload only; the Explorer link also includes the rejection fragments.

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {4-6,12} showLineNumbers
mutation closeAccount {
  closeAccount(
    input: {
      accountId: "$YOUR_ACCOUNT_ID"
      reason: { type: ClosingRequested }
      consentRedirectUrl: "$YOUR_REDIRECT_URL"
    }
  ) {
    ... on CloseAccountSuccessPayload {
      __typename
      consent {
        consentUrl
      }
    }
  }
}
```

## Payload {#payload}

```json {6} showLineNumbers
{
  "data": {
    "closeAccount": {
      "__typename": "CloseAccountSuccessPayload",
      "consent": {
        "consentUrl": "https://identity.swan.io/consent?consentId=$YOUR_CONSENT_ID&env=Sandbox"
      }
    }
  }
}
```
