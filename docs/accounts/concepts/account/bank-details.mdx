...
The PDF is generated automatically after the account's main IBAN is assigned, meaning the account must have the [payment level `Unlimited` and the account type `PaymentService`](/accounts/concepts/account/type-and-level).
