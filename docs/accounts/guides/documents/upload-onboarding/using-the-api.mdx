---
title: Using the API
audience:
  - dev
counterpart: /accounts/guides/documents/upload-onboarding/from-the-dashboard
related:
  - /accounts/guides/documents/upload-onboarding
  - /accounts/guides/documents/upload-onboarding/from-the-dashboard
  - /accounts/concepts/documents
---

# Upload an onboarding document using the API

Upload <Term id="supporting-documents">supporting documents</Term> for an account onboarding with the API.

import UploadOnboardingPrerequisites from '../partials/_upload-onboarding-prereqs.mdx';

:::tip Prerequisites
<UploadOnboardingPrerequisites />
:::

:::info Webhooks
Subscribe to the `SupportingDocumentCollection` [webhook event types](/build/using-api/webhooks#events) to get automatic updates about collections and missing required documents.
These webhooks only work if you used the API to submit documents.
:::

## Guide {#guide}

Use the API to upload required documents in three steps.

### Step 1: Generate upload URL {#guide-generate-url}

1. Call the `generateSupportingDocumentUploadUrl` mutation.
1. Enter all required information: `supportingDocumentCollectionId`, `filename`, `supportingDocumentPurpose`, and `supportingDocumentType`.
1. Add the `GenerateSupportingDocumentUploadUrlSuccessPayload` with required `key/value` pairs and `url`.
1. Add other optional messages to the payload, either for validation or in case of rejection.

<a href="https://explorer.swan.io?query=%3D&tab=api" className="explorer-badge">Open in API Explorer</a>

```graphql {4-7,13-15,17} showLineNumbers
mutation GenerateUploadUrl {
  generateSupportingDocumentUploadUrl(
    input: {
      supportingDocumentCollectionId: "$COLLECTION_ID"
      filename: "$DOCUMENT_FILE"
      supportingDocumentPurpose: CompanyRegistration
      supportingDocumentType: RegisterExtract
    }
  ) {
    ... on GenerateSupportingDocumentUploadUrlSuccessPayload {
      __typename
      upload {
        fields {
          key
          value
        }
        url
      }
    }
  }
}
```

### Step 2: Recover generated URL from payload {#guide-recover-url}

import UploadRecoverUrl from '../partials/_upload-recover-url.mdx';

<UploadRecoverUrl />

```json {8-9,12-13,20-21,24-25,28-29,32-33,36-37,40} showLineNumbers
{
  "data": {
    "generateSupportingDocumentUploadUrl": {
      "__typename": "GenerateSupportingDocumentUploadUrlSuccessPayload",
      "upload": {
        "fields": [
          {
            "key": "Key",
            "value": "INPUT/865ed98c-64aa-4288-a51a-e2f199287ab8"
          },
          {
            "key": "X-Amz-Algorithm",
            "value": "AWS4-HMAC-SHA256"
          },
          {
            "key": "X-Amz-Credential",
            "value": "ASIAWHEXXLUXL7AGJ4VK/20220705/eu-west-1/s3/aws4_request"
          },
          {
            "key": "X-Amz-Date",
            "value": "20220705T153359Z"
          },
          {
            "key": "X-Amz-Security-Token",
            "value": ""
          },
          {
            "key": "Policy",
            "value": ""
          },
          {
            "key": "X-Amz-Signature",
            "value": "994a3ba731d27a4606b90f4adc60b85af80ec8d2f048ffc37ebb97833c06657e"
          },
          {
            "key": "X-Amz-Meta-X-Swan-Supporting-Document-Id",
            "value": "865ed98c-64aa-4288-a51a-e2f199287ab8"
          }
        ],
        "url": "https://s3.eu-west-1.amazonaws.com/swan-supporting-document-master-sandbox"
      }
    }
  }
}
```

### Step 3: Send POST request {#guide-send-post}

import UploadPostRequest from '../partials/_upload-post-request.mdx';

<UploadPostRequest />

:::info Programmatic uploads
When uploading via code (not cURL), send the file as a readable stream and make sure your HTTP client sets the `Content-Length` header correctly. Some libraries require this explicitly for multipart form uploads.
:::

