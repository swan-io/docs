...
* [Collection status](./about.mdx#collection-statuses) must be `Waiting for document`.
* [Document status](./about.mdx#document-statuses) must be `Waiting for upload`.
:::
...
1. In the API Explorer, add the `generateSupportingDocumentUploadUrl` mutation.
