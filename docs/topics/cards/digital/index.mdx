...
Subscribe to the `DigitalCard.Created` and `DigitalCard.Updated` [webhooks](../../../developers/using-api/webhooks.mdx#events-cards) to get notified when a digital card is created and each time its status changes.
