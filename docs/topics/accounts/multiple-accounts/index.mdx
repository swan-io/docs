...
**Multiple Accounts allows a single company to create additional Swan accounts** under the same legal entity. These accounts share the same registration number and [local IBAN](/topics/accounts/ibans/#local) as the first account created during onboarding.
...
Multiple Accounts is available for **[company account](/topics/onboarding/company/) types**, which include:
...
Multiple Accounts is available to all partners, but **isn't activated** by default. Contact your Strategic Account Manager (SAM) to request activation before creating accounts.
...
To create an account for another company (with a different registration number) or for a different establishment (a branch of the same company at a new address), the user must complete a new company onboarding process. 
...
This setting is activated by default. Unless you deactivate it, end users will be able to create multiple accounts from Swan's interface. 
