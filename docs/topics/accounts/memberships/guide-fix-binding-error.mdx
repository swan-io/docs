...
**For all other binding error types**, only account members with the `canManageAccountMembership` permission and status `Enabled` can resolve the error using the API mutation below:
