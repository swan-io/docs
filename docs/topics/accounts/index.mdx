...
| `Canceled` | An `Upcoming` transaction is canceled by someone with the right to do so, such as the [account holder](../../glossary.mdx#account-holder), an [account member](../../glossary.mdx#account-membership), or a [merchant](../../glossary.mdx#merchants). Only transactions with the status `Upcoming` can be `Canceled`, and `Canceled` transactions don't impact the account balance. |
