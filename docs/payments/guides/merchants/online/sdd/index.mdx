...
|---|---|
| `availableToCancel` | Pending amount that can still be canceled. Cancellations must be requested by the day before the scheduled execution and within the specified [cutoff time](/payments/guides/merchants/online/sdd/#canceling). |
...
[Update their profile](./update.mdx) by calling the `requestMerchantPaymentMethodsUpdate` mutation.
...
You must [declare mandates to Swan](./declare.mdx) using the API.
...
When an SDD transaction is **returned**, the debtor's account is debited the amount, but the payment is reversed and the funds return to the debtor's account.
...
- [Accept payments with SEPA Direct Debit (step-by-step)](./accept.mdx)
- [Request SEPA Direct Debit payment method](./request.mdx)
- [Declare a payment mandate](./declare.mdx)
- [Get SEPA Direct Debit mandate URL](./get-mandate.mdx)
- [Initiate a SEPA Direct Debit transaction](./initiate.mdx)
- [Update SEPA Direct Debit payment method](./update.mdx)
- [Sandbox](/payments/reference/sandbox/sandbox-merchant-sdd)