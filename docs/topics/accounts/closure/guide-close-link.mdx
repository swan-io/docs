...
To complete the account closure process, the account must meet all [conditions to move from `Closing` to `Closed`](./index.mdx#closing-to-closed-conditions). A zero balance is one of these conditions, along with the SEPA Credit Transfer timing and the `Upcoming`/`Pending` transaction checks.
...
| ➖ Negative | Swan works with your end users to reach a zero balance. The account can also receive incoming SEPA Credit Transfers to help clear the debt. |
